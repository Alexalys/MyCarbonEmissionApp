...
class Connection(object):  # pylint:disable=r0902,r0904
    ...
    def __init__(
        ...
        if log:
            wmsg = (
                "log parameter is deprecated and will be remove in 0.3.0. "
                "Use cnopts param."
            )
        ...
        if ciphers is not None:
            wmsg = (
                "ciphers parameter is deprecated and will be remove in "
        ...
        with self.cd(remotedir):
            for sattr in self._sftp.listdir_attr("."):
                if S_ISREG(sattr.st_mode):
                    ...
                    self.get(
                        rname, reparent(localdir, rname), preserve_mtime=preserve_mtime
                    )
    ...
    def get_r(self, remotedir, localdir, preserve_mtime=False):
        ...
        for fname in wtcb.flist:
            ...
            self.get(fname, reparent(localdir, fname), preserve_mtime=preserve_mtime)
    ...
    def put(
        ...
        sftpattrs = self._sftp.put(
            localpath, remotepath, callback=callback, confirm=confirm
        )
        if preserve_mtime:
    ...
    def putfo(self, flo, remotepath=None, file_size=0, callback=None, confirm=True):
        ...
        """
        self._sftp_connect()
        return self._sftp.putfo(
            flo, remotepath, file_size=file_size, callback=callback, confirm=confirm
        )
    ...
    def listdir_attr(self, remotepath="."):
        ...
        self._sftp_connect()
        return sorted(
            self._sftp.listdir_attr(remotepath), key=lambda attr: attr.filename
        )
    ...
    def makedirs(self, remotedir, mode=777):
        ...
        if self.isdir(remotedir):
            ...
            raise OSError(
                "a file with the same name as the remotedir, "
