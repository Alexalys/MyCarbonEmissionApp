...
from scapy.extlib import plt, Line2D, MATPLOTLIB_INLINED, MATPLOTLIB_DEFAULT_PLOT_KARGS
...
class _PacketList(Generic[_Inner]):
    ...
    def __repr__(self):
        ...
        for p in self.stats:
            s += " %s%s%s" % (
                ct.packetlist_proto(p._name),
                ct.punct(":"),
                ct.packetlist_value(stats[p]),
            )
        s += " %s%s%s" % (
            ct.packetlist_proto("Other"),
            ct.punct(":"),
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):
            return self.__class__(
                self.res.__getitem__(item), name="mod %s" % self.listname
            )
    ...
    def __add__(
        ...
        # type: (...) -> _PacketList._T
        return self.__class__(
            self.res + other.res, name="%s+%s" % (self.listname, other.listname)
        )
    ...
    def filter(self, func):
        ...
        return self.__class__(
            [x for x in self.res if func(*x)], name="filtered %s" % self.listname
        )
    ...
    def diffplot(
        ...
        **kargs  # type: Any
        ...
        if lfilter is None:
            lst_pkts = [
                f(self.res[i], self.res[i + 1]) for i in range(len(self.res) - delay)
            ]
        else:
            lst_pkts = [
        ...
        for i, res in enumerate(self.res):
            ...
            if p.haslayer(conf.padding_layer):
                if lfilter is None or lfilter(p):
                    ...
                    hexdump(p.getlayer(conf.padding_layer).load)  # type: ignore
    ...
    def afterglow(
        ...
        **kargs  # type: Any
        ...
        def minmax(x):
            ...
            m, M = reduce(
                lambda a, b: (min(a[0], b[0]), max(a[1], b[1])), ((a, a) for a in x)
            )
            if m == M:
                m = 0
            if M == 0:
        ...
        for s in sl:
            ...
            gr += (
                '"src.%s" [label = "%s", shape=box, fillcolor="#FF0000", style=filled, fixedsize=1, height=%.2f,width=%.2f];\n'
                % (repr(s), repr(s), n, n)
            )  # noqa: E501
        gr += "# event nodes\n"
        for e in el:
            ...
            gr += (
                '"evt.%s" [label = "%s", shape=circle, fillcolor="#00FFFF", style=filled, fixedsize=1, height=%.2f, width=%.2f];\n'
                % (repr(e), repr(e), n, n)
            )  # noqa: E501
        for d in dl:
            n = dl[d]
            ...
            gr += (
                '"dst.%s" [label = "%s", shape=triangle, fillcolor="#0000ff", style=filled, fixedsize=1, height=%.2f, width=%.2f];\n'
                % (repr(d), repr(d), n, n)
            )  # noqa: E501
    ...
    def canvas_dump(self, **kargs):
        ...
        for i, res in enumerate(self.res):
            ...
            c.text(
                cbb.left(),
                cbb.top() + 1,
                r"\font\cmssfont=cmss12\cmssfont{Frame %i/%i}" % (i, len_res),
                [pyx.text.size.LARGE],
            )  # noqa: E501
            if conf.verb >= 2:
                os.write(1, b".")
    ...
    def sessions(
        self, session_extractor=None  # type: Optional[Callable[[Packet], str]]
    ):
                ...
                if "Ether" in p:
                    if "IP" in p or "IPv6" in p:
                        ...
                        if "TCP" in p:
                            ...
                            fmt = (
                                "ICMP {} > {} type=%r,ICMP.type% code=%r,"
                                "ICMP.code% id=%ICMP.id%"
                            )
                        elif "ICMPv6" in p:
                            fmt = (
                                "ICMPv6 {} > {} type=%r,ICMPv6.type% "
                                "code=%r,ICMPv6.code%"
                            )
                        elif "IPv6" in p:
                            fmt = "IPv6 {} > {} nh=%IPv6.nh%"
    ...
    def getlayer(
        ...
        return PacketList(
            [
                pc
                for pc in (self._elt2pkt(p).getlayer(**getlayer_arg) for p in self.res)
                if pc is not None
            ],
            name,
            stats,
    ...
    def convert_to(
        ...
        return PacketList(
            [self._elt2pkt(p).convert_to(other_cls) for p in self.res], name, stats
        )
