...
class ASN1F_field(ASN1F_element):
    ...
    def i2m(self, pkt, x):
        ...
        if isinstance(x, ASN1_Object):
            if (
                self.ASN1_tag == ASN1_Class_UNIVERSAL.ANY
...
class ASN1F_enum_INTEGER(ASN1F_INTEGER):
    def __init__(
        self, name, default, enum, context=None, implicit_tag=None, explicit_tag=None
    ):
        ASN1F_INTEGER.__init__(
            self,
            name,
            default,
            context=context,
...
class ASN1F_BIT_STRING(ASN1F_field):
    ...
    def __init__(
        ...
        if default is not None and default_readable:
            default = b"".join(
                binrepr(orb(x)).zfill(8).encode("utf8") for x in default
            )  # noqa: E501
        ASN1F_field.__init__(
            self,
            name,
            default,
            context=context,
...
class ASN1F_SEQUENCE_OF(ASN1F_field):
    ...
    def __init__(
        self, name, default, cls, context=None, implicit_tag=None, explicit_tag=None
    ):
        self.cls = cls
        ASN1F_field.__init__(
            self,
    ...
    def build(self, pkt):
        ...
        if (
            isinstance(val, ASN1_Object) and val.tag == ASN1_Class_UNIVERSAL.RAW
        ):  # noqa: E501
            s = val
        elif val is None:
            s = b""
...
class ASN1F_CHOICE(ASN1F_field):
    ...
    def __init__(self, name, default, *args, **kwargs):
        ...
        for kwarg in ["context", "explicit_tag"]:
            setattr(self, kwarg, kwargs.get(kwarg))
        ASN1F_field.__init__(
            self, name, None, context=self.context, explicit_tag=self.explicit_tag
        )
        self.default = default
    ...
    def randval(self):
        ...
        for p in six.itervalues(self.choices):
            if hasattr(p, "ASN1_root"):  # should be ASN1_Packet class
                ...
                if isinstance(
                    p, type
                ):  # should be (basic) ASN1F_field class  # noqa: E501
...
class ASN1F_PACKET(ASN1F_field):
    ...
    def m2i(self, pkt, s):
        diff_tag, s = BER_tagging_dec(
            s,
            hidden_tag=self.cls.ASN1_root.ASN1_tag,  # noqa: E501
            implicit_tag=self.implicit_tag,
            explicit_tag=self.explicit_tag,
            safe=self.flexible_tag,
        )
...
class ASN1F_FLAGS(ASN1F_BIT_STRING):
    def __init__(
        self, name, default, mapping, context=None, implicit_tag=None, explicit_tag=None
    ):
