...
class DestField(Field[str, bytes]):
    ...
    def bind_addr(cls, layer, addr, **condition):
        ...
        cls.bindings.setdefault(layer, []).append((addr, condition))  # type: ignore
...
class SourceIPField(IPField):
    ...
    def __findaddr(self, pkt):
        ...
        if isinstance(dst, (Gen, list)):
            r = {
                conf.route.route(str(daddr)) for daddr in dst
...
class SourceIP6Field(IP6Field):
    ...
    def i2h(self, pkt, x):
        ...
        if x is None:
            ...
            dst = (
                "::" if self.dstname is None else getattr(pkt, self.dstname)
            )  # noqa: E501
            if isinstance(dst, (Gen, list)):
...
class LEThreeBytesField(ByteField):
    ...
    def getfield(self, pkt, s):
        ...
        return s[3:], self.m2i(
            pkt, struct.unpack(self.fmt, s[:3] + b"\x00")[0]
        )  # noqa: E501
...
class PacketListField(_PacketField[List[BasePacket]]):
    ...
    def __init__(
        ...
        super(PacketListField, self).__init__(name, default, pkt_cls)  # type: ignore
...
class NetBIOSNameField(StrFixedLenField):
    ...
    def i2m(self, pkt, y):
        ...
        x = x[:len_pkt]
        x = b"".join(
            chb(0x41 + (orb(b) >> 4)) + chb(0x41 + (orb(b) & 0xF)) for b in x
        )  # noqa: E501
        return b" " + x
    ...
    def m2i(self, pkt, x):
        ...
        return b"".join(
            map(
                lambda x, y: chb((((orb(x) - 1) & 0xF) << 4) + ((orb(y) - 1) & 0xF)),
                x[::2],
                x[1::2],
            )
        )
                ...
                if v is not None:
                    ...
                    for i in x:
                        for val in six.itervalues(these_names):
                            ...
                            assert (
                                False
                            ), 'Unknown flag "{}" with this dependency'.format(
                                i
                            )  # noqa: E501
                            continue
                    return s
...
class UTCTimeField(Field[float, int]):
    __slots__ = ["epoch", "delta", "strf", "use_msec", "use_micro", "use_nano"]
...
class _ScalingField(object):
    ...
    def i2repr(self, pkt, x):
        ...
        return "%s %s" % (self.i2h(pkt, x), self.unit)  # type: ignore
