...
def BER_len_dec(s):
    ...
    if len(s) <= tmp_len:
        raise BER_Decoding_Error(
            "BER_len_dec: Got %i bytes while expecting %i" % (len(s) - 1, tmp_len),
            remaining=s,
        )
    ll = 0
...
class BERcodec_Object(six.with_metaclass(BERcodec_metaclass)):
    ...
    def check_type_check_len(cls, s):
        ...
        if len(s3) < l:
            raise BER_Decoding_Error(
                "%s: Got %i bytes while expecting %i" % (cls.__name__, len(s3), l),
                remaining=s,
            )
        return l, s3[:l], s3[l:]
...
class BERcodec_BIT_STRING(BERcodec_Object):
    ...
    def do_dec(cls, s, context=None, safe=False):
        ...
        if len(s) > 0:
            ...
            raise BER_Decoding_Error(
                "BERcodec_BIT_STRING found no content " "(not even unused_bits byte)",
                remaining=s,
            )
    ...
    def enc(cls, s):
        ...
        if len(s) % 8 == 0:
            ...
            s += b"0" * unused_bits
        s = b"".join(
            chb(int(b"".join(chb(y) for y in x), 2)) for x in zip(*[iter(s)] * 8)
        )
        s = chb(unused_bits) + s
