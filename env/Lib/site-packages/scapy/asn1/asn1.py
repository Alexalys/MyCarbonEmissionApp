...
class RandASN1Object(RandField):
    ...
    def _fix(self, n=0):
        ...
        if issubclass(o, ASN1_INTEGER):
            ...
            z = RandIP()._fix()
            return o(z)
        elif issubclass(o, ASN1_GENERALIZED_TIME) or issubclass(
            o, ASN1_UTC_TIME
        ):  # noqa: E501
            ...
            return o(
                [self.__class__(objlist=self.objlist)._fix(n + 1) for _ in range(z)]
            )
...
class ASN1Tag(EnumElement):
    ...
    def clone(self):  # not a real deep copy. self.codec is shared
        return self.__class__(
            self._key, self._value, self._context, self._codec
        )  # noqa: E501
...
class ASN1_Class_metaclass(Enum_metaclass):
    ...
    def __new__(
        cls, name, bases, dct
    ):  # XXX factorise a bit with Enum_metaclass.__new__()  # noqa: E501
        for b in bases:
...
class ASN1_DECODING_ERROR(ASN1_Object):
    ...
    def __repr__(self):
        return "<%s[%r]{{%r}}>" % (
            self.__dict__.get("name", self.__class__.__name__),  # noqa: E501
            self.val,
            self.exc.args[0],
        )
        ...
        if name == "val_readable":
            ...
            if isinstance(value, str):
                if any(c for c in value if c not in ["0", "1"]):
                    ...
                    val_readable = b"".join(
                        chb(int("".join(x), 2)) for x in bytes_arr
                    )  # noqa: E501
            else:
...
class ASN1_UTC_TIME(ASN1_STRING):
    ...
    def __setattr__(self, name, value):
        ...
        if name == "val":
            ...
            if isinstance(value, str) and len(value) == _len and value[-1] == "Z":
