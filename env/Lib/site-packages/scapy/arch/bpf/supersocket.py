...
class _L2bpfSocket(SuperSocket):
    ...
    def __init__(
        ...
        try:
            fcntl.ioctl(
                self.ins, BIOCSBLEN, struct.pack("I", BPF_BUFFER_LENGTH)
            )  # noqa: E501
        except IOError:
        ...
        try:
            fcntl.ioctl(
                self.ins, BIOCSETIF, struct.pack("16s16x", self.iface.encode())
            )  # noqa: E501
        except IOError:
    ...
    def set_promisc(self, value):
        ...
        try:
            ...
            raise Scapy_Exception(
                "Cannot set promiscuous mode on interface " "(%s)!" % self.iface
            )
...
class L2bpfListenSocket(_L2bpfSocket):
    ...
    def extract_frames(self, bpf_buffer):
        ...
        bh_caplen = struct.unpack(
            "I", bpf_buffer[bh_tstamp_offset : bh_tstamp_offset + 4]
        )[
            0
        ]  # noqa: E501
        next_offset = bh_tstamp_offset + 4
        bh_datalen = struct.unpack("I", bpf_buffer[next_offset : next_offset + 4])[
            0
        ]  # noqa: E501
        next_offset += 4
        ...
        self.received_frames.append((self.guessed_cls, frame_str, None))
...
class L3bpfSocket(L2bpfSocket):
    ...
    def send(self, pkt):
        ...
        if self.assigned_interface != iff:
            try:
                fcntl.ioctl(
                    self.outs, BIOCSETIF, struct.pack("16s16x", iff.encode())
                )  # noqa: E501
            except IOError:
