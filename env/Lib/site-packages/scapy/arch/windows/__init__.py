...
def win_find_exe(filename, installsubdir=None, env="ProgramFiles"):
    ...
    """
    fns = (
        [filename] if filename.endswith(".exe") else [filename + ".exe", filename]
    )  # noqa: E501
    for fn in fns:
        try:
...
class WinProgPath(ProgPath):
    ...
    def _reload(self):
        ...
        self.usbpcapcmd = win_find_exe(
            "USBPcapCMD", installsubdir="USBPcap", env="programfiles"
        )
        ...
        self.cscript = win_find_exe(
            "cscript", installsubdir="System32", env="SystemRoot"
        )
        self.cmd = win_find_exe("cmd", installsubdir="System32", env="SystemRoot")
        if self.wireshark:
            try:
                new_manuf = load_manuf(
                    os.path.sep.join(self.wireshark.split(os.path.sep)[:-1])
                    + os.path.sep
                    + "manuf"
                )
            except (
                IOError,
                OSError,
            ):  # FileNotFoundError not available on Py2 - using OSError  # noqa: E501
                log_loading.warning(
...
class NetworkInterface_Win(NetworkInterface):
    ...
    def _npcap_set(self, key, val):
        ...
        res, code = _exec_cmd(
            _encapsulate_admin(" ".join([_WlanHelper, self.guid[1:-1], key, val]))
        )
