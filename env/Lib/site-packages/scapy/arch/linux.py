...
def get_alias_address(
    ...
    names = [
        names_b[i : i + offset].split(b"\0", 1)[0] for i in range(0, out, name_len)
    ]  # noqa: E501
...
def read_routes():
    ...
    try:
        ifreq = ioctl(
            s, SIOCGIFADDR, struct.pack("16s16x", conf.loopback_name.encode("utf8"))
        )  # noqa: E501
        ...
        if addrfamily == socket.AF_INET:
            ...
            routes.append(
                (dst, msk, "0.0.0.0", conf.loopback_name, ifaddr, 1)
            )  # noqa: E501
        else:
            warning(
                "Interface %s: unknown address family (%i)"
                % (conf.loopback_name, addrfamily)
            )  # noqa: E501
    except IOError as err:
        if err.errno == 99:
            warning(
                "Interface %s: no address assigned" % conf.loopback_name
            )  # noqa: E501
        else:
            warning(
                "Interface %s: failed to get address config (%s)"
    ...
    for line_b in f.readlines()[1:]:
        ...
        try:
            ifreq = ioctl(
                s, SIOCGIFADDR, struct.pack("16s16x", iff.encode("utf8"))
            )  # noqa: E501
            ...
            if addrfamily == socket.AF_INET:
                ...
                warning(
                    "Interface %s: unknown address family (%i)", iff, addrfamily
                )  # noqa: E501
                continue
...
class L3PacketSocket(L2Socket):
    ...
    def send(self, x):
        ...
        if self.lvl == 3 and type_x != self.LL:
            warning(
                "Incompatible L3 types detected using %s instead of %s !",
                type_x,
                self.LL,
            )
            self.LL = type_x
...
class VEthPair(object):
    ...
    def up(self):
        ...
        for idx in [0, 1]:
            subprocess.check_call(
                ["ip", "link", "set", self.ifaces[idx], "up"]
            )  # noqa: E501
    ...
    def down(self):
        ...
        for idx in [0, 1]:
            subprocess.check_call(
                ["ip", "link", "set", self.ifaces[idx], "down"]
            )  # noqa: E501
