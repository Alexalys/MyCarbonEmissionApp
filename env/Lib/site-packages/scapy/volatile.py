    ...
    def next(self):
        while True:
            if self.turns == 0 or (self.i == 0 and self.renewkeys):
                ...
                self.sbox = [
                    self.rnd.randint(0, self.fsmask) for _ in range(self.sbox_size)
                ]
            self.turns += 1
...
class RandString(RandField):
    _DEFAULT_CHARS = (
        b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"  # noqa: E501
    )
...
class RandBin(RandString):
    def __init__(self, size=None):
        super(RandBin, self).__init__(
            size=size, chars=b"".join(chb(c) for c in range(256))
        )  # noqa: E501
    ...
    def _command_args(self):
        ...
        if (
            isinstance(self.size, RandNumExpo)
            and self.size.lambd == 0.01
            and self.size.base == 0
        ):
...
class RandTermString(RandBin):
    ...
    def _command_args(self):
        return ", ".join(
            (super(RandTermString, self)._command_args(), "term=%r" % self.term)
        )
...
class RandOID(RandString):
    ...
    def _command_args(self):
        ...
        if not isinstance(self.depth, VolatileValue):
            ...
            not isinstance(self.depth, RandNumExpo)
            or self.depth.lambd != 0.1
            or self.depth.base != 0
        ):
        ...
        if not isinstance(self.idnum, VolatileValue):
            ...
            not isinstance(self.idnum, RandNumExpo)
            or self.idnum.lambd != 0.01
            or self.idnum.base != 0
        ):
    ...
    def _fix(self):
        ...
        while i < ln:
            ...
            if c == "(":
                ...
                e = current.pop()
                current.append(
                    [current] + [e] * (int(random.expovariate(self._lambda)) + 1)
                )  # noqa: E501
            elif c == "*":
                e = current.pop()
                current.append(
                    [current] + [e] * int(random.expovariate(self._lambda))
                )  # noqa: E501
            elif c == "?":
    ...
    def __init__(
        ...
        if template:
            if node or clock_seq or namespace or name or version:
                raise ValueError(
                    "UUID template must be the only parameter, " "if specified"
                )
            ...
            if self.version == 1:
                if namespace or name:
                    raise ValueError(
                        "namespace and name may not be used with " "version 1"
                    )
                self.node = node
                ...
                if node or clock_seq:
                    raise ValueError(
                        "node and clock_seq may not be used with "
                ...
                if namespace or name or node or clock_seq:
                    raise ValueError(
                        "node, clock_seq, node and clock_seq may "
