...
def load_layer(name, globals_dict=None, symb_list=None):
    ...
    """
    _load(
        "scapy.layers." + LAYER_ALIASES.get(name, name),
        ...
        if _debug:
            if desc["status"] == "skip":
                ...
                raise Scapy_Exception(
                    "Module %s is missing its " "contrib infos !" % mod
                )
...
def save_session(fname="", session=None, pickleProto=-1):
    ...
    for k in list(to_be_saved):
        ...
        if hasattr(i, "__module__") and (
            k[0] == "_" or i.__module__.startswith("IPython")
        ):
            del to_be_saved[k]
        if isinstance(i, ConfClass):
            ...
            if k[0] != "_":
                log_interactive.error(
                    "[%s] (%s) can't be saved.", k, type(to_be_saved[k])
                )
            del to_be_saved[k]
...
def init_session(
    ...
    scapy_builtins = {
        k: v
        for k, v in six.iteritems(importlib.import_module(".all", "scapy").__dict__)
    ...
    if IPYTHON:
        ...
        try:
            ...
            if int(IPython.__version__[0]) >= 6:
                cfg.TerminalInteractiveShell.term_title_format = (
