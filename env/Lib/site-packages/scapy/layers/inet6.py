...
from scapy.data import DLT_IPV6, DLT_RAW, DLT_RAW_ALT, ETHER_ANY, ETH_P_IPV6, MTU
...
class _IPv6GuessPayload:
    ...
    def default_payload_class(self, p):
        if self.nh == 58:  # ICMPv6
            ...
            if len(p) >= icmp6typesminhdrlen.get(
                t, float("inf")
            ):  # Other ICMPv6 messages  # noqa: E501
...
class IPv6(_IPv6GuessPayload, Packet, IPTools):
        ...
        if self.plen == 0 and self.nh == 0 and len(data) >= 8:
            ...
            while offset <= len(data):
                ...
                if opt_type == 0xC2:  # Jumbo option
                    jumbo_len = struct.unpack("I", data[offset + 2 : offset + 2 + 4])[
                        0
                    ]  # noqa: E501
                    break
                offset = 4 * idx + 2
    ...
    def hashret(self):
        ...
        if self.nh == 43 and isinstance(
            self.payload, IPv6ExtHdrSegmentRouting
        ):  # noqa: E501
        ...
        if conf.checkIPsrc and conf.checkIPaddr and not in6_ismaddr(sd):
            ...
            return (
                strxor(sd, ss) + struct.pack("B", nh) + self.payload.hashret()
            )  # noqa: E501
        else:
    ...
    def answers(self, other):
        ...
        if conf.checkIPaddr:
            ...
            if in6_ismaddr(other.dst):
                if in6_ismaddr(self.dst):
                    if (od == sd) or (
                        in6_isaddrllallnodes(self.dst)
                        and in6_isaddrllallservers(other.dst)
                    ):  # noqa: E501
        ...
        if (
            self.nh == 58
            and isinstance(self.payload, _ICMPv6)
            and self.payload.type < 128
        ):  # noqa: E501
            ...
            )  # Buggy if self.payload is a IPv6ExtHdrRouting  # noqa: E501
        elif other.nh == 43 and isinstance(
            other.payload, IPv6ExtHdrSegmentRouting
        ):  # noqa: E501
            return self.payload.answers(
                other.payload.payload
            )  # Buggy if self.payload is a IPv6ExtHdrRouting  # noqa: E501
        ...
        elif self.nh == 60 and isinstance(
            self.payload, IPv6ExtHdrDestOpt
        ):  # BU in reply to BRR, for instance  # noqa: E501
...
class IPerror6(IPv6):
    ...
    def answers(self, other):
        ...
        if isinstance(self.underlayer, _ICMPv6) and self.underlayer.type < 128:
            ...
            if (ss == os and sd == od) or (ss == os and request_has_rh):  # < Basic case
                # ^ Request has a RH : don't check dst address

                # Let's deal with possible MSS Clamping
                if (
                    isinstance(selfup, TCP)
                    and isinstance(otherup, TCP)
                    and selfup.options != otherup.options
                ):  # seems clamped
...
def in6_chksum(nh, u, p):
    ...
    while u is not None and not isinstance(u, IPv6):
        if (
            isinstance(u, IPv6ExtHdrRouting)
            and u.segleft != 0
            and len(u.addresses) != 0
            and final_dest_addr_found == 0
        ):
            rthdr = u.addresses[-1]
            final_dest_addr_found = 1
        elif (
            isinstance(u, IPv6ExtHdrSegmentRouting)
            and u.segleft != 0
            and len(u.addresses) != 0
            and final_dest_addr_found == 0
        ):
            ...
            isinstance(u, IPv6ExtHdrDestOpt)
            and (len(u.options) == 1)
            and isinstance(u.options[0], HAO)
        ):
...
class Jumbo(Packet):  # IPv6 Hop-By-Hop Option
    name = "Jumbo Payload"
    fields_desc = [
        _OTypeField("otype", 0xC2, _hbhopts),
        ByteField("optlen", 4),
...
class HAO(Packet):  # IPv6 Destination Options Header Option
    ...
    fields_desc = [
        _OTypeField("otype", 0xC9, _hbhopts),
        ByteField("optlen", 16),
        IP6Field("hoa", "::"),
    ]
...
class IPv6ExtHdrHopByHop(_IPv6ExtHdr):
    ...
    fields_desc = [
        ...
        _PhantomAutoPadField(
            "autopad", 1
        ),  # autopad activated by default  # noqa: E501
        _OptionsField(
            "options",
            [],
            HBHOptUnknown,
...
class IPv6ExtHdrRouting(_IPv6ExtHdr):
    ...
    fields_desc = [
        ByteEnumField("nh", 59, ipv6nh),
        FieldLenField(
            "len", None, count_of="addresses", fmt="B", adjust=lambda pkt, x: 2 * x
        ),  # in 8 bytes blocks  # noqa: E501
        ByteField("type", 0),
...
class IPv6ExtHdrSegmentRoutingTLV(Packet):
    ...
    # RFC 8754 sect 2.1
    fields_desc = [
        ByteEnumField("type", None, _segment_routing_header_tlvs),
        ByteField("len", 0),
...
class IPv6ExtHdrSegmentRoutingTLVIngressNode(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [
        ByteEnumField("type", 1, _segment_routing_header_tlvs),
        ByteField("len", 18),
...
class IPv6ExtHdrSegmentRoutingTLVEgressNode(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [
        ByteEnumField("type", 2, _segment_routing_header_tlvs),
        ByteField("len", 18),
        ByteField("reserved", 0),
        ByteField("flags", 0),
...
class IPv6ExtHdrSegmentRoutingTLVPad1(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [
        ByteEnumField("type", 0, _segment_routing_header_tlvs),
...
class IPv6ExtHdrSegmentRoutingTLVPadN(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [
        ByteEnumField("type", 4, _segment_routing_header_tlvs),
        FieldLenField("len", None, length_of="padding", fmt="B"),
        StrLenField("padding", b"\x00", length_from=lambda pkt: pkt.len),
    ]  # noqa: E501
...
def defragment6(packets):
    ...
    if len(lst) != llen:
        warning(
            "defragment6: some fragmented packets have been removed from list"
        )  # noqa: E501
    ...
    for p in res:
        ...
        if offset != len(fragmentable):
            warning(
                "Expected an offset of %d. Found %d. Padding with XXXX"
...
class _ICMPv6(Packet):
    ...
    def answers(self, other):
        ...
        if (
            isinstance(self.underlayer, IPerror6)
            or isinstance(self.underlayer, _IPv6ExtHdr)
            and isinstance(other, _ICMPv6)
        ):
...
class ICMPv6PacketTooBig(_ICMPv6Error):
    ...
    fields_desc = [
        ByteEnumField("type", 2, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        IntField("mtu", 1280),
    ]
...
class ICMPv6EchoReply(ICMPv6EchoRequest):
    ...
    def answers(self, other):
        ...
        return (
            isinstance(other, ICMPv6EchoRequest)
...
class ICMPv6MLDMultAddrRec(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField(
            "sources_number", None, length_of="sources", adjust=lambda p, num: num // 16
        ),
        IP6Field("dst", "::"),
...
class ICMPv6MLReport2(_ICMPv6):  # RFC 3810
    ...
    fields_desc = [
        ...
        PacketListField(
            "records", [], ICMPv6MLDMultAddrRec, count_from=lambda p: p.records_number
        ),
    ]
...
class ICMPv6MRD_Advertisement(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 151, icmp6types),
...
class ICMPv6MRD_Solicitation(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 152, icmp6types),
        ByteField("res", 0),
        XShortField("cksum", None),
    ]
    overload_fields = {IPv6: {"nh": 58, "hlim": 1, "dst": "ff02::2"}}
...
class ICMPv6MRD_Termination(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 153, icmp6types),
        ByteField("res", 0),
        XShortField("cksum", None),
    ]
...
class _ICMPv6NDGuessPayload:
    ...
    def guess_payload_class(self, p):
        if len(p) > 1:
            return icmp6ndoptscls.get(
                orb(p[0]), Raw
            )  # s/Raw/ICMPv6NDOptUnknown/g ?  # noqa: E501
...
class ICMPv6NDOptUnknown(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField("type", None),
        FieldLenField(
            "len", None, length_of="data", fmt="B", adjust=lambda pkt, x: x + 2
        ),
...
class TruncPktLenField(PacketLenField):
    ...
    def __init__(
        self, name, default, cls, cur_shift, length_from=None, shift=0
    ):  # noqa: E501
        PacketLenField.__init__(
...
class ICMPv6NDOptRedirectedHdr(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField("type", 4),
        FieldLenField(
            "len", None, length_of="pkt", fmt="B", adjust=lambda pkt, x: (x + 8) // 8
        ),
...
class ICMPv6NDOptIPAddr(_ICMPv6NDGuessPayload, Packet):  # RFC 4068
    ...
    fields_desc = [
        ...
        ByteEnumField(
            "optcode",
            1,
            {1: "Old Care-Of Address", 2: "New Care-Of Address", 3: "NAR's IP address"},
        ),
        ByteField("plen", 64),
        IntField("res", 0),
        IP6Field("addr", "::"),
...
class ICMPv6NDOptRouteInfo(_ICMPv6NDGuessPayload, Packet):  # RFC 4191
    ...
    fields_desc = [
        ByteField("type", 24),
        FieldLenField(
            "len", None, length_of="prefix", fmt="B", adjust=lambda pkt, x: x // 8 + 1
        ),
        ByteField("plen", None),
        BitField("res1", 0, 3),
...
class ICMPv6NDOptRDNSS(_ICMPv6NDGuessPayload, Packet):  # RFC 5006
    ...
    fields_desc = [
        ByteField("type", 25),
        FieldLenField(
            "len", None, count_of="dns", fmt="B", adjust=lambda pkt, x: 2 * x + 1
        ),
        ShortField("res", None),
...
class DomainNameListField(StrLenField):
    ...
    def i2m(self, pkt, x):
        ...
        tmp = (
            [chb(len(z)) + z.encode("utf8") for z in y.split(".")] for y in x
        )  # Also encode string to bytes  # noqa: E501
...
class ICMPv6NDOptDNSSL(_ICMPv6NDGuessPayload, Packet):  # RFC 6106
    ...
    fields_desc = [
        ...
        IntField("lifetime", 0xFFFFFFFF),
        DomainNameListField(
            "searchlist", [], length_from=lambda pkt: 8 * pkt.len - 8, padded=True
        ),
    ]
...
class ICMPv6ND_RS(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 133, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        IntField("res", 0),
    ]
...
class ICMPv6ND_NS(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [
        ByteEnumField("type", 135, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        IntField("res", 0),
        IP6Field("tgt", "::"),
...
class ICMPv6ND_NA(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [
        ByteEnumField("type", 136, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        BitField("R", 1, 1),
        BitField("S", 0, 1),
...
class ICMPv6ND_Redirect(_ICMPv6NDGuessPayload, _ICMPv6, Packet):
    ...
    fields_desc = [
        ByteEnumField("type", 137, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XIntField("res", 0),
        IP6Field("tgt", "::"),
...
class ICMPv6NDOptSrcAddrList(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [
        ByteField("type", 9),
        FieldLenField(
            "len", None, count_of="addrlist", fmt="B", adjust=lambda pkt, x: 2 * x + 1
        ),
        StrFixedLenField("res", b"\x00" * 6, 6),
...
class ICMPv6ND_INDSol(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 141, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
...
class ICMPv6ND_INDAdv(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 142, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XIntField("reserved", 0),
    ]
...
class NIReplyDataField(StrField):
    ...
    def h2i(self, pkt, x):
        ...
        if qtype == 2:  # DNS name
            ...
            if isinstance(x, list):
                x = [
                    val.encode() if isinstance(val, str) else val for val in x
                ]  # noqa: E501
    ...
    def addfield(self, pkt, s, val):
        ...
        if t == 2:
            ...
            return s + b"".join(
                map(
                    lambda x_y1: struct.pack("!I", x_y1[0])
                    + inet_pton(socket.AF_INET6, x_y1[1]),
                    tmp,
                )
            )  # noqa: E501
        elif t == 4:
            return s + b"".join(
                map(
                    lambda x_y2: struct.pack("!I", x_y2[0])
                    + inet_pton(socket.AF_INET, x_y2[1]),
                    tmp,
                )
            )  # noqa: E501
        else:
    ...
    def i2repr(self, pkt, x):
        ...
        if isinstance(x, tuple) and len(x) == 2:
            ...
            if t == 2:  # DNS names
                ...
                return "[ %s ]" % (
                    ", ".join(map(lambda x_y: "(%d, %s)" % (x_y[0], x_y[1]), val))
                )  # noqa: E501
            return repr(val)
...
class ICMPv6RPL(_ICMPv6):  # RFC 6550
    name = "RPL"
    fields_desc = [
        ByteEnumField("type", 155, icmp6types),
...
class ICMPv6HAADRequest(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 144, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XShortField("id", None),
...
class ICMPv6HAADReply(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 145, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XShortField("id", None),
        BitEnumField("R", 1, 1, {1: "MR"}),
...
class ICMPv6MPSol(_ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 146, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XShortField("id", None),
        XShortField("res", 0),
...
class ICMPv6MPAdv(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [
        ByteEnumField("type", 147, icmp6types),
        ByteField("code", 0),
        XShortField("cksum", None),
        XShortField("id", None),
...
class MIP6OptBRAdvice(_MIP6OptAlign):
    ...
    fields_desc = [
        ByteEnumField("otype", 2, _mobopttypes),
        ByteField("olen", 2),
        ShortField("rinter", 0),
    ]
    x = 2
...
class MIP6OptNonceIndices(_MIP6OptAlign):
    ...
    fields_desc = [
        ByteEnumField("otype", 4, _mobopttypes),
        ByteField("olen", 16),
        ShortField("hni", 0),
        ShortField("coni", 0),
    ]
...
class MIP6OptBindingAuthData(_MIP6OptAlign):
    ...
    fields_desc = [
        ByteEnumField("otype", 5, _mobopttypes),
        ByteField("olen", 16),
...
class MIP6OptMobNetPrefix(_MIP6OptAlign):  # NEMO - RFC 3963
    ...
    fields_desc = [
        ByteEnumField("otype", 6, _mobopttypes),
        ByteField("olen", 18),
        ByteField("reserved", 0),
        ByteField("plen", 64),
        IP6Field("prefix", "::"),
...
class MIP6OptLLAddr(_MIP6OptAlign):  # Sect 6.4.4 of RFC 4068
    ...
    fields_desc = [
        ByteEnumField("otype", 7, _mobopttypes),
        ByteField("olen", 7),
...
class MIP6OptMNID(_MIP6OptAlign):  # RFC 4283
    ...
    fields_desc = [
        ...
        FieldLenField(
            "olen", None, length_of="id", fmt="B", adjust=lambda pkt, x: x + 1
        ),
...
class MIP6OptMsgAuth(_MIP6OptAlign):  # RFC 4285 (Sect. 5)
    ...
    fields_desc = [
        ...
        FieldLenField(
            "olen", None, length_of="authdata", fmt="B", adjust=lambda pkt, x: x + 5
        ),
        ByteEnumField(
            "subtype",
            1,
...
class MIP6OptReplayProtection(_MIP6OptAlign):  # RFC 4285 (Sect. 6)
    ...
    fields_desc = [
        ByteEnumField("otype", 10, _mobopttypes),
        ByteField("olen", 8),
...
class MIP6OptCGAParams(_MIP6OptAlign):  # RFC 4866 (Sect. 5.1)
    ...
    fields_desc = [
        ByteEnumField("otype", 12, _mobopttypes),
        FieldLenField("olen", None, length_of="cgaparams", fmt="B"),
        StrLenField("cgaparams", "", length_from=lambda pkt: pkt.olen),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptSignature(_MIP6OptAlign):  # RFC 4866 (Sect. 5.2)
    ...
    fields_desc = [
        ByteEnumField("otype", 13, _mobopttypes),
        FieldLenField("olen", None, length_of="sig", fmt="B"),
        StrLenField("sig", "", length_from=lambda pkt: pkt.olen),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptHomeKeygenToken(_MIP6OptAlign):  # RFC 4866 (Sect. 5.3)
    ...
    fields_desc = [
        ByteEnumField("otype", 14, _mobopttypes),
        FieldLenField("olen", None, length_of="hkt", fmt="B"),
        StrLenField("hkt", "", length_from=lambda pkt: pkt.olen),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptCareOfTest(_MIP6OptAlign):  # RFC 4866 (Sect. 5.5)
    ...
    fields_desc = [
        ByteEnumField("otype", 16, _mobopttypes),
        FieldLenField("olen", None, length_of="cokt", fmt="B"),
        StrLenField("cokt", b"\x00" * 8, length_from=lambda pkt: pkt.olen),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6OptUnknown(_MIP6OptAlign):
    ...
    fields_desc = [
        ...
        StrLenField("odata", "", length_from=lambda pkt: pkt.olen),
    ]
    x = 0
    y = 0  # alignment requirement: none
...
class MIP6MH_BRR(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _OptionsField(
            "options", [], MIP6OptUnknown, 8, length_from=lambda pkt: 8 * pkt.len
        ),
    ]
    overload_fields = {IPv6: {"nh": 135}}
...
class MIP6MH_HoTI(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _OptionsField(
            "options", [], MIP6OptUnknown, 16, length_from=lambda pkt: 8 * (pkt.len - 1)
        ),
    ]
    overload_fields = {IPv6: {"nh": 135}}
...
class MIP6MH_HoT(_MobilityHeader):
    ...
    fields_desc = [
        ...
        _OptionsField(
            "options", [], MIP6OptUnknown, 24, length_from=lambda pkt: 8 * (pkt.len - 2)
        ),
    ]
    overload_fields = {IPv6: {"nh": 135}}
...
class MIP6MH_BU(_MobilityHeader):
    ...
    fields_desc = [
        ...
        ByteField(
            "len", None
        ),  # unit == 8 bytes (excluding the first 8 bytes)  # noqa: E501
        ...
        _OptionsField(
            "options", [], MIP6OptUnknown, 12, length_from=lambda pkt: 8 * pkt.len - 4
        ),
    ]
    overload_fields = {IPv6: {"nh": 135}}
...
class MIP6MH_BA(_MobilityHeader):
    ...
    def answers(self, other):
        if (
            ...
            and self.mhtype == 6
            and other.flags & 0x1
            and self.seq == other.seq  # Ack request flags is set
        ):
            return 1
        return 0
...
class TracerouteResult6(TracerouteResult):
    ...
    def show(self):
        return self.make_table(
            lambda s, r: (
                ...
                ),  # TODO: ICMPv6 !  # noqa: E501
                s.hlim,
                r.sprintf(
                    "%-42s,IPv6.src% {TCP:%TCP.flags%}"
    ...
    def get_trace(self):
        ...
        for s, r in self.res:
            ...
            t = not (
                ICMPv6TimeExceeded in r
                or ICMPv6DestUnreach in r
                or ICMPv6PacketTooBig in r
                or ICMPv6ParamProblem in r
            )
...
def traceroute6(
    ...
    **kargs
    ...
    if l4 is None:
        a, b = sr(
            IPv6(dst=target, hlim=(minttl, maxttl))
            / TCP(seq=RandInt(), sport=sport, dport=dport),  # noqa: E501
            timeout=timeout,
            filter="icmp6 or tcp",
            verbose=verbose,
            **kargs
        ...
        a, b = sr(
            IPv6(dst=target, hlim=(minttl, maxttl)) / l4,
            timeout=timeout,
            verbose=verbose,
            **kargs
        )
...
class L3RawSocket6(L3RawSocket):
    def __init__(
        self, type=ETH_P_IPV6, filter=None, iface=None, promisc=None, nofilter=0
    ):  # noqa: E501
...
class _IPv6inIP(SuperSocket):
    ...
    def send(self, x):
        return self.worker.send(
            IP(dst=self.dst, src=self.src, proto=socket.IPPROTO_IPV6) / x
        )  # noqa: E501
...
def _NDP_Attack_DAD_DoS(
    reply_callback, iface=None, mac_src_filter=None, tgt_filter=None, reply_mac=None
):
    """
...
def NDP_Attack_DAD_DoS_via_NS(
    iface=None, mac_src_filter=None, tgt_filter=None, reply_mac=None
):
    """
    ...
    def ns_reply_callback(req, reply_mac, iface):
        ...
        rep = (
            Ether(src=reply_mac) / IPv6(src="::", dst=dst) / ICMPv6ND_NS(tgt=tgt)
        )  # noqa: E501
...
def NDP_Attack_NA_Spoofing(
    iface=None, mac_src_filter=None, tgt_filter=None, reply_mac=None, router=False
):
    """
...
def NDP_Attack_NS_Spoofing(
    ...
    if dst:
        ...
        tmp = inet_ntop(
            socket.AF_INET6, in6_getnsma(inet_pton(socket.AF_INET6, target))
        )
        ipv6_params["dst"] = tmp
...
def NDP_Attack_Kill_Default_Router(
    iface=None, mac_src_filter=None, ip_src_filter=None, reply_mac=None, tgt_mac=None
):
    """
