...
class TimeStampField(FixedPointField):
    ...
    def i2repr(self, pkt, val):
        ...
        return time.strftime(
            "%a, %d %b %Y %H:%M:%S +0000", time.gmtime(int(val - _NTP_BASETIME))
        )
...
class NTPHeader(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrFixedLenEnumField("ref_id", "", length=4, enum=_reference_identifiers),
...
class _NTPInvalidDataException(Exception):
    ...
    def __init__(self, details):
        Exception.__init__(
            self, "Data does not seem to be a valid NTP message" + details
        )
...
class NTPControl(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(
            NTPControlStatusField("status_word", "", Packet), lambda p: p.response == 1
        ),
...
class NTPPrivateRespPacketListField(PacketListField):
    ...
    def m2i(self, pkt, s):
        ...
        if pkt.request_code == 44 or pkt.request_code == 45:
            ...
            ret = _private_data_objects.get(pkt.request_code, conf.raw_layer)(
                s
            )  # noqa: E501
    ...
    def getfield(self, pkt, s):
        ...
        if length > 0:
            ...
            while (
                len(remain) >= pkt.data_item_size and item_counter < pkt.nb_items
            ):  # noqa: E501
...
class NTPPrivateReqPacketListField(PacketListField):
    ...
    def getfield(self, pkt, s):
        ...
        if length > 0:
            item_counter = 0
            while (
                len(remain) >= pkt.data_item_size * pkt.nb_items
...
class NTPPrivatePktTail(Packet):
    ...
    fields_desc = [
        ...
        IntField("key_id", 0),
        XStrFixedLenField("dgst", "", length_from=lambda x: _NTP_AUTH_MD5_DGST_SIZE),
    ]
...
class NTPPrivate(NTP):
    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketField("authenticator", "", NTPPrivatePktTail),
