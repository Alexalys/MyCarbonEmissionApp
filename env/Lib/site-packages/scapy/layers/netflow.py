    ...
    def post_build(self, pkt, pay):
        if self.count is None:
            count = sum(
                ...
                if x
                in [NetflowFlowsetV9, NetflowDataflowsetV9, NetflowOptionsFlowsetV9]
            )
            pkt = struct.pack("!H", count) + pkt[2:]
        return pkt + pay
...
class NetflowTemplateFieldV9(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField("fieldType", None, 15, NetflowV910TemplateFieldTypes),
        ShortField("fieldLength", None),
        ConditionalField(IntField("enterpriseNumber", 0), lambda p: p.enterpriseBit),
    ]

    def __init__(self, *args, **kwargs):
        Packet.__init__(self, *args, **kwargs)
        if (
            self.fieldType is not None
            and self.fieldLength is None
            and self.fieldType in NetflowV9TemplateFieldDefaultLengths
        ):
...
class NetflowFlowsetV9(Packet):
    ...
    fields_desc = [
        ShortField("flowSetID", 0),
        FieldLenField(
            "length", None, length_of="templates", adjust=lambda pkt, x: x + 4
        ),
        PacketListField(
            "templates", [], NetflowTemplateV9, length_from=lambda pkt: pkt.length - 4
        ),
    ]
        ...
        if _f_type:
            ...
            _fields_desc.append(
                _CustomStrFixedLenField(
                    NetflowV910TemplateFieldTypes.get(k, "unknown_data"), b"", length=j
                )
            )
...
class NetflowDataflowsetV9(Packet):
    ...
    fields_desc = [
        ...
        ShortField("length", None),
        PacketListField(
            "records", [], NetflowRecordV9, length_from=lambda pkt: pkt.length - 4
        ),
    ]
...
class NetflowSession(IPSession):
    ...
    def _process_packet(self, pkt):
        _netflowv9_defragment_packet(
            pkt, self.definitions, self.definitions_opts, self.ignored
        )
        return pkt
...
class NetflowOptionsFlowsetScopeV9(Packet):
    ...
    fields_desc = [
        ShortEnumField("scopeFieldType", None, ScopeFieldTypes),
    ...
    fields_desc = [
        ...
        StrLenField(
            ...
            length_from=lambda pkt: (
                pkt.length - pkt.option_scope_length - pkt.option_field_length - 10
            ),
        ),
    ]
...
class NetflowOptionsFlowset10(NetflowOptionsFlowsetV9):
    ...
    fields_desc = [
        ...
        FieldLenField(
            ...
            count_of="options",
            adjust=lambda pkt, x: (
                x + pkt.get_field("scope_field_count").i2m(pkt, None)
            ),
        ),
        ...
        PacketListField(
            ...
            [],
            NetflowOptionsFlowsetOptionV9,
            count_from=lambda pkt: (pkt.field_count - pkt.scope_field_count),
        ),
        StrLenField(
            "pad",
            None,
            length_from=lambda pkt: (pkt.length - (pkt.scope_field_count * 4) - 10),
        ),
    ]
