...
class dot15d4AddressField(Field):
    ...
    def i2repr(self, pkt, x):
        ...
        if len(hex(self.i2m(pkt, x))) < 7:  # short address
            ...
            return ":".join(
                ["%s%s" % (x[i], x[i + 1]) for i in range(0, len(x), 2)]
            )  # noqa: E501
    ...
    def getfield(self, pkt, s):
        if self.adjust(pkt, self.length_of) == 2:
            return s[2:], self.m2i(
                pkt, struct.unpack(self.fmt[0] + "H", s[:2])[0]
            )  # noqa: E501
        elif self.adjust(pkt, self.length_of) == 8:
            return s[8:], self.m2i(
                pkt, struct.unpack(self.fmt[0] + "Q", s[:8])[0]
            )  # noqa: E501
        else:
...
class Dot15d4(Packet):
    ...
    fields_desc = [
        ...
        Emph(
            BitEnumField(
                "fcf_frametype", 0, 3, {0: "Beacon", 1: "Data", 2: "Ack", 3: "Command"}
            )
        ),  # noqa: E501
        BitEnumField(
            "fcf_srcaddrmode", 0, 2, {0: "None", 1: "Reserved", 2: "Short", 3: "Long"}
        ),  # fcf p2 b1  # noqa: E501
        BitField(
            "fcf_framever", 0, 2
        ),  # 00 compatibility with 2003 version; 01 compatible with 2006 version  # noqa: E501
        BitEnumField(
            "fcf_destaddrmode", 2, 2, {0: "None", 1: "Reserved", 2: "Short", 3: "Long"}
        ),  # fcf p2 b2  # noqa: E501
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 %Dot15d4.fcf_frametype% ackreq(%Dot15d4.fcf_ackreq%) ( %Dot15d4.fcf_destaddrmode% -> %Dot15d4.fcf_srcaddrmode% ) Seq#%Dot15d4.seqnum%"
        )  # noqa: E501
        ...
        if isinstance(other, Dot15d4):
            if self.fcf_frametype == 2:  # ack
                if self.seqnum != other.seqnum:  # check for seqnum matching
                    ...
                    other.fcf_ackreq == 1
                ):  # check that an ack was indeed requested  # noqa: E501
                    return 1
        return 0
...
class Dot15d4AuxSecurityHeader(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XByteField("sec_keyid_keyindex", 0xFF),
...
class Dot15d4Data(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XLEShortField("src_panid", 0x0), lambda pkt: util_srcpanid_present(pkt)
        ),
        ConditionalField(
            dot15d4AddressField(
        ...
        ConditionalField(
            PacketField(
                "aux_sec_header", Dot15d4AuxSecurityHeader(), Dot15d4AuxSecurityHeader
            ),  # noqa: E501
    ...
    def guess_payload_class(self, payload):
        ...
        if conf.dot15d4_protocol == "sixlowpan":
            ...
            if conf.dot15d4_protocol is None:
                _msg = (
                    "Please set conf.dot15d4_protocol to select a "
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 Data ( %Dot15d4Data.src_panid%:%Dot15d4Data.src_addr% -> %Dot15d4Data.dest_panid%:%Dot15d4Data.dest_addr% )"
        )  # noqa: E501
...
class Dot15d4Beacon(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField(
            "sf_battlifeextend", 0, 1, [False, True]
        ),  # not used by ZigBee  # noqa: E501
        BitField("sf_finalcapslot", 15, 4),  # not used by ZigBee
        # GTS Fields
        #  GTS Specification (1 byte)
        BitEnumField(
            "gts_spec_permit", 1, 1, [False, True]
        ),  # GTS spec bit 7, true=1 iff PAN cord is accepting GTS requests  # noqa: E501
        ...
        ConditionalField(
            BitField("gts_dir_reserved", 0, 1),
            lambda pkt: pkt.getfieldval("gts_spec_desccount") != 0,
        ),  # noqa: E501
        ConditionalField(
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 Beacon ( %Dot15d4Beacon.src_panid%:%Dot15d4Beacon.src_addr% ) assocPermit(%Dot15d4Beacon.sf_assocpermit%) panCoord(%Dot15d4Beacon.sf_pancoord%)"
        )  # noqa: E501
...
class Dot15d4Cmd(Packet):
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 Command %Dot15d4Cmd.cmd_id% ( %Dot15dCmd.src_panid%:%Dot15d4Cmd.src_addr% -> %Dot15d4Cmd.dest_panid%:%Dot15d4Cmd.dest_addr% )"
        )  # noqa: E501
...
class Dot15d4CmdCoordRealign(Packet):
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 Coordinator Realign Payload ( PAN ID: %Dot15dCmdCoordRealign.pan_id% : channel %Dot15d4CmdCoordRealign.channel% )"
        )  # noqa: E501
...
def util_srcpanid_present(pkt):
    ...
    if (pkt.underlayer.getfieldval("fcf_srcaddrmode") != 0) and (
        pkt.underlayer.getfieldval("fcf_panidcompress") == 0
    ):  # noqa: E501
        return True
    else:
        return False
...
class Dot15d4CmdAssocReq(Packet):
    ...
    fields_desc = [
        ...
        BitField(
            "alternate_pan_coordinator", 0, 1
        ),  # Alternate PAN Coordinator  # noqa: E501
    ]

    def mysummary(self):
        return self.sprintf(
            "802.15.4 Association Request Payload ( Alt PAN Coord: %Dot15d4CmdAssocReq.alternate_pan_coordinator% Device Type: %Dot15d4CmdAssocReq.device_type% )"
        )  # noqa: E501
...
class Dot15d4CmdAssocResp(Packet):
    ...
    fields_desc = [
        ...
        ),  # Address assigned to device from coordinator (0xFFFF == none)  # noqa: E501
        # Association Status
        # 0x00 == successful
        ...
        ByteEnumField(
            "association_status",
            0x00,
            {0: "successful", 1: "PAN_at_capacity", 2: "PAN_access_denied"},
        ),  # noqa: E501
    ]

    def mysummary(self):
        return self.sprintf(
            "802.15.4 Association Response Payload ( Association Status: %Dot15d4CmdAssocResp.association_status% Assigned Address: %Dot15d4CmdAssocResp.short_address% )"
        )  # noqa: E501
...
class Dot15d4CmdDisassociation(Packet):
    ...
    fields_desc = [
        ...
        ByteEnumField(
            ...
            0x02,
            {1: "coord_wishes_device_to_leave", 2: "device_wishes_to_leave"},
        ),  # noqa: E501
    ]

    def mysummary(self):
        return self.sprintf(
            "802.15.4 Disassociation Notification Payload ( Disassociation Reason %Dot15d4CmdDisassociation.disassociation_reason% )"
        )  # noqa: E501
...
class Dot15d4CmdGTSReq(Packet):
    ...
    def mysummary(self):
        return self.sprintf(
            "802.15.4 GTS Request Command ( %Dot15d4CmdGTSReq.gts_len% : %Dot15d4CmdGTSReq.gts_dir% )"
        )  # noqa: E501
