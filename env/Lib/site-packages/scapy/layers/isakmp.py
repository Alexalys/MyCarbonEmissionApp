        ...
        while len(m) >= 4:
            ...
            if is_tlv:
                ...
                if value_len + 4 > len(m):
                    warning(
                        "Bad length for ISAKMP transform type=%#6x" % trans_type
                    )  # noqa: E501
                value = m[4 : 4 + value_len]
                value = reduce(
                    lambda x, y: (x << 8) | y,
                    struct.unpack("!%s" % ("B" * len(value),), value),
                    0,
                )  # noqa: E501
            else:
                trans_type &= 0x7FFF
...
class ISAKMP(ISAKMP_class):  # rfc2408
    ...
    fields_desc = [
        ...
        ),  # XXX use a Flag field  # noqa: E501
        IntField("id", 0),
...
class ISAKMP_payload_Proposal(ISAKMP_class):
    ...
    fields_desc = [
        ...
        ByteField("res", 0),
        FieldLenField(
            "length", None, "trans", "H", adjust=lambda pkt, x: x + 8
        ),  # noqa: E501
...
class ISAKMP_payload_SA(ISAKMP_class):
    ...
    fields_desc = [
        ...
        ByteField("res", 0),
        FieldLenField(
            "length", None, "prop", "H", adjust=lambda pkt, x: x + 12
        ),  # noqa: E501
