...
class LLTD(Packet):
    ...
    def post_build(self, pkt, pay):
        if (self.real_dst is None or self.real_src is None) and isinstance(
    ...
    def answers(self, other):
        ...
        if self.tos == 0:
            if (
                self.function == 0
                and isinstance(self.payload, LLTDDiscover)
                ...
                return (
                    other.tos == 1
                    and other.function == 1
                    and LLTDAttributeHostID in other
                    and other[LLTDAttributeHostID].mac
            ...
            if self.function == 1 and isinstance(self.payload, LLTDHello):
                ...
                return (
                    other.tos == 0
                    and other.function == 0
                    and other.real_src == self.current_mapper_address
                )
        return False
...
class LLTDDiscover(Packet):
    ...
    fields_desc = [
        ShortField("gen_number", 0),
        FieldLenField("stations_count", None, count_of="stations_list", fmt="H"),
        FieldListField(
            "stations_list",
            [],
            MACField("", ETHER_ANY),
    ...
    def mysummary(self):
        return (
            self.sprintf("Stations: %stations_list%")
...
class LLTDEmit(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "descs_list", [], LLTDEmiteeDesc, count_from=lambda pkt: pkt.descs_count
        ),
    ]

    def mysummary(self):
        return ", ".join(desc.sprintf("%src% > %dst%") for desc in self.descs_list), [
            LLTD
        ]
...
class LLTDQueryResp(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "descs_list", [], LLTDRecveeDesc, count_from=lambda pkt: pkt.descs_count
        ),
    ]
    ...
    def mysummary(self):
        return self.sprintf(
            "%d response%s" % (self.descs_count, "s" if self.descs_count > 1 else "")
        ), [LLTD]
...
class LLTDQueryLargeTlvResp(Packet):
    ...
    def mysummary(self):
        return self.sprintf(
            "%%len%% bytes%s" % (" (last)" if not self.flags & 2 else "")
        ), [LLTD]
...
class LLTDAttributeCharacteristics(LLTDAttribute):
    ...
    fields_desc = [
        ...
        # ByteField("len", 2),
        FieldLenField(
            "len", None, length_of="reserved2", fmt="B", adjust=lambda _, x: x + 2
        ),
...
class LLTDAttributeMachineName(LLTDAttribute):
    ...
    def mysummary(self):
        return (
            self.sprintf("Hostname: %r" % self.hostname),
...
class LargeTlvBuilder(object):
    ...
    def parse(self, plist):
        ...
        for pkt in plist[LLTD]:
            if LLTDQueryLargeTlv in pkt:
                ...
                self.types_offsets[key] = (
                    pkt[LLTDQueryLargeTlv].type,
    ...
    def get_data(self):
        ...
        """
        return {
            key: "".join(chr(byte) for byte in data)
