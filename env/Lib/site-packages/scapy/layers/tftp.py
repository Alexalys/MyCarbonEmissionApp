...
class TFTP_Options(Packet):
    fields_desc = [
        PacketListField("options", [], TFTP_Option, length_from=lambda x: None)
    ]  # noqa: E501
...
class TFTP_read(Automaton):
    ...
    def BEGIN(self):
        ...
        self.l3 = (
            IP(dst=self.server) / UDP(sport=self.my_tid, dport=self.port) / TFTP()
        )  # noqa: E501
        self.last_packet = self.l3 / TFTP_RRQ(
            filename=self.filename, mode="octet"
        )  # noqa: E501
...
class TFTP_write(Automaton):
    ...
    def BEGIN(self):
        ...
        self.last_packet = self.l3 / TFTP_WRQ(
            filename=self.filename, mode="octet"
        )  # noqa: E501
        self.send(self.last_packet)
        self.res = ""
    ...
    def SEND_DATA(self):
        self.awaiting += 1
        self.last_packet = (
            self.l3 / TFTP_DATA(block=self.awaiting) / self.data.pop(0)
        )  # noqa: E501
        self.send(self.last_packet)
        if self.data:
            raise self.WAITING_ACK()
...
class TFTP_WRQ_server(Automaton):
    ...
    def ack_WRQ(self, pkt):
        ...
        self.l3 = (
            ...
            / UDP(sport=self.my_tid, dport=pkt.sport)
            / TFTP()
        )  # noqa: E501
        if options is None:
            ...
            self.last_packet = (
                self.l3 / TFTP_OACK() / TFTP_Options(options=opt)
            )  # noqa: E501
            self.send(self.last_packet)
...
class TFTP_RRQ_server(Automaton):
    ...
    def RECEIVED_RRQ(self, pkt):
        ...
        self.l3 = (
            IP(src=ip.dst, dst=ip.src) / UDP(sport=self.my_tid, dport=ip.sport) / TFTP()
        )  # noqa: E501
        ...
        if self.filename in self.store:
            ...
            if fn.startswith(
                self.dir
            ):  # Check we're still in the server's directory  # noqa: E501
                try:
                    with open(fn) as fd:
                        self.data = fd.read()
                except IOError:
    ...
    def send_error(self):
        self.send(
            self.l3 / TFTP_ERROR(errorcode=1, errormsg=TFTP_Error_Codes[1])
        )  # noqa: E501
