...
class IPTools(object):
    ...
    def _ttl(self):
        ...
        return (
            self.hlim if isinstance(self, scapy.layers.inet6.IPv6) else self.ttl
        )  # noqa: E501
...
class IPOption_RR(IPOption):
    ...
    fields_desc = [
        _IPOption_HDR,
        FieldLenField(
            "length", None, fmt="B", length_of="routers", adjust=lambda pkt, l: l + 3
        ),  # noqa: E501
...
class IPOption_Timestamp(IPOption):
    ...
    fields_desc = [
        ...
        ConditionalField(
            IPField("internet_address", "0.0.0.0"), lambda pkt: pkt.flg != 0
        ),
...
class IPOption_SDBM(IPOption):
    ...
    fields_desc = [
        _IPOption_HDR,
        FieldLenField(
            "length", None, fmt="B", length_of="addresses", adjust=lambda pkt, l: l + 2
        ),  # noqa: E501
        FieldListField(
            "addresses",
            [],
    ...
    def _fix(self):
        ...
        rand_patterns = [
            random.choice(
                list(
                    (opt, fmt)
                    for opt, fmt in six.itervalues(TCPOptions[0])
                    if opt != "EOL"
                )
            )
    ...
    def any2i(self, pkt, val):
        if isinstance(val, str):
            ...
            if hmsms:
                ...
                ms = int(((ms or "") + "000")[:3])
                val = (
                    (int(h) * 60 + int(m or 0)) * 60 + int(s or 0)
                ) * 1000 + ms  # noqa: E501
            else:
                val = 0
        elif val is None:
...
class IP(Packet, IPTools):
    ...
    def hashret(self):
        ...
        if conf.checkIPsrc and conf.checkIPaddr:
            return (
                strxor(
                    inet_pton(socket.AF_INET, self.src),
    ...
    def answers(self, other):
        ...
        if conf.checkIPaddr:
            if (
                other.dst == "224.0.0.251" and self.dst == "224.0.0.251"
            ):  # mDNS  # noqa: E501
...
def in4_chksum(proto, u, p):
    ...
    sr_options = [
        ...
        if isinstance(opt, IPOption_LSRR) or isinstance(opt, IPOption_SSRR)
    ]
...
class TCP(Packet):
    ...
    def hashret(self):
        if conf.checkIPsrc:
            return (
                struct.pack("H", self.sport ^ self.dport) + self.payload.hashret()
            )  # noqa: E501
        else:
            return self.payload.hashret()
    ...
    def mysummary(self):
        ...
        elif conf.ipv6_enabled and isinstance(
            self.underlayer, scapy.layers.inet6.IPv6
        ):  # noqa: E501
            return self.underlayer.sprintf(
...
class UDP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            ...
            elif isinstance(self.underlayer, scapy.layers.inet6.IPv6) or isinstance(
...
class ICMP(Packet):
    ...
    fields_desc = [
        ...
        MultiEnumField(
            "code", 0, icmpcodes, depends_on=lambda pkt: pkt.type, fmt="B"
        ),  # noqa: E501
        XShortField("chksum", None),
        ConditionalField(
            XShortField("id", 0), lambda pkt: pkt.type in [0, 8, 13, 14, 15, 16, 17, 18]
        ),  # noqa: E501
        ConditionalField(
            XShortField("seq", 0),
            lambda pkt: pkt.type in [0, 8, 13, 14, 15, 16, 17, 18],
        ),  # noqa: E501
        ConditionalField(
            ICMPTimeStampField("ts_ori", None), lambda pkt: pkt.type in [13, 14]
        ),  # noqa: E501
        ConditionalField(
            ICMPTimeStampField("ts_rx", None), lambda pkt: pkt.type in [13, 14]
        ),  # noqa: E501
        ConditionalField(
            ICMPTimeStampField("ts_tx", None), lambda pkt: pkt.type in [13, 14]
        ),  # noqa: E501
        ConditionalField(
            IPField("gw", "0.0.0.0"), lambda pkt: pkt.type == 5
        ),  # noqa: E501
        ConditionalField(ByteField("ptr", 0), lambda pkt: pkt.type == 12),  # noqa: E501
        ConditionalField(
            ByteField("reserved", 0), lambda pkt: pkt.type in [3, 11]
        ),  # noqa: E501
        ConditionalField(
            ByteField("length", 0), lambda pkt: pkt.type in [3, 11, 12]
        ),  # noqa: E501
        ConditionalField(
            IPField("addr_mask", "0.0.0.0"), lambda pkt: pkt.type in [17, 18]
        ),  # noqa: E501
        ConditionalField(
            ShortField("nexthopmtu", 0), lambda pkt: pkt.type == 3
        ),  # noqa: E501
        MultipleTypeField(
            [
                (ShortField("unused", 0), lambda pkt: pkt.type in [11, 12]),
                (
                    IntField("unused", 0),
                    lambda pkt: pkt.type
                    not in [0, 3, 5, 8, 11, 12, 13, 14, 15, 16, 17, 18],
                ),
            ],
    ...
    def answers(self, other):
        ...
        if (
            ...
            in [(8, 0), (13, 14), (15, 16), (17, 18), (33, 34), (35, 36), (37, 38)]
            and self.id == other.id  # noqa: E501
...
class TracerouteResult(SndRcvList):
    ...
    def show(self):
        return self.make_table(
            lambda s, r: (
                s.sprintf(
                    "%IP.dst%:{TCP:tcp%ir,TCP.dport%}{UDP:udp%ir,UDP.dport%}{ICMP:ICMP}"
                ),  # noqa: E501
                s.ttl,
    ...
    def trace3D_notebook(self):
        ...
        class IPsphere(vpython.sphere):
            ...
            def fullinfos(self):
                ...
                a, b = sr(
                    IP(dst=self.ip) / TCP(dport=[21, 22, 23, 25, 80, 443], flags="S"),
                    timeout=2,
                    verbose=0,
                )  # noqa: E501
                if len(a) == 0:
        ...
        vpython.scene.title = (
            "<center><u><b>%s</b></u></center>" % self.listname
        )  # noqa: E501
...
Touch screen: pinch/extend to zoom, swipe or two-finger rotate.""",
        ...
        for t in rings:
            ...
            for i in range(tmp_len):
                ...
                s = IPsphere(
                    pos=vpython.vec(
                        (tmp_len - 1) * vpython.cos(2 * i * vpython.pi / tmp_len),
                        (tmp_len - 1) * vpython.sin(2 * i * vpython.pi / tmp_len),
                        2 * t,
                    ),  # noqa: E501
                    ip=r[i][0],
                    color=col,
        ...
        for trlst in six.itervalues(tr3d):
            ...
            for ip in trlst:
                vpython.cylinder(
                    pos=start, axis=ip.pos - start, color=col, radius=0.2
                )  # noqa: E501
                start = ip.pos
    ...
    def world_trace(self):
        ...
        for trace_id in rt:
            ...
            for i in range(max(trace)):
                ...
                try:
                    ...
                    continue
                loctrace.append(
                    (sresult.location.longitude, sresult.location.latitude)
                )  # noqa: E501
            if loctrace:
        ...
        for key, trc in six.iteritems(trt):
            ...
            line_col = LineCollection(
                data_lines, linewidths=2, label=key[1], color=color
            )
    ...
    def make_graph(self, ASres=None, padding=0):
        ...
        for s, r in self.res:
            r = (
                r.getlayer(IP)
                or (conf.ipv6_enabled and r[scapy.layers.inet6.IPv6])
                or r
            )  # noqa: E501
            s = (
                s.getlayer(IP)
                or (conf.ipv6_enabled and s[scapy.layers.inet6.IPv6])
                or s
            )  # noqa: E501
            ips[r.src] = None
            if TCP in s:
            ...
            trace = rt.get(trace_id, {})
            ttl = (
                conf.ipv6_enabled and scapy.layers.inet6.IPv6 in s and s.hlim or s.ttl
            )  # noqa: E501
            if not (ICMP in r and r[ICMP].type == 11) and not (
                conf.ipv6_enabled
                and scapy.layers.inet6.IPv6 in r
                and scapy.layers.inet6.ICMPv6TimeExceeded in r
            ):  # noqa: E501
                if trace_id in ports_done:
                    continue
                ports_done[trace_id] = None
                ...
                if TCP in r:
                    ...
                    p.append(
                        r.sprintf(
                            "{IP:<P%ir,proto%> IP %proto%}{IPv6:<P%ir,nh%> IPv6 %nh%}"
                        )
                    )  # noqa: E501
                    trace[ttl] = r.sprintf(
        ...
        for p in ports:
            s += (
                '\t"%s" [shape=record,color=black,fillcolor=green,style=filled,label="%s|%s"];\n'
                % (p, p, "|".join(ports[p]))
            )  # noqa: E501
        ...
        for bh in blackholes:
            s += (
                "\t%s [shape=octagon,color=black,fillcolor=red,style=filled];\n" % bh
            )  # noqa: E501
        ...
        if padding:
            ...
            for rcv in pad:
                s += (
                    '\t"%s" [shape=triangle,color=black,fillcolor=red,style=filled];\n'
                    % rcv
                )  # noqa: E501
    ...
    def graph(self, ASres=conf.AS_resolver, padding=0, **kargs):
        ...
        if (
            self.graphdef is None
            or self.graphASres != ASres
            or self.graphpadding != padding
        ):
...
@conf.commands.register
def IPID_count(
    lst, funcID=lambda x: x[1].id, funcpres=lambda x: x[1].summary()
):  # noqa: E501
    ...
    classes = [idlst[0]]
    classes += [
        t[1] for t in zip(idlst[:-1], idlst[1:]) if abs(t[0] - t[1]) > 50
    ]  # noqa: E501
