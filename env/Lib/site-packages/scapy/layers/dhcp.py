...
DHCPOptions = {
    ...
    55: _DHCPParamReqFieldListField(
        "param_req_list", [], ByteField("opcode", 0), length_from=lambda x: 1
    ),  # noqa: E501
    56: "error_message",
    ...
    def i2repr(self, pkt, x):
        ...
        for v in x:
            if isinstance(v, tuple) and len(v) >= 2:
                if v[0] in DHCPRevOptions and isinstance(
                    DHCPRevOptions[v[0]][1], Field
                ):  # noqa: E501
        ...
        for o in x:
            if isinstance(o, tuple) and len(o) >= 2:
                ...
                if isinstance(name, int):
                    ...
                    if f is not None:
                        lval = (
                            f.addfield(pkt, b"", f.any2i(pkt, val)) for val in lval
                        )  # noqa: E501
                    else:
...
class BOOTP_am(AnsweringMachine):
    ...
    def make_reply(self, req):
        ...
        del repb.payload
        rep = (
            Ether(dst=mac) / IP(dst=ip) / UDP(sport=req.dport, dport=req.sport) / repb
        )  # noqa: E501
        return rep
