...
class RadioTapExtendedPresenceMask(Packet):
    ...
    fields_desc = [
        FlagsField("present", None, -32, ["b%s" % i for i in range(0, 31)] + ["Ext"])
    ]
    ...
    def _restart_indentation(self, index):
        st = index * 32
        self.fields_desc[0].names = ["b%s" % (i + st) for i in range(0, 31)] + [
            "Ext"
        ]  # noqa: E501
...
class RadioTapTLV(Packet):
    fields_desc = [
        ...
        ConditionalField(OUIField("oui", 0), lambda pkt: pkt.type == 30),  # VendorNS
...
class RadioTap(Packet):
    ...
    fields_desc = [
        ...
        # TSFT
        ConditionalField(
            ReversePadField(LELongField("mac_timestamp", 0), 8),
        ...
        # Rate
        ConditionalField(
            ScalingField("Rate", 0, scaling=0.5, unit="Mbps", fmt="B"),
            lambda pkt: pkt.present and pkt.present.Rate,
        ),
        # Channel
        ConditionalField(
            ReversePadField(LEShortField("ChannelFrequency", 0), 2),
        ...
        # dBm_AntSignal
        ConditionalField(
            ScalingField("dBm_AntSignal", 0, unit="dBm", fmt="b"),
            lambda pkt: pkt.present and pkt.present.dBm_AntSignal,
        ),
        # dBm_AntNoise
        ConditionalField(
            ScalingField("dBm_AntNoise", 0, unit="dBm", fmt="b"),
            lambda pkt: pkt.present and pkt.present.dBm_AntNoise,
        ),
        # Lock_Quality
        ConditionalField(
            ReversePadField(LEShortField("Lock_Quality", 0), 2),
            lambda pkt: pkt.present and pkt.present.Lock_Quality,
        ),
        # Antenna
        ConditionalField(
            ByteField("Antenna", 0), lambda pkt: pkt.present and pkt.present.Antenna
        ),
        # RX Flags
        ConditionalField(
            ReversePadField(FlagsField("RXFlags", None, -16, _rt_rxflags), 2),
            lambda pkt: pkt.present and pkt.present.RXFlags,
        ),
        # TX Flags
        ConditionalField(
            ReversePadField(FlagsField("TXFlags", None, -16, _rt_txflags), 2),
            lambda pkt: pkt.present and pkt.present.TXFlags,
        ),
        # ChannelPlus
        ConditionalField(
            ReversePadField(
                FlagsField("ChannelPlusFlags", None, -32, _rt_channelflags2), 4
            ),
        ...
        # MCS
        ConditionalField(
            ReversePadField(FlagsField("knownMCS", None, -8, _rt_knownmcs), 4),
            lambda pkt: pkt.present and pkt.present.MCS,
        ),
        ConditionalField(
            BitField("Ness_LSB", 0, 1), lambda pkt: pkt.present and pkt.present.MCS
        ),
        ConditionalField(
            BitField("STBC_streams", 0, 2), lambda pkt: pkt.present and pkt.present.MCS
        ),
        ConditionalField(
        ...
        ConditionalField(
            BitEnumField(
                "guard_interval", 0, 1, {0: "Long_GI", 1: "Short_GI"}
            ),  # noqa: E501
            lambda pkt: pkt.present and pkt.present.MCS,
        ),
        ConditionalField(
        ...
        ConditionalField(
            ByteField("MCS_index", 0), lambda pkt: pkt.present and pkt.present.MCS
        ),
        # A_MPDU
        ConditionalField(
            ReversePadField(LEIntField("A_MPDU_ref", 0), 4),
        ...
        # VHT
        ConditionalField(
            ReversePadField(FlagsField("KnownVHT", None, -16, _rt_knownvht), 2),
        ...
        ConditionalField(
            ByteField("GroupID", 0), lambda pkt: pkt.present and pkt.present.VHT
        ),
        ConditionalField(
            ShortField("PartialAID", 0), lambda pkt: pkt.present and pkt.present.VHT
        ),
        # timestamp
        ConditionalField(
            ReversePadField(LELongField("timestamp", 0), 8),
        ...
        ConditionalField(
            ByteField("ts_flags", 0), lambda pkt: pkt.present and pkt.present.timestamp
        ),
        # HE - XXX not complete
        ConditionalField(
            ReversePadField(ShortField("he_data1", 0), 2),
            lambda pkt: pkt.present and pkt.present.HE,
        ),
        ConditionalField(
            ShortField("he_data2", 0), lambda pkt: pkt.present and pkt.present.HE
        ),
        ConditionalField(
            ShortField("he_data3", 0), lambda pkt: pkt.present and pkt.present.HE
        ),
        ConditionalField(
            ShortField("he_data4", 0), lambda pkt: pkt.present and pkt.present.HE
        ),
        ConditionalField(
            ShortField("he_data5", 0), lambda pkt: pkt.present and pkt.present.HE
        ),
        ConditionalField(
            ShortField("he_data6", 0), lambda pkt: pkt.present and pkt.present.HE
        ),
        # HE_MU
        ConditionalField(
            ReversePadField(LEShortField("hemu_flags1", 0), 2),
        ...
        ConditionalField(
            FieldListField("RU_channel1", [], ByteField, count_from=lambda x: 4),
            lambda pkt: pkt.present and pkt.present.HE_MU,
        ),
        ConditionalField(
            FieldListField("RU_channel2", [], ByteField, count_from=lambda x: 4),
            lambda pkt: pkt.present and pkt.present.HE_MU,
        ),
        # HE_MU_other_user
        ConditionalField(
            ReversePadField(LEShortField("hemuou_per_user_1", 0x7FFF), 2),
        ...
        ConditionalField(
            FlagsField("hemuou_per_user_known", 0, -16, _rt_hemuother_per_user_known),
            lambda pkt: pkt.present and pkt.present.HE_MU_other_user,
        ),
        # L_SIG
        ConditionalField(
            ReversePadField(FlagsField("lsig_data1", 0, -16, ["rate", "length"]), 2),
        ...
        ConditionalField(
            BitField("lsig_rate", 0, 4), lambda pkt: pkt.present and pkt.present.L_SIG
        ),
        # TLV fields
        ConditionalField(
...
_dot11_addr_meaning = [
    ...
    ["BSSID", None, None, None],  # Extension
]
...
class Dot11(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (
                    FlagsField("FCfield", 0, 4, ["pw-mgt", "MD", "protected", "order"]),
                    lambda pkt: (pkt.type, pkt.subtype) == (1, 6),
                )
            ],
            FlagsField(
    ...
    def answers(self, other):
        if isinstance(other, Dot11):
            if self.type == 0:  # management
                if (
                    self.addr1.lower() != other.addr2.lower()
    ...
    def address_meaning(self, index):
        ...
        if self.type == 0:  # Management
            ...
            meaning = _dot11_addr_meaning[2][index][self.FCfield.to_DS][
    ...
    def network_stats(self):
        ...
        while isinstance(p, Dot11Elt):
            ...
            if p.ID == 0 and "ssid" not in summary:
                ...
                if (
                    any(x.suite == 8 for x in p.akm_suites)
                    and all(x.suite not in [2, 6] for x in p.akm_suites)
                    and p.mfp_capable
                    and p.mfp_required
                    and all(x.cipher not in [1, 2, 5] for x in p.pairwise_cipher_suites)
                ):
                    # WPA3 only mode
                    wpa_version = "WPA3"
                elif (
                    any(x.suite == 8 for x in p.akm_suites)
                    and any(x.suite == 2 for x in p.akm_suites)
                    and p.mfp_capable
                    and not p.mfp_required
                ):
...
class Dot11Elt(Packet):
    ...
    fields_desc = [
        ByteEnumField("ID", 0, _dot11_id_enum),
...
class Dot11EltRSN(Dot11Elt):
    ...
    fields_desc = [
        ...
        LEFieldLenField(
            "nb_pairwise_cipher_suites", None, count_of="pairwise_cipher_suites"
        ),
        PacketListField(
        ...
        PacketListField(
            "akm_suites", [AKMSuite()], AKMSuite, count_from=lambda p: p.nb_akm_suites
        ),
...
class Dot11EltCountry(Dot11Elt):
    ...
    fields_desc = [
        ...
        PacketListField(
            ...
            length_from=lambda pkt: (pkt.len - 3 - (pkt.len % 3)),
        ),
        ConditionalField(
            ByteField("pad", 0), lambda pkt: (len(pkt.descriptors) + 1) % 2
        ),
    ]
...
class Dot11EltRates(Dot11Elt):
    ...
    fields_desc = [
        ...
        ByteField("len", None),
        FieldListField("rates", [0x82], _RateField("", 0), length_from=lambda p: p.len),
    ]
...
class Dot11EltHTCapabilities(Dot11Elt):
    ...
    fields_desc = [
        ...
        BitEnumField(
            "SM_Power_Save", 0, 2, {0: "static SM", 1: "dynamic SM", 3: "disabled"}
        ),
...
class Dot11EltMicrosoftWPA(Dot11EltVendorSpecific):
    ...
    fields_desc = (
        Dot11EltVendorSpecific.fields_desc[:3]
...
class Dot11Auth(_Dot11EltUtils):
    ...
    fields_desc = [
        LEShortEnumField("algo", 0, ["open", "sharedkey"]),
...
class Dot11WEP(Dot11Encrypted):
    ...
    def encrypt(self, p, pay, key=None):
        ...
        if key:
            ...
            warning(
                "No WEP key set (conf.wepkey).. strange results expected.."
            )  # noqa: E501
            return b""
