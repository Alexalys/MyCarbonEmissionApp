...
class _ESPPlain(Packet):
    ...
    def data_for_encryption(self):
        return (
            raw(self.data) + self.padding + struct.pack("BB", self.padlen, self.nh)
        )  # noqa: E501
...
if conf.crypto_valid:
    ...
    log_loading.info(
        "Can't import python-cryptography v1.7+. "
...
class CryptAlgo(object):
    ...
    def check_key(self, key):
        ...
        """
        if self.key_size and not (
            len(key) == self.key_size or len(key) in self.key_size
        ):  # noqa: E501
            raise TypeError(
                "invalid key size %s, must be %s" % (len(key), self.key_size)
            )
...
if algorithms:
    ...
    _aes_ctr_format_mode_iv = (
        lambda sa, iv, **kw: sa.crypt_salt + iv + b"\x00\x00\x00\x01"
    )  # noqa: E501
    CRYPT_ALGOS["AES-CTR"] = CryptAlgo(
        "AES-CTR",
    ...
    CRYPT_ALGOS["Blowfish"] = CryptAlgo(
        "Blowfish", cipher=algorithms.Blowfish, mode=modes.CBC
    )
    ...
    CRYPT_ALGOS["DES"] = CryptAlgo(
        "DES", cipher=algorithms.TripleDES, mode=modes.CBC, key_size=(8,)
    )
...
class AuthAlgo(object):
    ...
    def check_key(self, key):
        ...
        if self.key_size and len(key) not in self.key_size:
            raise TypeError(
                "invalid key size %s, must be one of %s" % (len(key), self.key_size)
            )
    ...
    def verify(self, pkt, key, esn_en=False, esn=0):
        ...
        if pkt_icv != computed_icv:
            raise IPSecIntegrityError(
                "pkt_icv=%r, computed_icv=%r" % (pkt_icv, computed_icv)
            )
...
if HMAC and hashes:
    ...
    AUTH_ALGOS["HMAC-SHA1-96"] = AuthAlgo(
        "HMAC-SHA1-96", mac=HMAC, digestmod=hashes.SHA1, icv_size=12
    )
    AUTH_ALGOS["SHA2-256-128"] = AuthAlgo(
        "SHA2-256-128", mac=HMAC, digestmod=hashes.SHA256, icv_size=16
    )
    AUTH_ALGOS["SHA2-384-192"] = AuthAlgo(
        "SHA2-384-192", mac=HMAC, digestmod=hashes.SHA384, icv_size=24
    )
    AUTH_ALGOS["SHA2-512-256"] = AuthAlgo(
        "SHA2-512-256", mac=HMAC, digestmod=hashes.SHA512, icv_size=32
    )
    # XXX:Flagged as deprecated by 'cryptography'. Kept for backward compat
    AUTH_ALGOS["HMAC-MD5-96"] = AuthAlgo(
        "HMAC-MD5-96", mac=HMAC, digestmod=hashes.MD5, icv_size=12
    )
if CMAC and algorithms:
    AUTH_ALGOS["AES-CMAC-96"] = AuthAlgo(
        "AES-CMAC-96", mac=CMAC, digestmod=algorithms.AES, icv_size=12, key_size=(16,)
    )
...
def split_for_transport(orig_pkt, transport_proto):
    ...
    if header.version == 4:
        ...
        while isinstance(
            next_hdr, (IPv6ExtHdrHopByHop, IPv6ExtHdrRouting, IPv6ExtHdrDestOpt)
        ):  # noqa: E501
...
class SecurityAssociation(object):
    ...
    def __init__(
        ...
        if crypt_algo:
            if crypt_algo not in CRYPT_ALGOS:
                raise TypeError(
                    "unsupported encryption algo %r, try %r"
        ...
        if auth_algo:
            if auth_algo not in AUTH_ALGOS:
                raise TypeError(
                    "unsupported integrity algo %r, try %r"
        ...
        if tunnel_header and not isinstance(tunnel_header, (IP, IPv6)):
            raise TypeError(
                "tunnel_header must be %s or %s" % (IP.name, IPv6.name)
            )  # noqa: E501
    ...
    def check_spi(self, pkt):
        if pkt.spi != self.spi:
            raise TypeError(
                "packet spi=0x%x does not match the SA spi=0x%x" % (pkt.spi, self.spi)
            )
    ...
    def _encrypt_esp(self, pkt, seq_num=None, iv=None, esn_en=None, esn=None):
        ...
        if iv is None:
            ...
            if len(iv) != self.crypt_algo.iv_size:
                raise TypeError(
                    "iv length must be %s" % self.crypt_algo.iv_size
                )  # noqa: E501
    ...
    def encrypt(self, pkt, seq_num=None, iv=None, esn_en=None, esn=None):
        ...
        if not isinstance(pkt, self.SUPPORTED_PROTOS):
            raise TypeError(
                "cannot encrypt %s, supported protos are %s"
                % (pkt.__class__, self.SUPPORTED_PROTOS)
            )
        if self.proto is ESP:
            return self._encrypt_esp(
                pkt, seq_num=seq_num, iv=iv, esn_en=esn_en, esn=esn
            )
        else:
    ...
    def _decrypt_ah(self, pkt, verify=True, esn_en=None, esn=None):
        ...
        if verify:
            self.check_spi(pkt)
            self.auth_algo.verify(
                pkt, self.auth_key, esn_en=esn_en or self.esn_en, esn=esn or self.esn
            )
