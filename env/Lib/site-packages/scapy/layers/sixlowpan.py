...
LINK_LOCAL_PREFIX = (
    b"\xfe\x80\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"  # noqa: E501
)
...
class LoWPANMesh(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [(XShortField("src", 0x0), lambda pkt: pkt.v == 1)], XLongField("src", 0x0)
        ),
        MultipleTypeField(
            [(XShortField("dst", 0x0), lambda pkt: pkt.v == 1)], XLongField("dst", 0x0)
        ),
    ]
    ...
    fields_desc = [
        ...
        ConditionalField(
            MultipleTypeField(
                [
                    (
                        PacketField("hc2Field", LoWPAN_HC2_UDP(), LoWPAN_HC2_UDP),
                        lambda pkt: pkt.nh == 1,
                    ),
        ...
        IP6FieldLenField(
            "src", "::", lambda pkt: (0 if pkt.sp else 8) + (0 if pkt.si else 8)
        ),
        IP6FieldLenField(
            "dst", "::", lambda pkt: (0 if pkt.dp else 8) + (0 if pkt.di else 8)
        ),
        ...
        ConditionalField(
            XBitField("udpChecksum", 0, 16), lambda pkt: pkt.nh == 1 and pkt.hc2
        ),
        # Out of spec
...
class LoWPANFragmentationSubsequent(Packet):
    ...
    fields_desc = [
        ...
        ByteField(
            "datagramOffset", 0x0
        ),  # VALUE PRINTED IN OCTETS, wireshark does in bits (128 bits == 16 octets)  # noqa: E501
    ]
...
def _extract_upperaddress(pkt, source=True):
    ...
    if type(underlayer) == Ether:
        ...
        if underlayer.underlayer.fcf_destaddrmode == 3:  # Extended/long
            ...
            # Turn off the bit 7.
            return (
                tmp_ip[0:8] + struct.pack("B", (orb(tmp_ip[8]) ^ 0x2)) + tmp_ip[9:16]
            )  # noqa: E501
...
class LoWPAN_IPHC(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ByteEnumField("nhField", 0x0, ipv6nh), lambda pkt: pkt.nh == 0x0
        ),
        ...
        IP6FieldLenField(
            "dst", "::", length_of=dest_addr_size
        ),  # problem when it's 0  # noqa: E501
    ]
    ...
    def decompressDestAddr(self, packet):
        ...
        if self.m == 0 and self.dac == 0:
            if self.dam == 0:
                ...
                tmp_ip = (
                    LINK_LOCAL_PREFIX[0:8] + b"\x00\x00\x00\xff\xfe\x00" + tmp_ip[-2:]
                )  # noqa: E501
            elif self.dam == 3:
    ...
    def compressSourceAddr(self, ipv6):
        ...
        self.src = inet_ntop(
            socket.AF_INET6, b"\x00" * (16 - len(tmp_ip)) + tmp_ip
        )  # noqa: E501
        return self.src
    ...
    def decompressSourceAddr(self, packet):
        ...
        if self.sac == 0:
            if self.sam == 0x0:
                ...
                tmp_ip = (
                    LINK_LOCAL_PREFIX[0:8] + tmp_ip[16 - source_addr_size(self) : 16]
                )  # noqa: E501
            elif self.sam == 0x2:
    ...
    def guess_payload_class(self, payload):
        ...
        u = self.underlayer
        if u and isinstance(
            u, (LoWPANFragmentationFirst, LoWPANFragmentationSubsequent)
        ):
            return Raw
        return IPv6
...
class LoWPAN_NHC_UDP(LoWPAN_NHC_Hdr):
    fields_desc = [
        ...
        MultipleTypeField(
            [
                (BitField("udpSourcePort", 0, 16), lambda pkt: pkt.P in [0, 1]),
                (BitField("udpSourcePort", 0, 8), lambda pkt: pkt.P == 2),
                (BitField("udpSourcePort", 0, 4), lambda pkt: pkt.P == 3),
            ],
            BitField("udpSourcePort", 0x0, 16),
        ),
        MultipleTypeField(
            [
                (BitField("udpDestPort", 0, 16), lambda pkt: pkt.P in [0, 2]),
                (BitField("udpDestPort", 0, 8), lambda pkt: pkt.P == 1),
                (BitField("udpDestPort", 0, 4), lambda pkt: pkt.P == 3),
            ],
...
def sixlowpan_fragment(packet, datagram_tag=1):
    ...
    new_packet[0] = (
        LoWPANFragmentationFirst(datagramTag=datagram_tag, datagramSize=len(str_packet))
        / new_packet[0]
    )  # noqa: E501
    i = 1
    while i < len(new_packet):
        new_packet[i] = (
            LoWPANFragmentationSubsequent(
                datagramTag=datagram_tag,
                datagramSize=len(str_packet),
                datagramOffset=MAX_SIZE // 8 * i,
            )
            / new_packet[i]
        )  # noqa: E501
        i += 1
