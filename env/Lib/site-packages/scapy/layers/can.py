...
from scapy.compat import Tuple, Optional, Type, List, Union, Callable, IO, Any, cast
...
class SignalField(ScalingField):
    ...
    def addfield(self, pkt, s, val):
        ...
        if self._is_float_number():
            int_val = struct.unpack(self.fmt[0] + "I", struct.pack(self.fmt, val))[
                0
            ]  # type: int
    ...
    def getfield(self, pkt, s):
        ...
        if self._is_float_number():
            fld_val = struct.unpack(self.fmt, struct.pack(self.fmt[0] + "I", fld_val))[
                0
            ]
        elif self._is_signed_number():
...
class LEUnsignedSignalField(SignalField):
    def __init__(
        self, name, default, start, size, scaling=1, unit="", offset=0, ndigits=3
    ):
        # type: (str, Union[int, float], int, int, Union[int, float], str, Union[int, float], int) -> None  # noqa: E501
        SignalField.__init__(
            self, name, default, start, size, scaling, unit, offset, ndigits, "<B"
        )
...
class LEFloatSignalField(SignalField):
    def __init__(self, name, default, start, scaling=1, unit="", offset=0, ndigits=3):
        ...
        SignalField.__init__(
            self, name, default, start, 32, scaling, unit, offset, ndigits, "<f"
        )
...
class SignalPacket(Packet):
    def pre_dissect(self, s):
        # type: (bytes) -> bytes
        if not all(
            isinstance(f, SignalField)
            or (isinstance(f, ConditionalField) and isinstance(f.fld, SignalField))
