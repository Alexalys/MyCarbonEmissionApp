...
class Neighbor:
    def __init__(self):
        ...
        self.resolvers = (
            {}
        )  # type: Dict[Tuple[Type[Packet], Type[Packet]], _ResolverCallable] # noqa: E501
    ...
    def __repr__(self):
        # type: () -> str
        return "\n".join(
            "%-15s -> %-15s" % (l2.__name__, l3.__name__) for l2, l3 in self.resolvers
        )  # noqa: E501
...
def getmacbyip(ip, chainCC=0):
    ...
    try:
        res = srp1(
            Ether(dst=ETHER_BROADCAST) / ARP(op="who-has", pdst=ip),
            type=ETH_P_ARP,
            iface=iff,
            timeout=2,
            verbose=0,
...
class ARP(Packet):
            ...
            [
                (
                    ...
                    (
                        ...
                        lambda pkt, val: pkt.hwtype == 1
                        and (
                            pkt.hwlen == 6
                            or (
                                pkt.hwlen is None
                                and (val is None or len(val) == 6 or valid_mac(val))
                            )
                        ),
                    ),
                ),
            ...
            [
                (
                    ...
                    (
                        ...
                        lambda pkt, val: pkt.ptype == 0x0800
                        and (
                            pkt.plen == 4
                            or (pkt.plen is None and (val is None or valid_net(val)))
                        ),
                    ),
                ),
                (
                    ...
                    (
                        ...
                        lambda pkt, val: pkt.ptype == 0x86DD
                        and (
                            pkt.plen == 16
                            or (pkt.plen is None and (val is None or valid_net6(val)))
                        ),
                    ),
                ),
            ],
    ...
    def hashret(self):
        # type: () -> bytes
        return (
            struct.pack(">HHH", self.hwtype, self.ptype, ((self.op + 1) // 2))
    ...
    def answers(self, other):
        ...
        other_pdst = other.get_field("pdst").i2m(other, other.pdst)  # type: bytes
...
class GRE(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XShortField("offset", None),
            lambda pkt: pkt.chksum_present == 1 or pkt.routing_present == 1,
        ),  # noqa: E501
        ConditionalField(
            XIntField("key", None), lambda pkt: pkt.key_present == 1
        ),  # noqa: E501
        ConditionalField(
            XIntField("sequence_number", None), lambda pkt: pkt.seqnum_present == 1
        ),  # noqa: E501
    ]
...
class GRE_PPTP(GRE):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XIntField("ack_number", None), lambda pkt: pkt.acknum_present == 1
        ),
    ]  # noqa: E501
    ...
    def post_build(self, p, pay):
        ...
        if self.payload_len is None:
            pay_len = len(pay)
            p = (
                p[:4] + chb((pay_len >> 8) & 0xFF) + chb(pay_len & 0xFF) + p[6:]
            )  # noqa: E501
        return p
...
def is_promisc(ip, fake_bcast="ff:ff:00:00:00:00", **kargs):
    ...
    responses = srp1(
        Ether(dst=fake_bcast) / ARP(op="who-has", pdst=ip),
        type=ETH_P_ARP,
        iface_hint=ip,
        timeout=1,
        verbose=0,
...
def promiscping(net, timeout=2, fake_bcast="ff:ff:ff:ff:ff:fe", **kargs):
    ...
    ans, unans = srp(
        Ether(dst=fake_bcast) / ARP(pdst=net),
        filter="arp and arp[7] = 2",
        timeout=timeout,
        iface_hint=net,
        **kargs
...
class ARP_am(AnsweringMachine):
    ...
    def is_request(self, req):
        ...
        arp = req[ARP]
        return arp.op == 1 and (
            self.IP_addr is None or self.IP_addr == arp.pdst
        )  # noqa: E501
...
def etherleak(target, **kargs):
    ...
    return srp(
        ...
        prn=lambda s_r: conf.padding_layer in s_r[1]
        and hexstr(s_r[1][conf.padding_layer].load),  # noqa: E501
        filter="arp",
        **kargs
    )
