...
class _TLSSignature(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        SigAndHashAlgField("sig_alg", 0x0804, _tls_hash_sig),
        SigLenField("sig_len", None, fmt="!H", length_of="sig_val"),
        SigValField("sig_val", None, length_from=lambda pkt: pkt.sig_len),
    ]
...
class ECCurvePkt(Packet):
    name = "Elliptic Curve"
    fields_desc = [
        FieldLenField("alen", None, length_of="a", fmt="B"),
...
class ECTrinomialBasis(Packet):
    ...
    val = 0
    fields_desc = [
        FieldLenField("klen", None, length_of="k", fmt="B"),
...
class ServerECDHExplicitChar2Params(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        ...
        _ECBasisField(
            "basis", ECTrinomialBasis(), lambda pkt: pkt.basis_type, _tls_ec_basis_cls
        ),
...
_tls_server_ecdh_cls = {
    1: ServerECDHExplicitPrimeParams,
...
class ServerPSKParams(Packet):
    ...
    fields_desc = [
        FieldLenField(
            "psk_identity_hint_len", None, length_of="psk_identity_hint", fmt="!H"
        ),
        StrLenField(
            "psk_identity_hint", "", length_from=lambda pkt: pkt.psk_identity_hint_len
        ),
    ]  # noqa: E501
...
class ClientECDiffieHellmanPublic(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        FieldLenField("ecdh_Yclen", None, length_of="ecdh_Yc", fmt="B"),
        StrLenField("ecdh_Yc", "", length_from=lambda pkt: pkt.ecdh_Yclen),
    ]
...
class _UnEncryptedPreMasterSecret(Raw):
    ...
    def __init__(self, *args, **kargs):
        ...
        return super(_UnEncryptedPreMasterSecret, self).__init__(
            *args, **kargs
        )  # noqa: E501
...
class ClientPSKIdentity(Packet):
    ...
    name = "Server PSK parameters"
    fields_desc = [
        FieldLenField("psk_identity_len", None, length_of="psk_identity", fmt="!H"),
        StrLenField("psk_identity", "", length_from=lambda pkt: pkt.psk_identity_len),
    ]
