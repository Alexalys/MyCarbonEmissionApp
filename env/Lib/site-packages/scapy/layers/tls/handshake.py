...
from scapy.layers.tls.basefields import (
    _tls_version,
    _TLSVersionField,
...
from scapy.layers.tls.crypto.compression import (
...
class _TLSHandshake(_GenericTLSSessionInheritance):
    ...
    fields_desc = [
        ByteEnumField("msgtype", None, _tls_handshake_type),
        ThreeBytesField("msglen", None),
        StrLenField("msg", "", length_from=lambda pkt: pkt.msglen),
    ]
...
class TLSHelloRequest(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 0, _tls_handshake_type),
        ThreeBytesField("msglen", None),
    ]
...
class TLSServerHello(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _CompressionMethodsField(
            "comp", [0], _tls_compression_algs, itemfmt="B", length_from=lambda pkt: 1
        ),
        _ExtensionsLenField("extlen", None, length_of="ext"),
        _ExtensionsField(
            "ext", None, length_from=lambda pkt: (pkt.msglen - (pkt.sidlen or 0) - 40)
        ),
    ]
...
class TLSEncryptedExtensions(_TLSHandshake):
    ...
    def post_build_tls_session_update(self, msg_str):
        ...
        if connection_end == "server":
            if not early_data_accepted:
                s.prcs = readConnState(
                    ciphersuite=type(s.wcs.ciphersuite),
                    connection_end=connection_end,
                    tls_version=s.tls_version,
                )

                chts = s.tls13_derived_secrets[
    ...
    def post_dissection_tls_session_update(self, msg_str):
        ...
        if connection_end == "client":
            if not early_data_accepted:
                s.pwcs = writeConnState(
                    ciphersuite=type(s.rcs.ciphersuite),
                    connection_end=connection_end,
                    tls_version=s.tls_version,
                )
                chts = s.tls13_derived_secrets[
...
class _ASN1CertLenField(FieldLenField):
    ...
    def getfield(self, pkt, s):
        return s[3:], self.m2i(
            pkt, struct.unpack(self.fmt, b"\x00" + s[:3])[0]
        )  # noqa: E501
...
class TLSCertificate(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _ASN1CertListField("certs", [], length_from=lambda pkt: pkt.certslen),
    ]
...
class TLS13Certificate(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _ASN1CertAndExtListField(
            "certs", [], _ASN1CertAndExt, length_from=lambda pkt: pkt.certslen
        ),
    ]  # noqa: E501
...
class TLSServerKeyExchange(_TLSHandshake):
    ...
    def post_dissection(self, pkt):
        ...
        if (
            s.prcs
            and not s.prcs.key_exchange.anonymous
            and s.client_random
            and s.server_random
            and s.server_certs
...
class TLS13CertificateRequest(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _ExtensionsLenField("extlen", None, length_of="ext"),
        _ExtensionsField(
            "ext", None, length_from=lambda pkt: pkt.msglen - pkt.cert_req_ctxt_len - 3
        ),
    ]
...
class TLSServerHelloDone(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 14, _tls_handshake_type),
        ThreeBytesField("msglen", None),
    ]
    ...
    def build(self, *args, **kargs):
        ...
        if sig is None:
            ...
            if tls_version >= 0x0304:
                ...
                m = (
                    b"\x20" * 64 + context_string + b"\x00" + s.wcs.hash.digest(m)
                )  # noqa: E501
        ...
        if s.connection_end == "server":
            if s.client_certs and len(s.client_certs) > 0:
                ...
                if not sig_test:
                    pkt_info = pkt.firstlayer().summary()
                    log_runtime.info(
                        "TLS: invalid CertificateVerify signature [%s]", pkt_info
                    )  # noqa: E501
...
class TLSClientKeyExchange(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 16, _tls_handshake_type),
        ThreeBytesField("msglen", None),
        _TLSCKExchKeysField("exchkeys", length_from=lambda pkt: pkt.msglen),
    ]
    ...
    def tls_session_update(self, msg_str):
        ...
        if self.tls_session.extms:
            ...
            if self.tls_session.tls_version >= 0x303:
                ...
                self.tls_session.session_hash = Hash_MD5().digest(
                    to_hash
                ) + Hash_SHA().digest(to_hash)
...
class TLSFinished(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 20, _tls_handshake_type),
        ThreeBytesField("msglen", None),
    ...
    def build(self, *args, **kargs):
        ...
        if fval is None:
            ...
            if tls_version < 0x0304:
                ms = s.master_secret
                self.vdata = s.wcs.prf.compute_verify_data(
                    con_end, "write", handshake_msg, ms
                )
            else:
        ...
        if not s.frozen:
            ...
            if tls_version < 0x0304 and s.master_secret is not None:
                ...
                if self.vdata != verify_data:
                    pkt_info = pkt.firstlayer().summary()
                    log_runtime.info(
                        "TLS: invalid Finished received [%s]", pkt_info
                    )  # noqa: E501
...
class TLSHelloVerifyRequest(_TLSHandshake):
    ...
    fields_desc = [
        ...
        ThreeBytesField("msglen", None),
        FieldLenField("cookielen", None, fmt="B", length_of="cookie"),
        StrLenField("cookie", "", length_from=lambda pkt: pkt.cookielen),
    ]
...
class URLAndOptionalHash(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField(
            ...
            fmt="B",
            length_of="hash",
            adjust=lambda pkt, x: int(math.ceil(x / 20.0)),
        ),  # noqa: E501
...
class TLSCertificateURL(_TLSHandshake):
    ...
    fields_desc = [
        ...
        PacketListField(
            "uah", [], URLAndOptionalHash, length_from=lambda pkt: pkt.uahlen
        ),
    ]
...
class ThreeBytesLenField(FieldLenField):
    def __init__(self, name, default, length_of=None, adjust=lambda pkt, x: x):
        FieldLenField.__init__(
            self, name, default, length_of=length_of, fmt="!I", adjust=adjust
        )
...
class TLSSupplementalData(_TLSHandshake):
    ...
    fields_desc = [
        ...
        PacketListField(
            "sdata", [], SupDataEntry, length_from=lambda pkt: pkt.sdatalen
        ),
    ]
...
class TLS13NewSessionTicket(_TLSHandshake):
    ...
    fields_desc = [
        ...
        _ExtensionsField(
            ...
            None,
            length_from=lambda pkt: (
                pkt.msglen - (pkt.ticketlen or 0) - pkt.noncelen or 0  # noqa: E501
            )
            - 13,
        ),
    ]  # noqa: E501
...
class TLS13EndOfEarlyData(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 5, _tls_handshake_type),
        ThreeBytesField("msglen", None),
    ]
...
class TLS13KeyUpdate(_TLSHandshake):
    ...
    fields_desc = [
        ByteEnumField("msgtype", 24, _tls_handshake_type),
        ThreeBytesField("msglen", None),
