    ...
    def post_build(self, pkt, pay):
        if not self.tls_session.frozen and self.server_share.privkey:
            ...
            if len(privshare) > 0:
                pkt_info = pkt.firstlayer().summary()
                log_runtime.info(
                    "TLS: overwriting previous server key share [%s]", pkt_info
                )  # noqa: E501
...
class PSKIdentity(Packet):
    name = "PSK Identity"
    fields_desc = [
        FieldLenField("identity_len", None, length_of="identity"),
        TicketField("identity", "", length_from=lambda pkt: pkt.identity_len),
...
class TLS_Ext_PreSharedKey_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [
        ...
        PacketListField(
            "identities", [], PSKIdentity, length_from=lambda pkt: pkt.identities_len
        ),
