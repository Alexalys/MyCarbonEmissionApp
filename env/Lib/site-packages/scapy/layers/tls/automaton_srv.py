...
class TLSServerAutomaton(_TLSAutomaton):
    ...
    def WAITING_CLIENT(self):
        ...
        self.vprint()
        self.vprint(
            "Waiting for a new client on %s:%d" % (self.local_ip, self.local_port)
        )
        ...
        self.vprint(
            "Accepted connection from %s:%d" % (self.remote_ip, self.remote_port)
        )
        self.vprint()
    ...
    def verify_psk_binder(self, psk_identity, obfuscated_age, binder):
        ...
        with open(self.session_ticket_file, "rb") as f:
            for line in f:
                ...
                if psk_identity.__bytes__() == ticket_label:
                    ...
                    tls13_psk_secret = hkdf.expand_label(
                        resumption_secret, b"resumption", ticket_nonce, hash_len
                    )
                    # We verify that ticket age is not expired
                    agesec = int((time.time() - ticket_start_time))
                    # agems = agesec * 1000
                    ticket_age = (
                        obfuscated_age - ticket_age_add
                    ) % 0xFFFFFFFF  # noqa: F841, E501
                    ...
                    if s.tls13_retry:
                        ...
                        handshake_context = s.handshake_messages[0][
                            : -hash_len - 3
                        ]  # noqa: E501
                    ...
                    binder_key = hkdf.derive_secret(
                        tls13_early_secret, b"res binder", b""
                    )
                    computed_binder = hkdf.compute_verify_data(
        ...
        self.vprint(
            "Ticket has not been accepted ! Fallback to a complete handshake"
        )  # noqa: E501
        return None
    ...
    def tls13_should_add_ServerHello(self):
        ...
        if psk_identity and obfuscated_age and psk_key_exchange_mode:
            ...
            if is_out_of_band_psk:
                ...
                resumption_psk = self.verify_psk_binder(
                    psk_identity, obfuscated_age, binder
                )
    ...
    def save_ticket(self, ticket):
        ...
        """
        if (
            not isinstance(ticket, TLS13NewSessionTicket)
            or self.session_ticket_file is None
        ):
            return
        ...
        with open(self.session_ticket_file, "ab") as f:
            ...
            line += b";"
            line += binascii.hexlify(
                s.tls13_derived_secrets["resumption_secret"]
            )  # noqa: E501
            line += b";"
    ...
    def close_session(self):
        ...
        try:
            ...
            self.vprint(
                "Could not send termination Alert, maybe the client left?"
            )  # noqa: E501
            self.buffer_out = []
        self.socket.close()
        raise self.WAITING_CLIENT()
    ...
    def sslv2_should_handle_ClientCertificate(self):
        self.raise_on_packet(
            SSLv2ClientCertificate, self.SSLv2_HANDLED_CLIENTCERTIFICATE
        )
    ...
    def sslv2_close_session(self):
        ...
        try:
            ...
            self.vprint(
                "Could not send our goodbye. The client probably left."
            )  # noqa: E501
            self.buffer_out = []
        self.socket.close()
        raise self.WAITING_CLIENT()
