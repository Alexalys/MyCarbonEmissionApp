...
class TLSClientAutomaton(_TLSAutomaton):
        ...
        if s.advertised_tls_version >= 0x0304:
            ...
            if self.session_ticket_file_in:
                with open(self.session_ticket_file_in, "rb") as f:
                    ...
                    s.tls13_ticket_ciphersuite = struct.unpack(
                        "!H", f.read(resumed_ciphersuite_len)
                    )[0]
                ...
                if self.resumption_master_secret:
                    ...
                    if (
                        s.tls13_ticket_ciphersuite not in _tls_cipher_suites_cls
                    ):  # noqa: E501
                        warning(
                            "Unknown cipher suite %d", s.tls13_ticket_ciphersuite
                        )  # noqa: E501
    ...
    def should_add_ClientVerify(self):
        ...
        hs_msg = [type(m) for m in self.cur_session.handshake_messages_parsed]
        if (
            TLSCertificateRequest not in hs_msg
            or self.mycert is None
            or self.mykey is None
        ):
            return
    ...
    def add_ClientData(self):
        ...
        if not self.data_to_send:
            if self.is_atmt_socket:
                ...
                data = (
                    six.moves.input().replace("\\r", "\r").replace("\\n", "\n").encode()
                )  # noqa: E501
        else:
    ...
    def close_session(self):
        ...
        try:
            ...
            self.vprint(
                "Could not send termination Alert, maybe the server stopped?"
            )  # noqa: E501
        raise self.FINAL()
    ...
    def sslv2_should_handle_ServerVerify(self):
        self.raise_on_packet(
            SSLv2ServerVerify, self.SSLv2_HANDLED_SERVERVERIFY, get_next_msg=False
        )
    ...
    def sslv2_should_handle_RequestCertificate(self):
        self.raise_on_packet(
            SSLv2RequestCertificate, self.SSLv2_HANDLED_REQUESTCERTIFICATE
        )
    ...
    def sslv2_close_session(self):
        ...
        try:
            ...
            self.vprint(
                "Could not send our goodbye. The server probably stopped."
            )  # noqa: E501
        self.socket.close()
        raise self.FINAL()
    ...
    def tls13_should_add_ClientHello(self):
        ...
        if s.tls13_psk_secret:
            ...
            if s.client_session_ticket:
                ...
                psk_id = PSKIdentity(
                    identity=s.client_session_ticket,
                    obfuscated_ticket_age=obfuscated_age,
                )

                psk_binder_entry = PSKBinderEntry(
                    binder_len=hash_len, binder=b"\x00" * hash_len
                )

                ext += TLS_Ext_PreSharedKey_CH(
                    identities=[psk_id], binders=[psk_binder_entry]
                )
            else:
    ...
    def tls13_should_add_ClientHello_Retry(self):
        ...
        ext += TLS_Ext_SupportedVersion_CH(
            versions=[_tls_version[selected_version]]
        )  # noqa: E501
        ...
        if s.tls13_psk_secret:
            if self.tls13_psk_mode == "psk_dhe_ke":
                ...
                ext += TLS_Ext_SupportedGroups(
                    groups=[_tls_named_groups[selected_group]]
                )  # noqa: E501
                ext += TLS_Ext_KeyShare_CH(
                    client_shares=[KeyShareEntry(group=selected_group)]
                )  # noqa: E501
            else:
                ext += TLS_Ext_PSKKeyExchangeModes(kxmodes="psk_ke")
    ...
    def tls13_should_add_ClientCertificateVerify(self):
        ...
        hs_msg = [type(m) for m in self.cur_session.handshake_messages_parsed]
        if (
            TLS13CertificateRequest not in hs_msg
            or self.mycert is None
            or self.mykey is None
        ):
