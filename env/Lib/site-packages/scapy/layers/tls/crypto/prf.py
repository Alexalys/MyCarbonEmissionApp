...
class PRF(object):
    ...
    def __init__(self, hash_name="SHA256", tls_version=0x0303):
        ...
        if tls_version < 0x0300:  # SSLv2
            ...
            self.prf = _ssl_PRF
        elif tls_version == 0x0301 or tls_version == 0x0302:  # TLS 1.0  # TLS 1.1
            self.prf = _tls_PRF
    ...
    def compute_verify_data(self, con_end, read_or_write, handshake_msg, master_secret):
        ...
        if self.tls_version < 0x0300:
            ...
            md5_hash = md5.digest(
                master_secret
                + sslv3_md5_pad2
                + md5.digest(handshake_msg + label + master_secret + sslv3_md5_pad1)
            )
            sha1_hash = sha1.digest(
                master_secret
                + sslv3_sha1_pad2
                + sha1.digest(handshake_msg + label + master_secret + sslv3_sha1_pad1)
            )  # noqa: E501
    ...
    def postprocess_key_for_export(
        self, key, client_random, server_random, con_end, read_or_write, req_len
    ):
        """
    ...
    def generate_iv_for_export(
        self, client_random, server_random, con_end, read_or_write, req_len
    ):
        """
        ...
        if self.tls_version < 0x0300:
            ...
            iv_block = self.prf(
                "", b"IV block", client_random + server_random, 2 * req_len
            )
            if s:
                iv = iv_block[:req_len]
            else:
