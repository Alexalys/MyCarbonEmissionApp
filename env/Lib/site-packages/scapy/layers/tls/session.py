...
class connState(object):
    ...
    def derive_keys(self, client_random=b"", server_random=b"", master_secret=b""):
        ...
        key_block = self.prf.derive_key_block(
            master_secret, server_random, client_random, cs.key_block_len
        )
        ...
        if cipher_alg.type == "stream":
            ...
            if cs.kx_alg.export:
                ...
                iv = self.prf.generate_iv_for_export(
                    client_random, server_random, self.connection_end, self.row, reqLen
                )
...
class tlsSession(object):
    ...
    def mirror(self):
        ...
        self.triggered_prcs_commit, self.triggered_pwcs_commit = (
            self.triggered_pwcs_commit,
            self.triggered_prcs_commit,
        )
    ...
    def compute_master_secret(self):
        ...
        ms = self.pwcs.prf.compute_master_secret(
    ...
    def compute_ms_and_derive_keys(self):
        ...
        self.prcs.derive_keys(
            client_random=self.client_random,
            server_random=self.server_random,
            master_secret=self.master_secret,
        )
        self.pwcs.derive_keys(
            client_random=self.client_random,
            server_random=self.server_random,
            master_secret=self.master_secret,
        )
    ...
    def compute_sslv2_key_material(self):
        ...
        km = self.pwcs.prf.derive_key_block(
            self.master_secret,
        ...
        if conf.debug_tls:
            log_runtime.debug(
                "TLS: master secret: %s", repr_hex(self.master_secret)
            )  # noqa: E501
    ...
    def compute_tls13_early_secrets(self, external=False):
        ...
        cets = hkdf.derive_secret(
            self.tls13_early_secret, b"c e traffic", b"".join(self.handshake_messages)
        )

        self.tls13_derived_secrets["client_early_traffic_secret"] = cets
        ees = hkdf.derive_secret(
            self.tls13_early_secret, b"e exp master", b"".join(self.handshake_messages)
        )
    ...
    def compute_tls13_handshake_secrets(self):
        ...
        self.tls13_handshake_secret = hkdf.extract(
            secret, self.tls13_dhe_secret
        )  # noqa: E501

        chts = hkdf.derive_secret(
            self.tls13_handshake_secret,
            b"c hs traffic",
            b"".join(self.handshake_messages),
        )
        self.tls13_derived_secrets["client_handshake_traffic_secret"] = chts

        shts = hkdf.derive_secret(
            self.tls13_handshake_secret,
            b"s hs traffic",
            b"".join(self.handshake_messages),
        )
    ...
    def compute_tls13_traffic_secrets(self):
        ...
        cts0 = hkdf.derive_secret(
            self.tls13_master_secret, b"c ap traffic", b"".join(self.handshake_messages)
        )
        self.tls13_derived_secrets["client_traffic_secrets"] = [cts0]

        sts0 = hkdf.derive_secret(
            self.tls13_master_secret, b"s ap traffic", b"".join(self.handshake_messages)
        )
        self.tls13_derived_secrets["server_traffic_secrets"] = [sts0]

        es = hkdf.derive_secret(
            self.tls13_master_secret, b"exp master", b"".join(self.handshake_messages)
        )
    ...
    def compute_tls13_resumption_secret(self):
        ...
        if self.connection_end == "server":
            ...
            hkdf = self.pwcs.hkdf
        rs = hkdf.derive_secret(
            self.tls13_master_secret, b"res master", b"".join(self.handshake_messages)
        )
        self.tls13_derived_secrets["resumption_secret"] = rs

    def compute_tls13_next_traffic_secrets(
        self, connection_end, read_or_write
    ):  # noqa : E501
        """
    ...
    def consider_read_padding(self):
        ...
        return self.rcs.cipher.type == "block" and not (
            False in six.itervalues(self.rcs.cipher.ready)
        )
    ...
    def eq(self, other):
        ...
        if (
            not ok
            and self.dport == other.sport
            and self.sport == other.dport
            and self.ipdst == other.ipsrc
            and self.ipsrc == other.ipdst
        ):
            ok = True
