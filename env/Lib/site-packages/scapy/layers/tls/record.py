...
class _TLSMsgListField(PacketListField):
    ...
    def __init__(self, name, default, length_from=None):
        ...
        super(_TLSMsgListField, self).__init__(
            name, default, None, length_from=length_from
        )
    ...
    def getfield(self, pkt, s):
        ...
        if remain == b"":
            if (
                ((pkt.tls_session.tls_version or 0x0303) > 0x0200)
            ...
            if plen >= 2:
                ...
                if byte0 not in _tls_type or byte1 != 3:  # Unknown type
                    ...
                    if (
                        s
                        and (
                            s.tls_version == 0x0002
                            or s.advertised_tls_version == 0x0002
                        )
                        or (
                            _ssl_looks_like_sslv2(_pkt)
                            and (not s or s.tls_version is None)
                        )
                    ):
                ...
                if s and _tls_version_check(s.tls_version, 0x0304):
                    if (
                        s.rcs
                        and not isinstance(s.rcs.cipher, Cipher_NULL)
                        and byte0 == 0x17
                    ):
...
class TLSAlert(_GenericTLSSessionInheritance):
    name = "TLS Alert"
    fields_desc = [
        ByteEnumField("level", None, _tls_alert_level),
