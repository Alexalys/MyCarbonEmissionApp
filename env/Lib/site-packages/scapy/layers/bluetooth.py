    ...
    def answers(self, other):
        if other.id == self.id:
            if self.code == 1:
                return 1
            if (
                other.code in [2, 4, 6, 8, 10, 18] and self.code == other.code + 1
            ):  # noqa: E501
                if other.code == 8:
                    return 1
...
class L2CAP_ConnReq(Packet):
    ...
    fields_desc = [
        LEShortEnumField(
            "psm", 0, {1: "SDP", 3: "RFCOMM", 5: "telephony control"}
        ),  # noqa: E501
        LEShortField("scid", 0),
    ]
...
class ATT_Handle_Variable(Packet):
    ...
    fields_desc = [
        XLEShortField("handle", 0),
        XStrLenField("value", 0, length_from=lambda pkt: pkt.val_length),
    ]
...
class HCI_Cmd_Write_Extended_Inquiry_Response(Packet):
    ...
    fields_desc = [
        ByteField("fec_required", 0),
        PacketListField("eir_data", [], EIR_Hdr, length_from=lambda pkt: pkt.len),
    ]
...
class HCI_Cmd_LE_Remove_Device_From_White_List(
...
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_Write_Extended_Inquiry_Response, opcode=0x0C52
)  # noqa: E501
...
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Set_Advertising_Parameters, opcode=0x2006
)  # noqa: E501
...
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Create_Connection_Cancel, opcode=0x200E
)  # noqa: E501
...
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Add_Device_To_White_List, opcode=0x2011
)  # noqa: E501
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Remove_Device_From_White_List, opcode=0x2012
)  # noqa: E501
bind_layers(HCI_Command_Hdr, HCI_Cmd_LE_Connection_Update, opcode=0x2013)
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Read_Remote_Used_Features, opcode=0x2016
)  # noqa: E501


bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Start_Encryption_Request, opcode=0x2019
)  # noqa: E501
...
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Long_Term_Key_Request_Reply, opcode=0x201A
)  # noqa: E501
bind_layers(
    HCI_Command_Hdr, HCI_Cmd_LE_Long_Term_Key_Request_Negative_Reply, opcode=0x201B
)  # noqa: E501
...
bind_layers(
    HCI_Event_Command_Complete, HCI_Cmd_Complete_Read_BD_Addr, opcode=0x1009
)  # noqa: E501
bind_layers(
...
class LowEnergyBeaconHelper:
    ...
    def build_advertising_report(self):
        ...
        return HCI_LE_Meta_Advertising_Report(
            type=0, atype=1, data=self.build_eir()  # Undirected  # Random address
        )
