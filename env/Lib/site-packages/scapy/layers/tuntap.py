...
class LinuxTunPacketInfo(TunPacketInfo):
    ...
    fields_desc = [
        ...
        FlagsField(
            ...
            ["TUN_VNET_HDR"] + ["reserved%d" % x for x in range(1, 16)],
        ),
        ...
        **kwargs
        ...
        if LINUX:
            ...
            if len(self.iface) > LINUX_IFNAMSIZ:
                warning(
                    "Linux interface names are limited to %d bytes, "
            ...
            if not (self.iface.startswith(b"tap") or self.iface.startswith(b"tun")):
                raise ValueError(
                    "Interface names must start with `tun` or "
                    "`tap` on BSD and Darwin"
                )
            devname = b"/dev/" + self.iface
            if not self.strip_packet_info:
                warning(
                    "tun/tap devices on BSD and Darwin never include " "packet info!"
                )
                self.strip_packet_info = True
        else:
            raise NotImplementedError(
                "TunTapInterface is not supported on " "this platform!"
            )
        ...
        if LINUX:
            ...
            tsetiff = raw(
                LinuxTunIfReq(ifrn_name=bytes_encode(self.iface), ifru_flags=flags)
            )
