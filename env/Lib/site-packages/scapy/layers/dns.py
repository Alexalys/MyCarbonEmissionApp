...
def dns_encode(x, check_built=False):
    ...
    if (
        check_built
        and b"." not in x
        and ((x and orb(x[-1]) == 0) or (len(x) >= 2 and (orb(x[-2]) & 0xC0) == 0xC0))
    ):
...
def DNSgetstr(*args, **kwargs):
    ...
    warnings.warn(
        "DNSgetstr is deprecated. Use dns_get_str instead.", DeprecationWarning
    )
                ...
                if isinstance(current, InheritOriginDNSStrPacket):
                    for field in current.fields_desc:
                        if isinstance(field, DNSStrField) or (
                            isinstance(field, MultipleTypeField)
                            and current.type in [2, 3, 4, 5, 12, 15]
                        ):
...
class DNS(Packet):
    ...
    fields_desc = [
        ConditionalField(
            ShortField("length", None), lambda p: isinstance(p.underlayer, TCP)
        ),
        ShortField("id", 0),
        BitField("qr", 0, 1),
...
class DNSQR(InheritOriginDNSStrPacket):
    ...
    show_indent = 0
    fields_desc = [
        DNSStrField("qname", "www.example.com"),
...
class EDNS0TLV(Packet):
    ...
    fields_desc = [
        ShortEnumField(
            "optcode",
            0,
            {0: "Reserved", 1: "LLQ", 2: "UL", 3: "NSID", 4: "Reserved", 5: "PING"},
        ),  # noqa: E501
...
class DNSRRNSEC3(_DNSRRdummy):
    ...
    fields_desc = [
        ...
        FieldLenField(
            "hashlength", 0, fmt="!B", length_of="nexthashedownername"
        ),  # noqa: E501
        StrLenField(
            "nexthashedownername", "", length_from=lambda x: x.hashlength
        ),  # noqa: E501
        RRlistField("typebitmaps", ""),
    ]
...
class DNSRRTSIG(_DNSRRdummy):
    ...
    fields_desc = [
        ...
        StrLenField(
            "other_data", "", length_from=lambda pkt: pkt.other_len
        ),  # noqa: E501
    ]
