...
IPV6_ADDR_6TO4 = (
    0x0100  # Added to have more specific info (should be 0x0101 ?)  # noqa: E501
)
...
class EtherDA(DADict[int, str]):
    ...
    def __setitem__(self, attr, val):
        ...
        if isinstance(attr, str):
            attr, val = val, attr
            warnings.warn(
                "ETHER_TYPES now uses the integer value as key !", DeprecationWarning
            )
...
def load_ethertypes(filename):
    ...
    prot = load_protocols(
        filename or "Scapy's backup ETHER_TYPES",
        _fallback=DATA,
        _integer_base=16,
        _cls=EtherDA,
    )
...
class ManufDA(DADict[str, Tuple[str, str]]):
    ...
    def reverse_lookup(self, name, case_sensitive=False):
        ...
        """
        if case_sensitive:
            filtr = lambda x, l: any(
                x in z for z in l
            )  # type: Callable[[str, Tuple[str, str]], bool]  # noqa: E501
        else:
            name = name.lower()
    ...
    with open(filename, "rb") as fdesc:
        for line in fdesc:
            try:
                ...
                log_loading.warning(
                    "Couldn't parse one line from [%s] [%r]",
                    filename,
                    line,
                    exc_info=True,
                )
