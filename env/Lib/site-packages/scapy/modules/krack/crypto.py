...
def gen_TKIP_RC4_key(TSC, TA, TK):
    ...
    for i in range(PHASE1_LOOP_CNT):
        j = 2 * (i & 1)
        TTAK[0] = _CAST16(
            TTAK[0] + _SBOX16(TTAK[4] ^ _MK16(TK[1 + j], TK[0 + j]))
        )  # noqa: E501
        TTAK[1] = _CAST16(
            TTAK[1] + _SBOX16(TTAK[0] ^ _MK16(TK[5 + j], TK[4 + j]))
        )  # noqa: E501
        TTAK[2] = _CAST16(
            TTAK[2] + _SBOX16(TTAK[1] ^ _MK16(TK[9 + j], TK[8 + j]))
        )  # noqa: E501
        TTAK[3] = _CAST16(
            TTAK[3] + _SBOX16(TTAK[2] ^ _MK16(TK[13 + j], TK[12 + j]))
        )  # noqa: E501
        TTAK[4] = _CAST16(
            TTAK[4] + _SBOX16(TTAK[3] ^ _MK16(TK[1 + j], TK[0 + j])) + i
        )  # noqa: E501
...
def build_TKIP_payload(data, iv, mac, tk):
    ...
    TKIP_hdr = (
        chb(TSC1) + chb((TSC1 | 0x20) & 0x7F) + chb(TSC0) + chb(bitfield)
    )  # noqa: E501
