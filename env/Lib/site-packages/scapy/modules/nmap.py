...
class NmapKnowledgeBase(KnowledgeBase):
    ...
    def lazy_init(self):
        try:
            fdesc = open(
                conf.nmap_base if self.filename is None else self.filename, "rb"
            )
        ...
        for line in fdesc:
            ...
            sig[test] = dict(
                val.split("=", 1) for val in (values.split("%") if values else [])
            )
        if name is not None:
            self.base.append((name, sig))
        fdesc.close()
...
def nmap_udppacket_sig(snd, rcv):
    ...
    if rcv is None:
        ...
        res["RIPCK"] = (
            "E"
            if snd.chksum == rcv[IPerror].chksum
            else ("0" if rcv[IPerror].chksum == 0 else "F")
        )
        res["UCK"] = (
            "E"
            if snd.payload.chksum == rcv[UDPerror].chksum
            else ("0" if rcv[UDPerror].chksum == 0 else "F")
        )
        res["ULEN"] = "%X" % rcv[UDPerror].len
        res["DAT"] = (
            "E"
            if (
                isinstance(rcv[UDPerror].payload, NoPayload)
                or raw(rcv[UDPerror].payload) == raw(snd[UDP].payload)
            )
            else "F"
        )
    return res
