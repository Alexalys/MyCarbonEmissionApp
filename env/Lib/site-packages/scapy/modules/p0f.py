...
def p0f_correl(x, y):
    ...
    d += (
        x[0] == y[0]
        or y[0] == "*"
        or (y[0][0] == "%" and x[0].isdigit() and (int(x[0]) % int(y[0][1:])) == 0)
    )  # noqa: E501
    # ttl
        ...
        for i in range(len(xopt)):
            if not (
                ...
                or (
                    ...
                    and yopt[i][1] == "%"
                    and xopt[i][0] == yopt[i][0]
                    and int(xopt[i][1:]) % int(yopt[i][2:]) == 0
                )
            ):
                same = False
                break
                ...
                if int(opt[1:]) in TCPOptions[0]:
                    ...
                    options.append(
                        (
                            optname,
                            struct.unpack(
                                optstruct, RandString(struct.calcsize(optstruct))._fix()
                            ),
                        )
                    )  # noqa: E501
                else:
...
def p0f_getlocalsigs():
    ...
    if pid > 0:
        ...
        count = 14
        pl = sniff(
            iface=iface, filter="tcp and port " + str(port), count=count, timeout=3
        )  # noqa: E501
        for pkt in pl:
