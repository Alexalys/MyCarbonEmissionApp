...
class I3FieldLenField(X3BytesField, FieldLenField):
    ...
    def __init__(
        self, name, default, length_of=None, count_of=None, adjust=lambda pkt, x: x
    ):
...
def GuessAvpType(p, **kargs):
    if len(p) > AVP_Code_length + AVP_Flag_length:
        ...
        vnd = bool(
            struct.unpack("!B", p[AVP_Code_length : AVP_Code_length + AVP_Flag_length])[
                0
            ]
            & 128
        )  # noqa: E501
        ...
        try:
            for vnd in AvpDefDict:
                for code in AvpDefDict[vnd]:
                    ...
                    if (
                        val[0][: len(avpId)] == avpId
                    ):  # A prefix of the full name is considered valid  # noqa: E501
                        raise
            found = False
        except BaseException:
            found = True
...
class AVP_VL_NV(AVP_Generic):
    ...
    fields_desc = [
        IntField("avpCode", None),
        AVPFlags("avpFlags", None, 8, AVP_Flags_List),
        I3FieldLenField("avpLen", None, length_of="val", adjust=lambda pkt, x: x + 8),
    ]
...
class AVP_VL_V(AVP_Generic):
    ...
    fields_desc = [
        IntField("avpCode", None),
        AVPFlags("avpFlags", None, 8, AVP_Flags_List),
        I3FieldLenField("avpLen", None, length_of="val", adjust=lambda pkt, x: x + 12),
        AVPVendor("avpVnd", 0),
    ]
...
class AVP_Unknown(AVP_Generic):
    ...
    fields_desc = [
        ...
        ConditionalField(
            AVPVendor("avpVnd", 0), lambda pkt: pkt.avpFlags & 0x80
        ),  # noqa: E501
        StrLenField(
            "val",
            None,
...
class AVPV_Grouped(AVP_VL_V):
    fields_desc = [
        AVP_VL_V,
        PacketListField(
            "val", [], GuessAvpType, length_from=lambda pkt: pkt.avpLen - 12
        ),
    ]
...
class AVPNV_Grouped(AVP_VL_NV):
    fields_desc = [
        AVP_VL_NV,
        PacketListField(
            "val", [], GuessAvpType, length_from=lambda pkt: pkt.avpLen - 8
        ),
    ]
...
DR_cmd_def = {
    ...
    265: (
        "AA",
        "AA",
        {16777272: (192, 64), 1: (192, 64), 16777250: (192, 64), 16777264: (192, 64)},
    ),  # noqa: E501
    268: (
        "Diameter-EAP",
        "DE",
    ...
    301: (
        "Server-Assignment",
        "SA",
        {16777216: (192, 64), 16777265: (192, 64)},
    ),  # noqa: E501
...
class DiamG(Packet):
    ...
    fields_desc = [
        ...
        I3FieldLenField(
            "drLen", None, length_of="avpList", adjust=lambda p, x: x + DR_Header_Length
        ),
