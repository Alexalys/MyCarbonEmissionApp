...
class SET_S_STATUS(Packet):
    ...
    fields_desc = [
        FlagsField(
            ...
            0,
            8,
            ["CAL", "DAQ", "RESUME", "RES0", "RES1", "RES2", "STORE", "RUN"],
        ),
        StrFixedLenField("ccp_reserved", b"\xff" * 5, length=5),
    ]
...
class PROGRAM(Packet):
    fields_desc = [
        XByteField("size", 0),
        StrFixedLenField("data", b"\x00" * 0, length_from=lambda pkt: pkt.size),
        StrFixedLenField(
            "ccp_reserved", b"\xff" * 5, length_from=lambda pkt: 5 - pkt.size
        ),
    ]
...
class BUILD_CHKSUM_DTO(Packet):
    fields_desc = [
        ...
        StrFixedLenField(
            "checksum_data", b"\x00" * 4, length_from=lambda pkt: pkt.checksum_size
        ),
        StrFixedLenField(
            "ccp_reserved", b"\xff" * 0, length_from=lambda pkt: 4 - pkt.checksum_size
        ),
    ]
