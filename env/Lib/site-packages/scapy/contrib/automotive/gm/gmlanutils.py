...
__all__ = [
    ...
    "GMLAN_GetSecurityAccess",
    "GMLAN_RequestDownload",
    "GMLAN_TransferData",
    "GMLAN_TransferPayload",
    "GMLAN_ReadMemoryByAddress",
...
def GMLAN_InitDiagnostics(
    sock, broadcast_socket=None, timeout=None, verbose=None, retry=0
):  # noqa: E501
...
def GMLAN_GetSecurityAccess(
    sock, key_function, level=1, timeout=None, verbose=None, retry=0
):  # noqa: E501
    ...
    while retry >= 0:
        ...
        keypkt = GMLAN() / GMLAN_SA(
            subfunction=level + 1, securityKey=key_function(seed)
        )
        if verbose:
        ...
        if verbose:
            resp.show()
        if (
            resp.sprintf("%GMLAN.service%") == "SecurityAccessPositiveResponse"
        ):  # noqa: E501
            if verbose:
        ...
        # Invalid Key
            ...
            resp.sprintf("%GMLAN.service%") == "NegativeResponse"
            and resp.sprintf("%GMLAN.returnCode%") == "InvalidKey"
        ):
            if verbose:
                print("Key invalid")
            continue
...
def GMLAN_TransferData(
    sock, addr, payload, maxmsglen=None, timeout=None, verbose=None, retry=0
):  # noqa: E501
    ...
    if addr < 0 or addr >= 2 ** (8 * scheme):
        warning("Error: Invalid address %s for scheme %s", hex(addr), str(scheme))
        return False
...
def GMLAN_TransferPayload(
    ...
    :return: True on success.
    """
    if not GMLAN_RequestDownload(
        sock, len(payload), timeout=timeout, verbose=verbose, retry=retry
    ):
        return False
    if not GMLAN_TransferData(
        sock,
        addr,
        payload,
        maxmsglen=maxmsglen,
...
def GMLAN_ReadMemoryByAddress(sock, addr, length, timeout=None, verbose=None, retry=0):
    ...
    if length <= 0 or length > (4094 - scheme):
        warning(
            "Error: Invalid length %s for scheme %s. " "Choose between 0x1 and %s",
            hex(length),
            str(scheme),
            hex(4094 - scheme),
        )
...
def GMLAN_BroadcastSocket(interface):
    ...
    """
    return ISOTPSocket(
        interface, sid=0x101, did=0x0, basecls=GMLAN, extended_addr=0xFE, padding=True
    )
