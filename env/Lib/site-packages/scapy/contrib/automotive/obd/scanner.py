...
class OBD_Enumerator(Enumerator):
    def scan(
        ...
        state,
        requests,
        exit_scan_on_first_negative_response=False,
        ...
        **kwargs
        ...
        for req in requests:
            ...
            for _ in range(retries):
                res = self.sock.sr1(req, timeout=timeout, verbose=False, **kwargs)
                if not retry_if_busy_returncode:
                    break
                elif res and res.service == 0x7F and res.response_code == 0x21:
                    continue

            self.results.append(Enumerator.ScanResult(state, req, res))
            if res and res.service == 0x7F and exit_scan_on_first_negative_response:
                break
        self.update_stats()
    ...
    def filtered_results(self):
        return [
            r
            for r in super(OBD_Enumerator, self).filtered_results
    ...
    def show_negative_response_details(self, dump=False):
        nrs = [
            ...
            if r.resp is not None and r.resp.service == 0x7F
        ]
        s = ""
        if len(nrs):
            ...
            for nrc in nrcs:
                s += "\tNRC 0x%02x: %s received %d times" % (
                    nrc,
                    OBD_NR(response_code=nrc).sprintf("%OBD_NR.response_code%"),
    ...
    def get_label(
        ...
        return Enumerator.get_label(
            response, positive_case, response.sprintf("NR: %OBD_NR.response_code%")
        )
...
class OBD_S03_Enumerator(OBD_DTC_Enumerator):
    ...
    def get_table_entry(tup):
        ...
        label = OBD_Enumerator.get_label(
            res, positive_case=lambda: OBD_DTC_Enumerator.print_payload(res)
        )
...
class OBD_S01_Enumerator(OBD_Service_Enumerator):
    ...
    def get_table_entry(tup):
        _, _, res = tup
        label = OBD_Enumerator.get_label(
            res, positive_case=lambda: OBD_Service_Enumerator.print_payload(res)
        )
        return ("Service 01", "%s" % res.data_records[0].lastlayer().name, label)
...
class OBD_S02_Enumerator(OBD_Service_Enumerator):
    ...
    def get_pkts(self, p_range):
        return (OBD() / OBD_S02(requests=[OBD_S02_Record(pid=[x])]) for x in p_range)
    ...
    def get_table_entry(tup):
        ...
        return ("Service 02", "%s" % res.data_records[0].lastlayer().name, label)
...
class OBD_S08_Enumerator(OBD_Service_Enumerator):
    ...
    def get_table_entry(tup):
        ...
        return (
            "Service 08",
            "0x%02x %s" % (req.tid[0], res.data_records[0].lastlayer().name),
            label,
        )
...
class OBD_S09_Enumerator(OBD_Service_Enumerator):
    ...
    def get_table_entry(tup):
        ...
        return (
            "Service 09",
            "0x%02x %s" % (req.iid[0], res.data_records[0].lastlayer().name),
            label,
        )
