...
class UDS(ISOTP):
    ...
    def answers(self, other):
        ...
        if self.service == (other.service + 0x40):
            if isinstance(self.payload, NoPayload) or isinstance(
                other.payload, NoPayload
            ):
...
class UDS_DSCPR(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, UDS_DSC)
            and other.diagnosticSessionType == self.diagnosticSessionType
        )
...
class UDS_ERPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ByteField("powerDownTime", 0), lambda pkt: pkt.resetType == 0x04
        ),
    ]
...
class UDS_SA(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrField("securityAccessDataRecord", b""),
            lambda pkt: pkt.securityAccessType % 2 == 1,
        ),
        ConditionalField(
            StrField("securityKey", b""), lambda pkt: pkt.securityAccessType % 2 == 0
        ),
    ]
...
class UDS_SAPR(Packet):
    ...
    fields_desc = [
        ByteField("securityAccessType", 0),
        ConditionalField(
            StrField("securitySeed", b""), lambda pkt: pkt.securityAccessType % 2 == 1
        ),
    ]

    def answers(self, other):
        return (
            isinstance(other, UDS_SA)
...
class UDS_ATP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrField("timingParameterRequestRecord", b""),
...
class UDS_ATPPR(Packet):
    ...
    fields_desc = [
        ByteEnumField(
            "timingParameterAccessType", 0, UDS_ATP.timingParameterAccessTypes
        ),
        ConditionalField(
            StrField("timingParameterResponseRecord", b""),
...
class UDS_LC(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ByteField("baudrateIdentifier", 0), lambda pkt: pkt.linkControlType == 0x1
        ),
        ConditionalField(
            ByteField("baudrateHighByte", 0), lambda pkt: pkt.linkControlType == 0x2
        ),
        ConditionalField(
            ByteField("baudrateMiddleByte", 0), lambda pkt: pkt.linkControlType == 0x2
        ),
        ConditionalField(
            ByteField("baudrateLowByte", 0), lambda pkt: pkt.linkControlType == 0x2
        ),
    ]
...
class UDS_LCPR(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, UDS_LC) and other.linkControlType == self.linkControlType
        )
...
class UDS_RDBI(Packet):
    ...
    fields_desc = [
        FieldListField(
            "identifiers", None, XShortEnumField("dataIdentifier", 0, dataIdentifiers)
        )
    ]
...
class UDS_RMBA(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XByteField("memoryAddress1", 0), lambda pkt: pkt.memoryAddressLen == 1
        ),
        ConditionalField(
            XShortField("memoryAddress2", 0), lambda pkt: pkt.memoryAddressLen == 2
        ),
        ConditionalField(
            X3BytesField("memoryAddress3", 0), lambda pkt: pkt.memoryAddressLen == 3
        ),
        ConditionalField(
            XIntField("memoryAddress4", 0), lambda pkt: pkt.memoryAddressLen == 4
        ),
        ConditionalField(
            XByteField("memorySize1", 0), lambda pkt: pkt.memorySizeLen == 1
        ),
        ConditionalField(
            XShortField("memorySize2", 0), lambda pkt: pkt.memorySizeLen == 2
        ),
        ConditionalField(
            X3BytesField("memorySize3", 0), lambda pkt: pkt.memorySizeLen == 3
        ),
        ConditionalField(
            XIntField("memorySize4", 0), lambda pkt: pkt.memorySizeLen == 4
        ),
    ]
...
class UDS_RSDBIPR(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, UDS_RSDBI) and other.dataIdentifier == self.dataIdentifier
        )
...
class UDS_WDBIPR(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, UDS_WDBI) and other.dataIdentifier == self.dataIdentifier
        )
...
class UDS_RDTCI(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ByteField("DTCSeverityMask", 0), lambda pkt: pkt.reportType in [0x07, 0x08]
        ),
        ConditionalField(
            XByteField("DTCStatusMask", 0),
            lambda pkt: pkt.reportType
            in [0x01, 0x02, 0x07, 0x08, 0x0F, 0x11, 0x12, 0x13],
        ),
        ConditionalField(
        ...
        ConditionalField(
            ByteField("DTCSnapshotRecordNumber", 0),
            lambda pkt: pkt.reportType in [0x3, 0x4, 0x5],
        ),
        ConditionalField(
            ByteField("DTCExtendedDataRecordNumber", 0),
...
class UDS_RDTCIPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XByteField("DTCStatusAvailabilityMask", 0),
            lambda pkt: pkt.reportType
            in [
                0x01,
                0x07,
        ...
        ConditionalField(
            ShortField("DTCCount", 0),
            lambda pkt: pkt.reportType in [0x01, 0x07, 0x11, 0x12],
        ),
        ConditionalField(
            StrField("DTCAndStatusRecord", b""),
            lambda pkt: pkt.reportType
            in [0x02, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x13, 0x15],
        ),
        ConditionalField(
            StrField("dataRecord", b""),
            lambda pkt: pkt.reportType
            in [0x03, 0x04, 0x05, 0x06, 0x08, 0x09, 0x10, 0x14],
        ),
    ]
...
class UDS_RFT(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField("filePathAndNameLength", 0, length_of="filePathAndName", fmt="H"),
        StrLenField(
            "filePathAndName", b"", length_from=lambda p: p.filePathAndNameLength
        ),
        ConditionalField(
            BitField("compressionMethod", 0, 4),
            lambda p: p.modeOfOperation not in [2, 5],
        ),
        ConditionalField(
            BitField("encryptingMethod", 0, 4),
            lambda p: p.modeOfOperation not in [2, 5],
        ),
        ConditionalField(
            FieldLenField(
                "fileSizeParameterLength", 0, fmt="B", length_of="fileSizeUnCompressed"
            ),
...
class UDS_RFTPR(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            FieldLenField(
                "lengthFormatIdentifier", 0, length_of="maxNumberOfBlockLength", fmt="B"
            ),
        ...
        ConditionalField(
            BitField("compressionMethod", 0, 4), lambda p: p.modeOfOperation != 0x02
        ),
        ConditionalField(
            BitField("encryptingMethod", 0, 4), lambda p: p.modeOfOperation != 0x02
        ),
        ConditionalField(
            FieldLenField(
...
class UDS_IOCBIPR(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, UDS_IOCBI) and other.dataIdentifier == self.dataIdentifier
        )
...
class UDS_NR(Packet):
    ...
    def answers(self, other):
        return self.requestServiceId == other.service and (
            ...
            or conf.contribs["UDS"]["treat-response-pending-as-answer"]
        )
...
def UDS_SessionEnumerator(sock, session_range=range(0x100), reset_wait=1.5):
    ...
    pkts = (
        ...
        for tup in product(
            ...
            UDS() / UDS_ER(resetType="hardReset"),
        )
        for req in tup
    )
    results, _ = sock.sr(
        pkts,
        timeout=len(session_range) * reset_wait * 2 + 1,
        verbose=False,
        inter=reset_wait,
    )
    return [
        ...
        if req is not None
        and req.service != 0x11
        and (res.service == 0x50 or res.negativeResponseCode not in [0x10, 0x11, 0x12])
    ]
...
def UDS_ServiceEnumerator(sock, session="DefaultSession", filter_responses=True):
    ...
    return [
        (session, p)
        for _, p in found_services[0]
        if p.service != 0x7F
        or (p.negativeResponseCode not in [0x10, 0x11] or not filter_responses)
    ]
...
def getTableEntry(tup):
    ...
    if pkt.service == 0x7F:
        return (
            session,
            "0x%02x: %s"
            % (pkt.requestServiceId, pkt.sprintf("%UDS_NR.requestServiceId%")),
        ...
        return (
            session,
            "0x%02x: %s"
            % (pkt.service & ~0x40, pkt.get_field("service").i2s[pkt.service & ~0x40]),
            "PositiveResponse",
        )
