...
class Enumerator(object):
    ...
    def dump(self, completed_only=True):
        ...
        data = [
            {
                ...
                "isotp_params": {
                    "resp_src": resp.src,
                    "resp_dst": resp.dst,
                    "resp_exsrc": resp.exsrc,
                    "resp_exdst": resp.exdst,
                }
                if resp is not None
                else None,
            }
            for s, req, resp in self.results
        ...
        return {
            ...
            "states_completed": [(str(k), v) for k, v in self.state_completed.items()],
            "data": data,
        }

    def remove_completed_states(self):
        selected_states = [k for k, v in self.state_completed.items() if not v]
        uncompleted_results = [r for r in self.results if r.state in selected_states]
    ...
    def update_stats(self):
        ...
        answertimes = [
            x.resp.time - x.req.sent_time
            for x in answered
            if x.resp.time is not None and x.req.sent_time is not None
        ]
    ...
    def show_negative_response_details(self, dump=False):
        raise NotImplementedError("This needs a protocol specific " "implementation")
    ...
    def show(self, dump=False, filtered=True, verbose=False):
        ...
        s += (
            "%d requests were sent, %d answered, %d unanswered"
            % (len(self.results), self.stats["answered"], self.stats["unanswered"])
            + "\n"
        )

        s += (
            "Times between request and response:\tMIN: %f\tMAX: %f\tAVG: %f"
            % (
                self.stats["answertime_min"],
                self.stats["answertime_max"],
        ...
        if verbose:
            completed = [(x, self.state_completed[x]) for x in self.scanned_states]
            s += (
                make_lined_table(
                    completed,
                    lambda tup: ("Scan state completed", tup[0], tup[1]),
                    dump=dump,
                )
                or ""
            )
        ...
        if response is None:
            ...
            if isinstance(positive_case, six.string_types):
                ...
                raise Scapy_Exception(
                    "Unsupported Type for positive_case. "
...
class Scanner(object):
    ...
    def __init__(self, socket, reset_handler=None, enumerators=None, **kwargs):
        ...
        if enumerators:
            ...
            self.enumerators = [
                e(self.socket) for e in enumerators if not isinstance(e, Enumerator)
            ] + [
    ...
    def dump(self, completed_only=True):
        return {
            "format_version": 0.1,
            "enumerators": [e.dump(completed_only) for e in self.enumerators],
    ...
    def get_state_paths(self):
        paths = [
            Graph.dijkstra(self.state_graph, EcuState(), s)
            for s in self.state_graph.nodes
            if s != EcuState()
        ]
        return sorted(
            [p for p in paths if p is not None] + [[EcuState()]], key=lambda x: x[-1]
        )
                ...
                for e in self.enumerators:
                    if e.state_completed[final_state]:
                        log_interactive.debug(
                            "[+] State %s for %s completed", repr(final_state), e
                        )
                        continue
                    ...
                    log_interactive.info(
                        "[i] EXECUTE SCAN %s for path %s", e.__class__.__name__, p
                    )
