...
class EcuState(object):
    ...
    def __contains__(self, item):
        ...
        try:
            return all(
                ov == sv or (hasattr(sv, "__iter__") and ov in sv)
                for sv, ov in zip(self.__dict__.values(), item.__dict__.values())
            )
    ...
    def __lt__(self, other):
        ...
        raise TypeError(
            "EcuStates should be identical. Something bad happen. "
    ...
    @staticmethod
    def get_modified_ecu_state(
        response, request, state, modify_in_place=False
    ):  # noqa: E501
...
class Ecu(object):
    ...
    def __init__(
        self, logging=True, verbose=True, store_supported_responses=True, lookahead=10
    ):
        ...
        for req, resp in answered:
            ...
            for sup_resp in self.__supported_responses:
                if resp == sup_resp.key_response:
                    if (
                        sup_resp.states is not None
                        and self.state not in sup_resp.states
                    ):
...
class EcuSession(DefaultSession):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ecu = Ecu(
            ...
            store_supported_responses=kwargs.pop("store_supported_responses", True),
        )  # noqa: E501
...
class EcuResponse:
    ...
    def __eq__(self, other):
        ...
        responses_equal = len(self.responses) == len(other.responses) and all(
            bytes(x) == bytes(y) for x, y in zip(self.responses, other.responses)
        )
        if self.__states is None:
            return responses_equal
        else:
            return (
                any(other.supports_state(s) for s in self.__states) and responses_equal
            )
...
class EcuAnsweringMachine(AnsweringMachine):
    ...
    def make_reply(self, req):
        ...
        return PacketList([self.__basecls(b"\x7f" + bytes(req)[0:1] + b"\x10")])
