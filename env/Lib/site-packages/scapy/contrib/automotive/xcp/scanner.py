...
class XCPOnCANScanner:
    ...
    def _scan(self, identifier, body, pid, answer_type):
        ...
        cto_request = (
            XCPOnCAN(identifier=identifier, flags=flags) / CTORequest(pid=pid) / body
        )

        req_and_res_list, _unanswered = self.__socket.sr(
            cto_request, timeout=self.__sniff_time, verbose=self.__verbose, multi=True
        )

        if len(req_and_res_list) == 0:
            self.log_verbose("No answer for identifier: " + str(identifier))
            return []

        valid_req_and_res_list = filter(
            lambda req_and_res: req_and_res[1].haslayer(answer_type), req_and_res_list
        )
    ...
    def _send_connect(self, identifier):
        ...
        xcp_req_and_res_list = self._scan(
            identifier, body, 0xFF, ConnectPositiveResponse
        )

        for req_and_res in xcp_req_and_res_list:
            result = XCPScannerResult(
                response_id=req_and_res[1].identifier, request_id=identifier
            )
            all_slaves.append(result)
            self.log_verbose(
                "Detected XCP slave for broadcast identifier: "
                + str(identifier)
                + "\nResponse: "
                + str(result)
            )
        ...
        if len(all_slaves) == 0:
            self.log_verbose("No XCP slave detected for identifier: " + str(identifier))
        return all_slaves
    ...
    def _send_get_slave_id(self, identifier):
        ...
        for req_and_res in xcp_req_and_res_list:
            ...
            if (
                response.position_1 != 0x58
                or response.position_2 != 0x43
                or response.position_3 != 0x50
            ):
                continue
            ...
            result = XCPScannerResult(
                request_id=request_id, response_id=response.identifier
            )
            all_slaves.append(result)
            self.log_verbose(
                "Detected XCP slave for broadcast identifier: "
    ...
    def scan_with_get_slave_id(self):
        ...
        GetSlaveId Message"""
        self.log_verbose(
            "Start scan with GetSlaveId id in range: " + str(self.id_range)
        )
    ...
    def scan_with_connect(self):
        # type: () -> List[XCPScannerResult]
        self.log_verbose("Start scan with CONNECT id in range: " + str(self.id_range))
        results = []
        for identifier in self.id_range:
