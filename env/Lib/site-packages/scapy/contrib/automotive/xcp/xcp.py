...
from scapy.contrib.automotive.xcp.utils import (
    ...
    identification_field_needs_alignment,
    get_daq_length,
...
class XCPOnTCP(TCP):
    ...
    def answers(self, other):
        if not isinstance(other, XCPOnTCP):
            return 0
        if isinstance(other.payload, CTORequest) and isinstance(
            self.payload, CTOResponse
        ):
...
class DTO(Packet):
    ...
    fields_desc = [
        ConditionalField(
            XByteField("fill", 0x00), lambda _: identification_field_needs_alignment()
        ),
        ConditionalField(
        ...
        ConditionalField(
            StrLenField("data", b"", length_from=lambda _: get_daq_data_field_length()),
