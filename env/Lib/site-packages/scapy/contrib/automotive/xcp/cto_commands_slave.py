...
from scapy.contrib.automotive.xcp.utils import (
    get_max_cto,
    get_ag,
    XCPEndiannessField,
    StrVarLenField,
...
class ConnectPositiveResponse(Packet):
    fields_desc = [
        FlagsField(
            "resource", 0, 8, ["cal_pag", "x1", "daq", "stim", "pgm", "x5", "x6", "x7"]
        ),
        FlagsField(
            "comm_mode_basic",
            0,
        ...
        ByteField("max_cto", 0),
        ConditionalField(
            ShortField("max_dto", 0), lambda p: p.comm_mode_basic.byte_order
        ),
        ConditionalField(
            LEShortField("max_dto_le", 0), lambda p: not p.comm_mode_basic.byte_order
        ),
    ...
    def post_dissection(self, pkt):
        if conf.contribs["XCP"]["allow_byte_order_change"]:
            ...
            if new_value != conf.contribs["XCP"]["byte_order"]:
                ...
                warning(
                    "Byte order changed to {0} because of received "
    ...
    def get_address_granularity(self):
        ...
        if (
            not comm_mode_basic.address_granularity_0
            and not comm_mode_basic.address_granularity_1
        ):
            return 1
        if (
            comm_mode_basic.address_granularity_0
            and not comm_mode_basic.address_granularity_1
        ):
            return 2
        if (
            not comm_mode_basic.address_granularity_0
            and comm_mode_basic.address_granularity_1
        ):
            return 4
        else:
            warning(
                "Getting address granularity from packet failed:" "both flags are 1"
            )
...
class SeedPositiveResponse(Packet):
    fields_desc = [
        ...
        StrVarLenField(
            "seed",
            b"",
            length_from=lambda p: p.seed_length,
...
class UploadPositiveResponse(Packet):
    fields_desc = [
        ConditionalField(
            StrLenField("alignment", b"", length_from=lambda pkt: get_ag() - 1),
            lambda _: get_ag() > 1,
        ),
...
class DAQProcessorInfoPositiveResponse(Packet):
    ...
    def write_identification_field_type_to_config(self):
        conf.contribs["XCP"]["identification_field_type_0"] = bool(
            self.daq_key_byte.identification_field_type_0
        )
        conf.contribs["XCP"]["identification_field_type_1"] = bool(
...
class DAQListInfoPositiveResponse(Packet):
    fields_desc = [
        FlagsField(
            ...
            0,
            8,
            ["predefined", "event_fixed", "daq", "stim", "x4", "x5", "x6", "x7"],
        ),
        ByteField("max_odt", 0),
...
class EvPacket(Packet):
    ...
    fields_desc = [
        ...
        StrLenField(
            "event_information_data", b"", max_length=lambda _: get_max_cto() - 2
        ),
    ]
...
class ServPacket(Packet):
    ...
    fields_desc = [
        ...
        StrLenField(
            "command_response_data", b"", max_length=lambda _: get_max_cto() - 2
        ),
    ]
