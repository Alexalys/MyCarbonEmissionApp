    ...
    def addfield(self, pkt, s, val):
        ...
        while val:
            if val > 127:
                ...
                data = b"".join(
                    chb(val | (0 if i == lastoffset else 128))
                    for i, val in enumerate(data)
                )
                return s + data
            if len(data) > 3:
                raise Scapy_Exception(
                    "%s: malformed length field" % self.__class__.__name__
                )
        # If val is None / 0
        return s + b"\x00"
...
class MQTTConnect(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            FieldLenField("wtoplen", None, length_of="willtopic"),
        ...
        ConditionalField(
            FieldLenField("wmsglen", None, length_of="willmsg"),
            lambda pkt: pkt.willflag == 1,
        ),
        ConditionalField(
        ...
        ConditionalField(
            FieldLenField("userlen", None, length_of="username"),
        ...
        ConditionalField(
            FieldLenField("passlen", None, length_of="password"),
...
class MQTTPublish(Packet):
    ...
    fields_desc = [
        ...
        StrLenField(
            "value", "", length_from=lambda pkt: (pkt.underlayer.len - pkt.length - 2)
        ),
    ]
