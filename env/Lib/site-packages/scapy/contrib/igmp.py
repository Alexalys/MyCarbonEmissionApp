...
class IGMP(Packet):
    ...
    def igmpize(self):
        ...
        gaddr = (
            self.gaddr if hasattr(self, "gaddr") and self.gaddr else "0.0.0.0"
        )  # noqa: E501
        ...
        if isinstance(underlayer, IP):
            if self.type == 0x11:
            ...
            elif self.type in [0x11, 0x22, 0x30, 0x31, 0x32] and isinstance(
                self, IGMPv3
            ):
                pass
            else:
                ...
                return False
            if not any(
                isinstance(x, IPOption_Router_Alert) for x in underlayer.options
            ):  # noqa: E501
