...
class GETATTR_Reply(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketField("attributes", Fattr3(), Fattr3), lambda pkt: pkt.status == 0
        ),
    ]
...
class LOOKUP_Reply(Packet):
    ...
    fields_desc = [
        ...
        IntField("af_dir", 0),
        ConditionalField(
            PacketField("dir_attributes", Fattr3(), Fattr3), lambda pkt: pkt.af_dir == 1
        ),
    ]
    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketListField(
                ...
                if pkt.value_follows == 1
                and (len(lst) == 0 or cur.value_follows == 1)
                and len(remain) > 4
                else None,
            ),
            lambda pkt: pkt.status == 0,
...
class CREATE_Call(Packet):
    ...
    fields_desc = [
        ...
        PacketField("filename", Object_Name(), Object_Name),
        IntEnumField(
            "create_mode", None, {0: "UNCHECKED", 1: "GUARDED", 2: "EXCLUSIVE"}
        ),
        ConditionalField(
            PacketField("attributes", Sattr3(), Sattr3),
...
class READ_Reply(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrLenField("fill", b"", length_from=lambda pkt: (4 - pkt.data_length) % 4),
            lambda pkt: pkt.status == 0,
        ),
    ]
