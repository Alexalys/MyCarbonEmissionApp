...
Battery_state = {
    0: "End-device connected to external source",
...
class RekeyInd(Packet):
    ...
    fields_desc = [
        PacketListField(
            "LoRaWANversion", b"", DevLoraWANversion, length_from=lambda pkt: 1
        )
    ]
...
class MACCommand_up(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketListField("LinkCheck", b"", LinkCheckReq, length_from=lambda pkt: 0),
            lambda pkt: (pkt.CID == 0x02),
        ),
        ConditionalField(
            PacketListField("LinkADR", b"", LinkADRReq, length_from=lambda pkt: 4),
            lambda pkt: (pkt.CID == 0x03),
        ),
        ConditionalField(
            PacketListField("DutyCycle", b"", DutyCycleReq, length_from=lambda pkt: 4),
            lambda pkt: (pkt.CID == 0x04),
        ),
        ConditionalField(
            PacketListField(
                "RXParamSetup", b"", RXParamSetupReq, length_from=lambda pkt: 4
            ),
            lambda pkt: (pkt.CID == 0x05),
        ),
        ConditionalField(
            PacketListField("DevStatus", b"", DevStatusReq, length_from=lambda pkt: 2),
            lambda pkt: (pkt.CID == 0x06),
        ),
        ConditionalField(
            PacketListField(
                "NewChannel", b"", NewChannelReq, length_from=lambda pkt: 5
            ),
            lambda pkt: (pkt.CID == 0x07),
        ),
        ConditionalField(
            PacketListField(
                "RXTimingSetup", b"", RXTimingSetupReq, length_from=lambda pkt: 1
            ),
        ...
        ConditionalField(
            PacketListField(
                "TxParamSetup", b"", TxParamSetupReq, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x09),
        ),
        ConditionalField(
            PacketListField("DlChannel", b"", DlChannelReq, length_from=lambda pkt: 4),
        ...
        ConditionalField(
            PacketListField(
                "ADRParamSetup", b"", ADRParamSetupReq, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x0C),
        ),
        ConditionalField(
            PacketListField(
                "DeviceTime", b"", DeviceTimeReq, length_from=lambda pkt: 0
            ),
            lambda pkt: (pkt.CID == 0x0D),
        ),
        ConditionalField(
            PacketListField(
                "ForceRejoin", b"", ForceRejoinReq, length_from=lambda pkt: 2
            ),
            lambda pkt: (pkt.CID == 0x0E),
        ),
        ConditionalField(
            PacketListField(
                "RejoinParamSetup", b"", RejoinParamSetupReq, length_from=lambda pkt: 1
            ),
...
class MACCommand_down(Packet):
    ...
    fields_desc = [
        ByteEnumField("CID", 0, CIDs_up),
        ConditionalField(
            PacketListField("Reset", b"", ResetConf, length_from=lambda pkt: 1),
            lambda pkt: (pkt.CID == 0x01),
        ),
        ConditionalField(
            PacketListField("LinkCheck", b"", LinkCheckAns, length_from=lambda pkt: 2),
            lambda pkt: (pkt.CID == 0x02),
        ),
        ConditionalField(
            PacketListField("LinkADR", b"", LinkADRAns, length_from=lambda pkt: 0),
            lambda pkt: (pkt.CID == 0x03),
        ),
        ConditionalField(
            PacketListField("DutyCycle", b"", DutyCycleAns, length_from=lambda pkt: 4),
            lambda pkt: (pkt.CID == 0x04),
        ),
        ConditionalField(
            PacketListField(
                "RXParamSetup", b"", RXParamSetupAns, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x05),
        ),
        ConditionalField(
            PacketListField(
                "DevStatusAns", b"", RXParamSetupAns, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x06),
        ),
        ConditionalField(
            PacketListField(
                "NewChannel", b"", NewChannelAns, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x07),
        ),
        ConditionalField(
            PacketListField(
                "RXTimingSetup", b"", RXTimingSetupAns, length_from=lambda pkt: 0
            ),
            lambda pkt: (pkt.CID == 0x08),
        ),
        ConditionalField(
            PacketListField(
                "TxParamSetup", b"", TxParamSetupAns, length_from=lambda pkt: 0
            ),
            lambda pkt: (pkt.CID == 0x09),
        ),
        ConditionalField(
            PacketListField("DlChannel", b"", DlChannelAns, length_from=lambda pkt: 1),
            lambda pkt: (pkt.CID == 0x0A),
        ),
        ConditionalField(
            PacketListField("Rekey", b"", RekeyConf, length_from=lambda pkt: 1),
            lambda pkt: (pkt.CID == 0x0B),
        ),
        ConditionalField(
            PacketListField(
                "ADRParamSetup", b"", ADRParamSetupAns, length_from=lambda pkt: 0
            ),
            lambda pkt: (pkt.CID == 0x0C),
        ),
        ConditionalField(
            PacketListField(
                "DeviceTime", b"", DeviceTimeAns, length_from=lambda pkt: 5
            ),
            lambda pkt: (pkt.CID == 0x0D),
        ),
        ConditionalField(
            PacketListField(
                "RejoinParamSetup", b"", RejoinParamSetupAns, length_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.CID == 0x0F),
        ),
    ]
...
class FHDR(Packet):
    ...
    fields_desc = [
        ConditionalField(
            PacketListField(
                "DevAddr", b"", DevAddrElem, length_from=lambda pkt: 4  # noqa: E501
            ),
            lambda pkt: (pkt.MType >= 0b010 and pkt.MType <= 0b101),
        ),
        ConditionalField(
            PacketListField("FCtrl", b"", FCtrl_Link, length_from=lambda pkt: 1),
            lambda pkt: (
                (pkt.MType & 0b1 == 1 and pkt.MType <= 0b101 and (pkt.MType & 0b10 > 0))
                or (pkt.MType & 0b1 == 0 and pkt.MType >= 0b010)
            ),
        ),
        ConditionalField(
            LEShortField("FCnt", 0),
...
class Join_Request(Packet):
    name = "Join_Request"
    fields_desc = [
        StrFixedLenField("AppEUI", b"\x00" * 8, 8),
...
datapayload_list = [
    (StrField("DataPayload", "", remain=4), lambda pkt: (dpload_type(pkt) == 0)),
    (StrField("DataPayload", "", remain=6), lambda pkt: (dpload_type(pkt) == 1)),
]
...
class FRMPayload(Packet):
    ...
    fields_desc = [
        ConditionalField(
            MultipleTypeField(datapayload_list, StrField("DataPayload", "", remain=4)),
            lambda pkt: (dpload_type(pkt) is not None),
        ),
        ConditionalField(
            PacketListField(
                "Join_Request_Field", b"", Join_Request, length_from=lambda pkt: 18
            ),
            lambda pkt: (pkt.MType == 0b000),
        ),
        ConditionalField(
            PacketListField(
                "Join_Accept_Field", b"", Join_Accept, count_from=lambda pkt: 1
            ),
            lambda pkt: (pkt.MType == 0b001 and LoRa.encrypted is False),
        ),
        ConditionalField(
            StrField("Join_Accept_Encrypted", 0),
...
class MACPayload(Packet):
    ...
    fields_desc = [
        FHDR,
        ConditionalField(
            ByteEnumField("FPort", 0, FPorts),
            lambda pkt: (
                pkt.MType >= 0b010 and pkt.MType <= 0b101 and pkt.FCtrl[0].FOptsLen == 0
            ),
        ),
        FRMPayload,
    ]
...
class MHDR(Packet):  # Same for 1.0 as for 1.1
    ...
    fields_desc = [
        BitEnumField("MType", 0b000, 3, MTypes),
...
class PHYPayload(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XIntField("MIC", 0),
            lambda pkt: (pkt.MType != 0b001 or LoRa.encrypted is False),
        ),
    ]
...
class LoRa(
    ...
    fields_desc = [
        ...
        PHYPayload,
        ConditionalField(XShortField("CRC", 0), lambda pkt: (pkt.MType & 0b1 == 0)),
    ]
