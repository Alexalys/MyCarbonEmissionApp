...
class LenStringPacket(Packet):
    ...
    fields_desc = [
        ...
        MultipleTypeField(
            [(StrFixedLenField("data", "", length=2), lambda pkt: not pkt.length)],
...
class ResultElement(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ShortEnumField("reason", 0, _defReason),
...
class ResultList(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "resultList", None, ResultElement, count_from=lambda pkt: pkt.nbResult
        ),
    ]
...
class ContextElement(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "transferSyntax", None, SyntaxId, count_from=lambda pkt: pkt.nbTransferSyn
        ),
    ]
...
class NTLMSSP(Packet):
    ...
    fields_desc = [
        ...
        StrLenField("domainName", "", length_from=lambda pkt: pkt.domainNameLen),
        StrLenField("userName", "", length_from=lambda pkt: pkt.userNameLen),
        StrLenField("workstation", "", length_from=lambda pkt: pkt.workstationLen),
        StrLenField(
...
def _opc_stubdata_length(pkt):
    ...
    if (
        OpcDaHeaderMessage in pkt.firstlayer()
        and pkt.firstlayer()[OpcDaHeaderMessage].pfc_flags & "objectUuid"
    ):
        stub_data_length -= 36
...
class OpcDaRequest(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ...
            lambda pkt: OpcDaHeaderMessage in pkt.firstlayer()
            and pkt.firstlayer()[OpcDaHeaderMessage].pfc_flags & "objectUuid",
        ),
        PacketLenField(
            "stubData",
...
class OpcDaHeaderMessage(Packet):
    ...
    fields_desc = [
        ...
        # DCE 1.1 RPC - 14.1
        BitEnumField(
            "integerRepresentation", 1, 4, {0: "bigEndian", 1: "littleEndian"}
        ),
        BitEnumField("characterRepresentation", 0, 4, {0: "ascii", 1: "ebcdic"}),
        ByteEnumField(
            "floatingPointRepresentation", 0, {0: "ieee", 1: "vax", 2: "cray", 3: "ibm"}
        ),
        ShortField("res", 0),
    ]
