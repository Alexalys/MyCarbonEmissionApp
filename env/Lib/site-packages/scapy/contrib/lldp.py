...
class LLDPDU(Packet):
    ...
    def post_build(self, pkt, pay):
        ...
        if under_layer is None:
            if conf.contribs["LLDP"].strict_mode():
                raise LLDPMissingLowerLayer(
                    "No lower layer (Ethernet " "or Dot1Q) provided."
                )
            else:
                return pkt + pay
        ...
        if frame_size < LLDPDU.ETHER_FRAME_MIN_LEN:
            return (
                pkt + pay + b"\x00" * (LLDPDU.ETHER_FRAME_MIN_LEN - frame_size)
            )  # noqa: E501
        return pkt + pay
    ...
    def _frame_structure_check(structure_description):
        ...
        standard_frame_structure = [
            LLDPDUChassisID.__name__,
        ...
        if len(structure_description) < 3:
            raise LLDPInvalidFrameStructure(
                ...
                "{}".format(
                    " ".join(structure_description), " ".join(standard_frame_structure)
                )
            )
    ...
    def _tlv_multiplicities_check(tlv_type_count):
        ...
        for tlv_type_name in standard_multiplicities:
            ...
            try:
                ...
                raise LLDPInvalidTLVCount(
                    "Missing TLV layer of type " "{}.".format(tlv_type_name)
                )
...
class LLDPDUChassisID(LLDPDU):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteField("family", 0), lambda pkt: pkt.subtype == 0x05),
        MultipleTypeField(
            [
                (MACField("id", None), lambda pkt: pkt.subtype == 0x04),
                (IPField("id", None), lambda pkt: pkt.subtype == 0x05),
            ],
            StrLenField(
                "id",
                "",
                length_from=lambda pkt: 0 if pkt._length is None else pkt._length - 1,
            ),
        ),
    ]
...
class LLDPDUPortID(LLDPDU):
    ...
    fields_desc = [
        ...
        ConditionalField(ByteField("family", 0), lambda pkt: pkt.subtype == 0x04),
        MultipleTypeField(
            [
    ...
    def _check(self):
        ...
        if conf.contribs["LLDP"].strict_mode() and self._length != 2:
            raise LLDPInvalidLengthField(
                "length must be 2 - got " "{}".format(self._length)
            )
...
class LLDPDUEndOfLLDPDU(LLDPDU):
    ...
    def _check(self):
        ...
        if conf.contribs["LLDP"].strict_mode() and self._length != 0:
            raise LLDPInvalidLengthField(
                "length must be 0 - got " "{}".format(self._length)
            )
...
class LLDPDUSystemCapabilities(LLDPDU):
    ...
    def _check(self):
        ...
        if conf.contribs["LLDP"].strict_mode() and self._length != 4:
            raise LLDPInvalidLengthField(
                "length must be 4 - got " "{}".format(self._length)
            )
...
class LLDPDUManagementAddress(LLDPDU):
    ...
    fields_desc = [
        ...
        BitFieldLenField(
            ...
            adjust=lambda pkt, x: 8 + len(pkt.management_address) + len(pkt.object_id),
        ),
        BitFieldLenField(
        ...
        ByteEnumField("management_address_subtype", 0x00, IANA_ADDRESS_FAMILY_NUMBERS),
        XStrLenField(
            "management_address",
            "",
            length_from=lambda pkt: 0
            if pkt._management_address_string_length is None
            else pkt._management_address_string_length - 1,
        ),
        ByteEnumField(
...
class LLDPDUGenericOrganisationSpecific(LLDPDU):
    ...
    fields_desc = [
        ...
        XStrLenField(
            "data",
            "",
            length_from=lambda pkt: 0 if pkt._length is None else pkt._length - 4,
        ),
    ]
