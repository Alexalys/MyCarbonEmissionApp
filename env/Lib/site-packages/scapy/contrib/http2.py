...
class HPackMagicBitField(fields.BitField):
    ...
    def getfield(self, pkt, s):
        ...
        assert (
            isinstance(r, tuple) and len(r) == 2 and isinstance(r[1], six.integer_types)
        ), "Second element of BitField.getfield return value expected to be an int or a long; API change detected"  # noqa: E501
        assert (
            r[1] == self._magic
        ), "Invalid value parsed from s; error in class guessing detected!"  # noqa: E501
        return r
    ...
    def m2i(self, pkt, x):
        ...
        assert (
            r == self._magic
        ), "Invalid value parsed from m2i; error in class guessing detected!"  # noqa: E501
        return r
...
class AbstractUVarIntField(fields.Field):
    ...
    def __init__(self, name, default, size):
        ...
        :raises: AssertionError
        """
        assert default is None or (
            isinstance(default, six.integer_types) and default >= 0
        )  # noqa: E501
        assert 0 < size <= 8
    ...
    def _parse_multi_byte(self, s):
        ...
        while byte & 0x80:
            ...
            assert (
                i < tmp_len
            ), "EINVAL: x: out-of-bound read: the string ends before the AbstractUVarIntField!"  # noqa: E501
            byte = orb(s[i])
        value += byte << (7 * (i - 1))
    ...
    def m2i(self, pkt, x):
        ...
        if isinstance(x, tuple):
            assert (
                8 - x[1]
            ) == self.size, "EINVAL: x: not enough bits remaining in current byte to read the prefix"  # noqa: E501
            val = x[0]
        else:
            assert (
                isinstance(x, bytes) and self.size == 8
            ), "EINVAL: x: tuple expected when prefix_len is not a full byte"  # noqa: E501
            val = x
    ...
    def addfield(self, pkt, s, val):
        ...
        if isinstance(s, bytes):
            assert (
                self.size == 8
            ), "EINVAL: s: tuple expected when prefix_len is not a full byte"  # noqa: E501
    ...
    def _detect_bytelen_from_str(s):
        ...
        while orb(s[i]) & 0x80 > 0:
            ...
            assert (
                i < tmp_len
            ), "EINVAL: s: out-of-bound read: unfinished AbstractUVarIntField detected"  # noqa: E501
        ret = i + 1
    ...
    def getfield(self, pkt, s):
        ...
        if isinstance(s, tuple):
            ...
            assert (
                8 - ti == self.size
            ), "EINVAL: s: not enough bits remaining in current byte to read the prefix"  # noqa: E501
            val = ts
        else:
            assert (
                isinstance(s, bytes) and self.size == 8
            ), "EINVAL: s: tuple expected when prefix_len is not a full byte"  # noqa: E501
            val = s
...
class HPackStrLenField(fields.Field):
    ...
    def m2i(self, pkt, x):
        ...
        assert (
            t is not None and tmp_len is not None
        ), "Conversion from string impossible: no type or length specified"  # noqa: E501
...
class HPackHdrString(packet.Packet):
    ...
    def self_build(self, **kwargs):
        ...
        if self.getfieldval("type") is None:
            self.type = (
                1 if isinstance(self.getfieldval("data"), HPackZString) else 0
            )  # noqa: E501
...
class H2PaddedDataFrame(H2DataFrame):
    ...
    fields_desc = [
        ...
        fields.StrLenField(
            "padding", "", length_from=lambda pkt: pkt.getfieldval("padlen")
        ),
    ]
...
class H2PaddedHeadersFrame(H2AbstractHeadersFrame):
    ...
    fields_desc = [
        fields.FieldLenField("padlen", None, length_of="padding", fmt="B"),
        fields.PacketListField(
            "hdrs", [], HPackHeaders, length_from=lambda pkt: pkt.get_hdrs_len()
        ),
        fields.StrLenField(
            "padding", "", length_from=lambda pkt: pkt.getfieldval("padlen")
        ),
...
class H2PaddedPriorityHeadersFrame(H2AbstractHeadersFrame):
    ...
    def get_hdrs_len(self):
        ...
        ret = int(
            self.s_len - padding_len_len - padding_len - (bit_cnt / 8) - weight_len
        )
        assert ret >= 0
        return ret
...
class H2SettingsFrame(H2FramePayload):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert len(args) == 0 or (
            isinstance(args[0], bytes) and len(args[0]) % 6 == 0
...
class H2PingFrame(H2FramePayload):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # RFC7540 par6.7 p42
        assert len(args) == 0 or (
            isinstance(args[0], (bytes, str)) and len(args[0]) == 8
...
class H2WindowUpdateFrame(H2FramePayload):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # RFC7540 par6.9 p46
        assert len(args) == 0 or (
            isinstance(args[0], (bytes, str)) and len(args[0]) == 4
...
class H2Frame(packet.Packet):
    ...
    def guess_payload_class(self, payload):
        ...
        if t == H2DataFrame.type_id:
            if H2DataFrame.flags[H2DataFrame.PADDED_FLAG].short in self.getfieldval(
                "flags"
            ):  # noqa: E501
        ...
        if t == H2HeadersFrame.type_id:
            if H2HeadersFrame.flags[
                H2HeadersFrame.PADDED_FLAG
            ].short in self.getfieldval(
                "flags"
            ):  # noqa: E501
                if H2HeadersFrame.flags[
                    H2HeadersFrame.PRIORITY_FLAG
                ].short in self.getfieldval(
                    "flags"
                ):  # noqa: E501
        ...
        if t == H2PushPromiseFrame.type_id:
            if H2PushPromiseFrame.flags[
                H2PushPromiseFrame.PADDED_FLAG
            ].short in self.getfieldval(
                "flags"
            ):  # noqa: E501
    ...
    def extract_padding(self, s):
        ...
        """
        assert (
            isinstance(self.len, six.integer_types) and self.len >= 0
        ), "Invalid length: negative len?"  # noqa: E501
        assert (
            len(s) >= self.len
        ), "Invalid length: string too short for this length"  # noqa: E501
...
packet.bind_layers(
    H2Frame, H2PaddedHeadersFrame, {"type": H2HeadersFrame.type_id}
)  # noqa: E501
packet.bind_layers(
    H2Frame, H2PriorityHeadersFrame, {"type": H2HeadersFrame.type_id}
)  # noqa: E501
packet.bind_layers(
    H2Frame, H2PaddedPriorityHeadersFrame, {"type": H2HeadersFrame.type_id}
)  # noqa: E501
...
packet.bind_layers(
    H2Frame, H2PushPromiseFrame, {"type": H2PushPromiseFrame.type_id}
)  # noqa: E501
packet.bind_layers(
    H2Frame, H2PaddedPushPromiseFrame, {"type": H2PaddedPushPromiseFrame.type_id}
)  # noqa: E501
packet.bind_layers(H2Frame, H2GoAwayFrame, {"type": H2GoAwayFrame.type_id})
packet.bind_layers(
    H2Frame, H2WindowUpdateFrame, {"type": H2WindowUpdateFrame.type_id}
)  # noqa: E501
packet.bind_layers(
    H2Frame, H2ContinuationFrame, {"type": H2ContinuationFrame.type_id}
)  # noqa: E501


#                                          HTTP/2 Connection Preface                                                   #  # noqa: E501
# From RFC 7540 par3.5
H2_CLIENT_CONNECTION_PREFACE = hex_bytes(
...
class HPackHdrTable(Sized):
    ...
    def __init__(
        self, dynamic_table_max_size=4096, dynamic_table_cap_size=4096
    ):  # noqa: E501
        # type: (int, int) -> None
        """
        ...
        assert (
            dynamic_table_max_size <= dynamic_table_cap_size
    ...
    def resize(self, ns):
        ...
        """
        assert (
            0 <= ns <= self._dynamic_table_cap_size
    ...
    def _reduce_dynamic_table(self, new_entry_size=0):
        ...
        while (
            dyn_tbl_sz > 0 and cur_sz + new_entry_size > self._dynamic_table_max_size
        ):  # noqa: E501
    ...
    def register(self, hdrs):
        ...
        if isinstance(hdrs, H2Frame):
            hdrs = [
                hdr
                for hdr in hdrs.payload.hdrs
                if isinstance(hdr, HPackLitHdrFldWithIncrIndexing)
            ]  # noqa: E501
        elif isinstance(hdrs, HPackLitHdrFldWithIncrIndexing):
            hdrs = [hdrs]
        else:
            hdrs = [
                hdr for hdr in hdrs if isinstance(hdr, HPackLitHdrFldWithIncrIndexing)
            ]  # noqa: E501
        ...
        for hdr in hdrs:
            try:
                if isinstance(hdr, HPackIndexedHdr):
                    ...
                    hdr, (HPackLitHdrFldWithIncrIndexing, HPackLitHdrFldWithoutIndexing)
                ):
                    if hdr.index != 0:
                    ...
                    if name.startswith(":"):
                        lst.append(
                            "{} {}".format(
                                name, hdr.hdr_value.getfieldval("data").origin()
                            )
                        )
                    else:
                        lst.append(
    ...
    def _convert_a_header_to_a_h2_header(
        self, hdr_name, hdr_value, is_sensitive, should_index
    ):  # noqa: E501
        ...
        if idx is not None:
            if is_sensitive(hdr_name, _hdr_value.getfieldval("data").origin()):
                return HPackLitHdrFldWithoutIndexing(
                    never_index=1, index=idx, hdr_value=_hdr_value
                ), len(
                    HPackHdrEntry(
                        self[idx].name(), _hdr_value.getfieldval("data").origin()
                    )
                )
            if should_index(hdr_name):
                return HPackLitHdrFldWithIncrIndexing(
                    index=idx, hdr_value=_hdr_value
                ), len(
                    HPackHdrEntry(
                        self[idx].name(), _hdr_value.getfieldval("data").origin()
                    )
                )
            return HPackLitHdrFldWithoutIndexing(index=idx, hdr_value=_hdr_value), len(
                HPackHdrEntry(self[idx].name(), _hdr_value.getfieldval("data").origin())
            )
        ...
        if is_sensitive(
            ...
            return HPackLitHdrFldWithoutIndexing(
                never_index=1, index=0, hdr_name=_hdr_name, hdr_value=_hdr_value
            ), len(
                HPackHdrEntry(
                    _hdr_name.getfieldval("data").origin(),
                    _hdr_value.getfieldval("data").origin(),
        ...
        if should_index(_hdr_name.getfieldval("data").origin()):
            return HPackLitHdrFldWithIncrIndexing(
                index=0, hdr_name=_hdr_name, hdr_value=_hdr_value
            ), len(
                HPackHdrEntry(
                    _hdr_name.getfieldval("data").origin(),
                    _hdr_value.getfieldval("data").origin(),
    ...
    def parse_txt_hdrs(
        ...
        cur_frm = (
            H2HeadersFrame()
        )  # type: Union[H2HeadersFrame, H2ContinuationFrame]  # noqa: E501
        cur_hdr_sz = 0
        ...
        for hdr_line in sio:
            ...
            if register and isinstance(
                new_hdr, HPackLitHdrFldWithIncrIndexing
            ):  # noqa: E501
            ...
            if max_frm_sz < len(raw(cur_frm)) + base_frm_len + new_hdr_len or (
                max_hdr_lst_sz != 0 and max_hdr_lst_sz < cur_hdr_sz + new_hdr_len
            ):
                flags = set()
        ...
        if body:
            ...
            while frgmt:
                ...
                ret.frames.append(
                    ...
                    / H2DataFrame(data=frgmt)  # noqa: E501
                )
                frgmt = nxt_frgmt
        return ret
