...
class ISOTP(Packet):
    ...
    def fragment(self):
        ...
        if len(self.data) <= data_bytes_in_frame:
            ...
            if self.dst is None or self.dst <= 0x7FF:
                ...
                pkt = CAN(identifier=self.dst, flags="extended", data=frame_data)
            return [pkt]
        ...
        if self.dst is None or self.dst <= 0x7FF:
            ...
            frame = CAN(
                identifier=self.dst, flags="extended", data=frame_header + frame_data
            )
    ...
    def defragment(can_frames, use_extended_addressing=None):
        ...
        while parser.count > 0:
            p = parser.pop()
            if (
                (use_extended_addressing is True and p.exdst is not None)
...
class ISOTP_FF(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            BitField("extended_message_size", 0, 32), lambda pkt: pkt.message_size == 0
        ),
        StrField("data", "", fmt="B"),
    ]
    ...
    * RX background thread can be stopped by the garbage collector
    ...
    def select(sockets, remain=None):
        ...
        def find_ready_sockets():
            return list(
                filter(
                    lambda x: (x.ins is not None)
                    and (x.ins.rx_queue is not None)
                    and (not x.ins.rx_queue.empty()),
                    sockets,
                )
            )
...
class TimeoutScheduler:
    ...
    def schedule(timeout, callback):
        ...
        with TimeoutScheduler._mutex:
            ...
            if TimeoutScheduler._thread is None:
                t = Thread(target=TimeoutScheduler._task, name="TimeoutScheduler._task")
    ...
    def _wait(handle):
        ...
        if to_wait > 0:
            log_runtime.debug(
                "TimeoutScheduler Thread going to sleep @ %f " + "for %fs", now, to_wait
            )
            interrupted = TimeoutScheduler._event.wait(to_wait)
            new = TimeoutScheduler._time()
            log_runtime.debug(
                "TimeoutScheduler Thread awake @ %f, slept for" + " %f, interrupted=%d",
                new,
                new - now,
                interrupted,
            )
    ...
    def _task():
        ...
        log_runtime.debug(
            "TimeoutScheduler Thread spawning @ %f", TimeoutScheduler._time()
        )
        ...
        try:
            ...
            log_runtime.debug(
                "TimeoutScheduler Thread dying @ %f", TimeoutScheduler._time()
            )
    ...
    def can_send(self, load):
        ...
        if self.src_id is None or self.src_id <= 0x7FF:
            ...
            self.can_socket.send(
                CAN(identifier=self.src_id, flags="extended", data=load)
            )
...
if six.PY3 and LINUX:
    ...
    class ISOTPNativeSocket(SuperSocket):
        ...
        def __build_can_isotp_fc_options(
            self,
        ...
        def __build_can_isotp_ll_options(
            self,
        ...
        def __bind_socket(self, sock, iface, sid, did):
            ...
            addr = SOCKADDR_CAN(
                ctypes.c_uint16(socket.PF_CAN),
                ifr.ifr_ifindex,
                ADDR_INFO(TP(ctypes.c_uint32(did), ctypes.c_uint32(sid))),
            )
        ...
        def __set_option_flags(
            ...
            if padding:
                option_flags = (
                    option_flags | CAN_ISOTP_TX_PADDING | CAN_ISOTP_RX_PADDING
                )
        ...
        def __init__(
            ...
            if not isinstance(iface, six.string_types):
                if hasattr(iface, "ins") and hasattr(iface.ins, "getsockname"):
                    ...
                    raise Scapy_Exception(
                        "Provide a string or a CANSocket " "object as iface parameter"
                    )
            ...
            self.can_socket.setsockopt(
                SOL_CAN_ISOTP, CAN_ISOTP_RECV_FC, self.__build_can_isotp_fc_options()
            )
            self.can_socket.setsockopt(
                SOL_CAN_ISOTP, CAN_ISOTP_LL_OPTS, self.__build_can_isotp_ll_options()
            )
...
def filter_periodic_packets(packet_dict, verbose=False):
    ...
    for idn in filter_dict:
        ...
        if all(abs(t1 - t2) < 0.001 for t1, t2 in zip(tg[1:], tg[:-1])):
            if verbose:
                print("[i] Identifier 0x%03x seems to be periodic. " "Filtered.")
            for k in key_lst:
                del packet_dict[k]


def get_isotp_fc(
    id_value, id_list, noise_ids, extended, packet, verbose=False
):  # noqa: E501
    ...
    try:
        ...
        if isotp_pci == 3 and 0 <= isotp_fc <= 2:
            if verbose:
                print(
                    "[+] Found flow-control frame from identifier 0x%03x"
                    " when testing identifier 0x%03x" % (packet.identifier, id_value)
                )
        ...
        print("[!] Unknown message Exception: %s on packet: %s" % (e, repr(packet)))
...
def scan(
    ...
    for value in scan_range:
        ...
        sock.sniff(
            prn=lambda pkt: get_isotp_fc(
                value, return_values, noise_ids, False, pkt, verbose
            ),
            timeout=sniff_time,
    ...
    for tested_id in return_values.keys():
        for value in range(max(0, tested_id - 2), tested_id + 2, 1):
            sock.sniff(
                prn=lambda pkt: get_isotp_fc(
                    value, cleaned_ret_val, noise_ids, False, pkt, verbose
                ),
...
def scan_extended(
    ...
    for value in scan_range:
        ...
        for ext_isotp_id in range(r[0], r[-1], scan_block_size):
            sock.sniff(
                prn=lambda p: get_isotp_fc(
                    ext_isotp_id, id_list, noise_ids, True, p, verbose
                ),
        ...
        for ext_isotp_id in id_list:
            for ext_id in range(
                max(ext_isotp_id - 2, 0), min(ext_isotp_id + scan_block_size + 2, 256)
            ):
                pkt.extended_address = ext_id
                full_id = (value << 8) + ext_id
                sock.sniff(
                    prn=lambda pkt: get_isotp_fc(
                        full_id, return_values, noise_ids, True, pkt, verbose
                    ),
...
def ISOTPScan(
    ...
    background_pkts = sock.sniff(
        timeout=noise_listen_time, started_callback=lambda: sock.send(dummy_pkt)
    )

    noise_ids = list(set(pkt.identifier for pkt in background_pkts))

    if extended_addressing:
        found_packets = scan_extended(
            sock,
            scan_range,
    ...
    return generate_isotp_list(found_packets, can_interface, extended_addressing)
...
def generate_code_output(found_packets, can_interface, extended_addressing=False):
    ...
    for pack in found_packets:
        if extended_addressing:
            ...
            result += (
                "ISOTPSocket(%s, sid=0x%x, did=0x%x, padding=%s, "
