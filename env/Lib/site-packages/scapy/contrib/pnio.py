...
class PNIORealTimeCyclicPDU(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "data",
            [],
            next_cls_cb=lambda pkt, lst, p, remain: pkt.next_cls_cb(lst, p, remain),
        ),
    ...
    def get_padding_length(self):
        if hasattr(self, "_len"):
            pad_len = (
                ...
                - 2
                - 1  # Cycle Counter size (ShortField)
                - 1  # DataStatus size (FlagsField over 8 bits)  # TransferStatus (ByteField)
            )
        else:
    ...
    def next_cls_cb(self, _lst, _p, _remain):
        ...
        if not (ether_layer is None and pnio_layer is None):
            ...
            layout = type(self).get_layout_from_config(
                ether_layer.src, ether_layer.dst, pnio_layer.frameID
            )
