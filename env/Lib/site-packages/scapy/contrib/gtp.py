...
class GTPHeader(Packet):
    ...
    def answers(self, other):
        return (
            isinstance(other, GTPHeader)
            and self.version == other.version
            and (not self.seq or self.seq == other.seq)
            and self.payload.answers(other.payload)
        )
...
class GTPPDUSessionContainer(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField(
            ...
            0,
            4,
            {0: "DL PDU SESSION INFORMATION", 1: "UL PDU SESSION INFORMATION"},
        ),
        BitField("QMP", 0, 1),
        # UL (type 1)
        ...
        # DL (type 0)
        ConditionalField(
            XBitField("PPI", 0, 3), lambda pkt: pkt.type == 0 and pkt.PPP == 1
        ),
        ConditionalField(
            XBitField("spareDl2", 0, 5), lambda pkt: pkt.type == 0 and pkt.PPP == 1
        ),
        ConditionalField(
            XBitField("dlSendTime", 0, 64), lambda pkt: pkt.type == 0 and pkt.QMP == 1
        ),
        ConditionalField(
            X3BytesField("dlQFISeqNum", 0), lambda pkt: pkt.type == 0 and pkt.SNP == 1
        ),
        # UL (type 1)
        ConditionalField(
            XBitField("dlSendTimeRpt", 0, 64),
            lambda pkt: pkt.type == 1 and pkt.QMP == 1,
        ),
        ConditionalField(
            XBitField("dlRecvTime", 0, 64), lambda pkt: pkt.type == 1 and pkt.QMP == 1
        ),
        ConditionalField(
            XBitField("ulSendTime", 0, 64), lambda pkt: pkt.type == 1 and pkt.QMP == 1
        ),
        ConditionalField(
            XBitField("dlDelayRslt", 0, 32),
            lambda pkt: pkt.type == 1 and pkt.dlDelayInd == 1,
        ),
        ConditionalField(
            XBitField("ulDelayRslt", 0, 32),
            lambda pkt: pkt.type == 1 and pkt.ulDelayInd == 1,
        ),
        ConditionalField(
            XBitField("UlQFISeqNum", 0, 24), lambda pkt: pkt.type == 1 and pkt.SNP == 1
        ),
        ConditionalField(
            XBitField("N3N9DelayRslt", 0, 32),
            lambda pkt: pkt.type == 1 and pkt.N3N9DelayInd == 1,
        ),
        # Common
...
class IE_SelectionMode(IE_Base):
    ...
    name = "Selection Mode"
    fields_desc = [
        ByteEnumField("ietype", 15, IEType),
...
class IE_Teardown(IE_Base):
    ...
    fields_desc = [
        ByteEnumField("ietype", 19, IEType),
    ...
    def m2i(self, pkt, s):
        ...
        while tmp_s:
            ...
            if tmp_len > len(tmp_s):
                warning(
                    "APN prematured end of character-string (size=%i, remaining bytes=%i)"
                    % (tmp_len, len(tmp_s))
                )  # noqa: E501
...
class IE_AccessPointName(IE_Base):
    ...
    fields_desc = [
        ByteEnumField("ietype", 131, IEType),
        ShortField("length", None),
...
class IE_GSNAddress(IE_Base):
    ...
    fields_desc = [
        ...
        ShortField("length", None),
        ConditionalField(
            IPField("ipv4_address", RandIP()), lambda pkt: pkt.length == 4
        ),
...
class IE_QoS(IE_Base):
    ...
    fields_desc = [
        ...
        # 3GPP TS 24.008 10.5.6.5
        ConditionalField(
            XBitField("spare1", 0x00, 2), lambda p: p.length and p.length > 1
        ),
        ConditionalField(
            XBitField("delay_class", 0x000, 3), lambda p: p.length and p.length > 1
        ),
        ConditionalField(
            XBitField("reliability_class", 0x000, 3),
        ...
        ConditionalField(
            XBitField("peak_troughput", 0x0000, 4), lambda p: p.length and p.length > 2
        ),
        ConditionalField(BitField("spare2", 0, 1), lambda p: p.length and p.length > 2),
        ConditionalField(
            XBitField("precedence_class", 0x000, 3), lambda p: p.length and p.length > 2
        ),
        ConditionalField(
            XBitField("spare3", 0x000, 3), lambda p: p.length and p.length > 3
        ),
        ConditionalField(
            XBitField("mean_troughput", 0x00000, 5), lambda p: p.length and p.length > 3
        ),
        ConditionalField(
            XBitField("traffic_class", 0x000, 3), lambda p: p.length and p.length > 4
        ),
        ConditionalField(
            XBitField("delivery_order", 0x00, 2), lambda p: p.length and p.length > 4
        ),
        ConditionalField(
            XBitField("delivery_of_err_sdu", 0x000, 3),
        ...
        ConditionalField(
            ByteField("max_sdu_size", None), lambda p: p.length and p.length > 5
        ),
        ConditionalField(
            ByteField("max_bitrate_up", None), lambda p: p.length and p.length > 6
        ),
        ConditionalField(
            ByteField("max_bitrate_down", None), lambda p: p.length and p.length > 7
        ),
        ConditionalField(
            XBitField("redidual_ber", 0x0000, 4), lambda p: p.length and p.length > 8
        ),
        ConditionalField(
            XBitField("sdu_err_ratio", 0x0000, 4), lambda p: p.length and p.length > 8
        ),
        ConditionalField(
            XBitField("transfer_delay", 0x00000, 6), lambda p: p.length and p.length > 9
        ),
        ConditionalField(
            XBitField("traffic_handling_prio", 0x000, 2),
        ...
        ConditionalField(
            XBitField("spare4", 0x000, 3), lambda p: p.length and p.length > 12
        ),
        ConditionalField(
            BitField("signaling_indication", 0, 1), lambda p: p.length and p.length > 12
        ),
        ConditionalField(
            XBitField("source_stats_desc", 0x0000, 4),
            lambda p: p.length and p.length > 12,
        ),
        ConditionalField(
            ByteField("max_bitrate_down_ext", None),
            lambda p: p.length and p.length > 13,
        ),
        ConditionalField(
            ByteField("guaranteed_bitrate_down_ext", None),
            lambda p: p.length and p.length > 14,
        ),
        ConditionalField(
            ByteField("max_bitrate_up_ext", None), lambda p: p.length and p.length > 15
        ),
        ConditionalField(
            ByteField("guaranteed_bitrate_up_ext", None),
            lambda p: p.length and p.length > 16,
        ),
        ConditionalField(
            ByteField("max_bitrate_down_ext2", None),
            lambda p: p.length and p.length > 17,
        ),
        ConditionalField(
            ByteField("guaranteed_bitrate_down_ext2", None),
            lambda p: p.length and p.length > 18,
        ),
        ConditionalField(
            ByteField("max_bitrate_up_ext2", None), lambda p: p.length and p.length > 19
        ),
        ConditionalField(
            ByteField("guaranteed_bitrate_up_ext2", None),
...
class IE_APNRestriction(IE_Base):
    ...
    fields_desc = [
        ByteEnumField("ietype", 149, IEType),
        ShortField("length", 1),
...
class IE_RATType(IE_Base):
    name = "Rat Type"
    fields_desc = [
        ByteEnumField("ietype", 151, IEType),
        ShortField("length", 1),
...
class IE_MSTimeZone(IE_Base):
    name = "MS Time Zone"
    fields_desc = [
        ByteEnumField("ietype", 153, IEType),
        ShortField("length", None),
...
class IE_IMEI(IE_Base):
    name = "IMEI"
    fields_desc = [
        ByteEnumField("ietype", 154, IEType),
        ShortField("length", None),
...
class IE_MSInfoChangeReportingAction(IE_Base):
    ...
    fields_desc = [
        ByteEnumField("ietype", 181, IEType),
        ShortField("length", 1),
        ByteField("Action", 0),
    ]
...
class IE_DirectTunnelFlags(IE_Base):
    ...
    # 29.060 7.7.81
    fields_desc = [
        ByteEnumField("ietype", 182, IEType),
        ShortField("length", 1),
        BitField("spare", 0, 5),
        BitField("EI", 0, 1),
        BitField("GCSI", 0, 1),
...
class IE_BearerControlMode(IE_Base):
    ...
    fields_desc = [
        ByteEnumField("ietype", 184, IEType),
        ShortField("length", 1),
...
class IE_EvolvedAllocationRetentionPriority(IE_Base):
    ...
    # 29.060 7.7.91
    fields_desc = [
        ByteEnumField("ietype", 191, IEType),
        ShortField("length", 1),
        BitField("spare1", 0, 1),
        BitField("PCI", 0, 1),
...
class IE_CharginGatewayAddress(IE_Base):
    ...
    fields_desc = [
        ...
        ShortField("length", 4),
        ConditionalField(
            IPField("ipv4_address", "127.0.0.1"), lambda pkt: pkt.length == 4
        ),
        ConditionalField(IP6Field("ipv6_address", "::1"), lambda pkt: pkt.length == 16),
    ]
...
bind_layers(
    GTPHeader, GTPSupportedExtensionHeadersNotification, gtp_type=31, S=1
)  # noqa: E501
