...
class BGPFieldIPv4(Field):
    ...
    def m2i(self, pkt, m):
        ...
        ip = b"".join(
            m[i + 1 : i + 2] if i < mask2iplen_res else b"\x00" for i in range(4)
        )  # noqa: E501
...
class BGPFieldIPv6(Field):
    ...
    def m2i(self, pkt, m):
        mask = orb(m[0])
        ip = b"".join(
            m[i + 1 : i + 2] if i < self.mask2iplen(mask) else b"\x00"
            for i in range(16)
        )  # noqa: E501
...
class _BGPInvalidDataException(Exception):
    ...
    def __init__(self, details):
        Exception.__init__(
            self, "Impossible to build packet from the given data" + details
        )
...
class BGPHeader(Packet):
    ...
    def guess_payload_class(self, payload):
        return _get_cls(
            _bgp_cls_by_type.get(self.type, conf.raw_layer), conf.raw_layer
        )  # noqa: E501
...
class BGPCapORFBlock(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.orf_number is None:
            count = len(
                self.entries
            )  # orf_type (1 byte) + send_receive (1 byte)  # noqa: E501
...
class BGPCapORF(BGPCapability):
    ...
    fields_desc = [
        ...
        BGPCapORFBlockPacketListField(
            "orf", [], BGPCapORFBlock, length_from=lambda p: p.length
        ),
    ]
...
class BGPCapGracefulRestart(BGPCapability):
    ...
    match_subclass = True
    fields_desc = [
        ByteEnumField("code", 64, _capabilities),
        ByteField("length", None),
...
class BGPCapFourBytesASN(BGPCapability):
    ...
    match_subclass = True
    fields_desc = [
        ByteEnumField("code", 65, _capabilities),
        ByteField("length", 4),
        IntField("asn", 0),
    ]
...
class BGPAuthenticationInformation(Packet):
    ...
    fields_desc = [
        ByteField("authentication_code", 0),
...
class BGPOptParam(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketField("param_value", None, BGPCapability), lambda p: p.param_type == 2
        ),
        # It"s obsolete, but one can use it provided that
        # param_type == 1.
        ConditionalField(
            PacketField("authentication_data", None, BGPAuthenticationInformation),
    ...
    def post_build(self, p, pay):
        ...
        if self.param_length is None:
            if self.param_value is None and self.authentication_data is None:
                ...
                length = (
                    len(p) - 2
                )  # parameter type (1 byte) - parameter length (1 byte)
...
class BGPOpen(BGP):
    ...
    fields_desc = [
        ...
        BGPOptParamPacketListField(
            "opt_params", [], BGPOptParam, length_from=lambda p: p.opt_param_len
        ),
    ]
...
class BGPPAExtCommTwoOctetASSpecific(Packet):
    ...
    fields_desc = [
        ShortField("global_administrator", 0),
        IntField("local_administrator", 0),
    ]  # noqa: E501
...
class BGPPAExtCommFourOctetASSpecific(Packet):
    ...
    fields_desc = [
        IntField("global_administrator", 0),
        ShortField("local_administrator", 0),
    ]  # noqa: E501
...
class BGPPAExtCommunity(Packet):
    ...
    fields_desc = [
        ...
        _TypeLowField(
            "type_low", None, enum_from=lambda x: _get_ext_comm_subtype(x.type_high)
        ),
        _ExtCommValuePacketField(
            "value", None, Packet, type_from=lambda x: (x.type_high, x.type_low)
        ),
    ]
    ...
    def post_build(self, p, pay):
        ...
        if self.type_low is None and self.value is not None:
            high, low = _ext_high_low_dict.get(
                self.value.__class__, (0x00, 0x00)
            )  # noqa: E501
...
class BGPPAExtComms(Packet):
    ...
    fields_desc = [
        _ExtCommsPacketListField("extended_communities", [], BGPPAExtCommunity)
    ]
...
class BGPPAMPReachNLRI(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            IPField("nh_v4_addr", "0.0.0.0"),
            lambda x: x.afi == 1 and x.nh_addr_len == 4,
        ),
        ConditionalField(
            IP6Field("nh_v6_addr", "::"), lambda x: x.afi == 2 and x.nh_addr_len == 16
        ),
        ConditionalField(
            IP6Field("nh_v6_global", "::"), lambda x: x.afi == 2 and x.nh_addr_len == 32
        ),
        ConditionalField(
            IP6Field("nh_v6_link_local", "::"),
...
class BGPPAMPUnreachNLRI_IPv6(Packet):
    ...
    fields_desc = [BGPNLRIPacketListField("withdrawn_routes", [], BGPNLRI_IPv6)]
...
class BGPPAAS4Path(Packet):
    ...
    name = "AS4_PATH"
    fields_desc = [
        ByteEnumField("segment_type", 2, {1: "AS_SET", 2: "AS_SEQUENCE"}),
        ByteField("segment_length", None),
        FieldListField("segment_value", [], IntField("asn", 0)),
    ]
...
class _PathAttrPacketField(PacketField):
    ...
    def m2i(self, pkt, m):
        ...
        # Unassigned
            ...
            (type_code >= 30 and type_code <= 39)
            or (type_code >= 41 and type_code <= 127)
            or (type_code >= 129 and type_code <= 254)
        ):
            ret = conf.raw_layer(m)
...
class BGPPathAttr(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ...
            lambda x: x.type_flags is not None and has_extended_length(x.type_flags),
        ),
        ConditionalField(
            ByteField("attr_len", None),
            lambda x: x.type_flags is not None
            and not has_extended_length(x.type_flags),
        ),
...
class BGPUpdate(BGP):
    ...
    fields_desc = [
        FieldLenField(
            "withdrawn_routes_len", None, length_of="withdrawn_routes", fmt="!H"
        ),
        ...
        BGPPathAttrPacketListField(
            "path_attr", [], BGPPathAttr, length_from=lambda p: p.path_attr_len
        ),
