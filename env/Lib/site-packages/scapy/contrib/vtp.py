    ...
    fields_desc = [
        ...
        ConditionalField(
            PacketListField(
                ...
                - 12
                - (4 * ((pkt.vlannamelen + 3) // 4)),  # noqa: E501
            ),
...
class VTP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            SignedIntField("rev", 0), lambda pkt: pkt.code == 1 or pkt.code == 2
        ),
        # updater identity
        ConditionalField(IPField("uid", "192.168.0.1"), lambda pkt: pkt.code == 1),
        ConditionalField(
            VTPTimeStampField("timestamp", "930301000000"), lambda pkt: pkt.code == 1
        ),
        ConditionalField(
            StrFixedLenField("md5", b"\x00" * 16, 16), lambda pkt: pkt.code == 1
        ),
        ConditionalField(
            PacketListField("vlaninfo", [], VTPVlanInfo), lambda pkt: pkt.code == 2
        ),
