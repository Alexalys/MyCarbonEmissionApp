...
class LEBitField(BitField):
    ...
    def _check_field_type(self, pkt, index):
        ...
        try:
            ...
            if (
                type(next_field) is not LEBitField
                and next_field.__class__.__base__ is not LEBitField
            ):
                raise LEBitFieldSequenceException(
                    "field after field {} must "
            ...
            for octet in s[0:bf_total_byte_length]:
                ...
                while octet_bits_left:
                    ...
                    if field_required_bits == octet_bits_left:
                        ...
                        # pick required bits
                        cur_val |= (
                            octet & ((2**field_required_bits) - 1)
                        ) << cur_val_bit_idx  # noqa: E501
                        pkt.fields[field.name] = cur_val
...
class LEBitFieldLenField(LEBitField):
    ...
    def i2m(self, pkt, x):
        return (FieldLenField.i2m.__func__ if six.PY2 else FieldLenField.i2m)(
            self, pkt, x
        )  # noqa: E501
...
class EtherCatType12DLPDU(Packet):
    ...
    def post_build(self, pkt, pay):
        ...
        if data_len > 2047:
            raise ValueError(
                "payload size {} exceeds maximum length {} "
...
class EtherCat(Packet):
    ...
    def post_build(self, pkt, pay):
        ...
        if pad_len > 0:
            ...
            return (
                raw(_EtherCatLengthCalc(length=self.length, type=self.type))
                + pay
                + raw(pad)
            )
