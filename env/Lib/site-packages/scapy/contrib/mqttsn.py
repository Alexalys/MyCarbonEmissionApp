    ...
    def addfield(self, pkt, s, val):
        ...
        if (val < 2) or (val > 0xFFFF):
            raise Scapy_Exception(
                "%s: invalid length field value" % self.__class__.__name__
            )
        elif val > 0xFF:
...
class MQTTSNSubscribe(Packet):
    ...
    fields_desc = FLAG_FIELDS + [
        ...
        ConditionalField(
            StrFixedLenField("short_topic", None, length=2),
            lambda pkt: pkt.tid_type == 0b10,
        ),
        ConditionalField(
            StrLenField("topic_name", None, length_from=_mqttsn_length_from(3)),
...
class MQTTSNDisconnect(Packet):
    ...
    fields_desc = [
        ConditionalField(
            ...
            lambda pkt: hasattr(pkt.underlayer, "len")
            and ((pkt.underlayer.len is None) or (pkt.underlayer.len > 2)),
        ),
    ]
