...
class IE_Compound(IE_Base):
    fields_desc = IE_Base.fields_desc + [
        PacketListField(
            "IE_list", None, IE_Dispatcher, length_from=lambda pkt: pkt.length
        )
    ]
...
class IE_SDF_Filter(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(
            FieldLenField(
                "flow_description_length", None, length_of="flow_description"
            ),
            lambda pkt: pkt.FD == 1,
        ),
        ConditionalField(
            StrLenField(
                "flow_description",
                "",
                length_from=lambda pkt: pkt.flow_description_length,
            ),
            lambda pkt: pkt.FD == 1,
        ),
        ...
        ConditionalField(
            IntField("security_parameter_index", 0), lambda pkt: pkt.SPI == 1
        ),
...
class IE_ForwardingPolicy(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        FieldLenField(
            "policy_identifier_length", None, length_of="policy_identifier", fmt="B"
        ),
        StrLenField(
            "policy_identifier",
            "",
        ...
        MultipleTypeField(
            [
                (
                    ...
                    (
                        lambda x: x.length == 1,
                        lambda x, val: x.length == 1
                        or (x.length is None and val < 256),
                    ),
                ),
                (
                    ShortField("count", 0),
...
class IE_PFDContents(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(
            StrLenField("flow", "", length_from=lambda pkt: pkt.flow_length),
            lambda pkt: pkt.FD == 1,
        ),
        ConditionalField(
            FieldLenField("url_length", None, length_of="url"), lambda pkt: pkt.URL == 1
        ),
        ConditionalField(
            StrLenField("url", "", length_from=lambda pkt: pkt.url_length),
            lambda pkt: pkt.URL == 1,
        ),
        ConditionalField(
            FieldLenField("domain_length", None, length_of="domain"),
            lambda pkt: pkt.DN == 1,
        ),
        ConditionalField(
            StrLenField("domain", "", length_from=lambda pkt: pkt.domain_length),
            lambda pkt: pkt.DN == 1,
        ),
        ConditionalField(
            FieldLenField("custom_length", None, length_of="custom"),
            lambda pkt: pkt.CP == 1,
        ),
        ConditionalField(
            StrLenField("custom", "", length_from=lambda pkt: pkt.custom_length),
            lambda pkt: pkt.CP == 1,
        ),
        ConditionalField(
            FieldLenField("dnp_length", None, length_of="dnp"), lambda pkt: pkt.DNP == 1
        ),
        ConditionalField(
            StrLenField("dnp", "", length_from=lambda pkt: pkt.dnp_length),
            lambda pkt: pkt.DNP == 1,
        ),
        ConditionalField(
            FieldLenField("additional_flow_length", None, length_of="additional_flow"),
        ...
        ConditionalField(
            FieldLenField("additional_url_length", None, length_of="additional_url"),
            lambda pkt: pkt.AURL == 1,
        ),
        ConditionalField(
            StrLenField(
                "additional_url", "", length_from=lambda pkt: pkt.additional_url_length
            ),
            lambda pkt: pkt.AURL == 1,
        ),
        ConditionalField(
            FieldLenField(
                "additional_dn_dnp_length", None, length_of="additional_dn_dnp"
            ),
...
class IE_FqCSID(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        FieldListField(
            "csids", None, ShortField("csid", 0), count_from=lambda x: x.num_csids
        ),
        ExtraDataField("extra_data"),
    ]
...
class IE_OuterHeaderCreation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ByteField("spare", 0),
        ConditionalField(
            XIntField("TEID", 0), lambda x: x.GTPUUDPIPV4 == 1 or x.GTPUUDPIPV6 == 1
        ),
        ConditionalField(
            IPField("ipv4", 0),
            lambda x: x.IPV4 == 1 or x.UDPIPV4 == 1 or x.GTPUUDPIPV4 == 1,
        ),
        ConditionalField(
            IP6Field("ipv6", 0),
            lambda x: x.IPV6 == 1 or x.UDPIPV6 == 1 or x.GTPUUDPIPV6 == 1,
        ),
        ConditionalField(
            ShortField("port", 0), lambda x: x.UDPIPV4 == 1 or x.UDPIPV6 == 1
        ),
...
class IE_PacketRate(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(
            BitEnumField("ul_time_unit", "minute", 3, TimeUnit), lambda x: x.ULPR == 1
        ),
        ...
        ConditionalField(
            BitEnumField("dl_time_unit", "minute", 3, TimeUnit), lambda x: x.DLPR == 1
        ),
...
class IE_OuterHeaderRemoval(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(
            XBitField("spare", None, 7), lambda x: x.length is not None and x.length > 1
        ),
        ConditionalField(
            BitField("pdu_session_container", None, 1),
...
class IE_RemoteGTP_U_Peer(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        ConditionalField(
            FieldLenField("network_instance_length", 1, length_of="network_instance"),
            lambda x: x.NI == 1,
        ),
        ConditionalField(
            APNStrLenField(
                "network_instance", "", length_from=lambda x: x.network_instance_length
            ),
            lambda x: x.NI == 1,
        ),
        ExtraDataField("extra_data"),
...
class IE_MACAddressesDetected(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        FieldListField(
            "macs", None, MACField("mac", 0), count_from=lambda x: x.num_macs
        ),
        ExtraDataField("extra_data"),
    ]
...
class IE_TraceInformation(IE_Base):
    ...
    fields_desc = IE_Base.fields_desc + [
        ...
        FieldLenField(
            "triggering_events_length", None, length_of="triggering_events", fmt="B"
        ),
        StrLenField(
            "triggering_events", "", length_from=lambda x: x.triggering_events_length
        ),
        ByteField("session_trace_depth", 0),
        FieldLenField(
            "list_of_interfaces_length", None, length_of="list_of_interfaces", fmt="B"
        ),
        StrLenField(
            "list_of_interfaces", "", length_from=lambda x: x.list_of_interfaces_length
        ),
