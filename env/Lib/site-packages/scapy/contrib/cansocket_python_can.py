...
class PriotizedCanMessage(object):
    ...
    def __lt__(self, other):
        ...
        return self.msg.timestamp < other.msg.timestamp or (
            self.msg.timestamp == other.msg.timestamp and self.count < other.count
        )
...
class SocketsPool(object):
    ...
    def register(self, socket, *args, **kwargs):
        k = str(
            str(kwargs.get("bustype", "unknown_bustype"))
            + "_"
...
class SocketWrapper(can_BusABC):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rx_queue = (
            queue.PriorityQueue()
        )  # type: queue.PriorityQueue[PriotizedCanMessage]  # noqa: E501
        self.name = None
        self.prio_counter = 0
...
class PythonCANSocket(SuperSocket):
    desc = (
        "read/write packets at a given CAN interface " "using a python-can bus object"
    )
    ...
    def recv_raw(self, x=0xFFFF):
        ...
        hdr = (
            msg.is_extended_id << 31
            | msg.is_remote_frame << 30
            | msg.is_error_frame << 29
            | msg.arbitration_id
        )
    ...
    def select(sockets, *args, **kwargs):
        ...
        return [
            s
            for s in sockets
            if isinstance(s, PythonCANSocket) and not s.iface.rx_queue.empty()
        ]
