...
DCP_SUBOPTIONS = {
    # ip
    0x01: {0x00: "Reserved", 0x01: "MAC Address", 0x02: "IP Parameter"},
    # device properties
    0x02: {
        0x00: "Reserved",
...
class DCPBaseBlock(Packet):
    ...
    fields_desc = [
        ...
        MultiEnumField(
            "sub_option", 2, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
...
class DCPIPBlock(Packet):
    fields_desc = [
        ...
        PadField(
            StrLenField(
                "padding", b"\x00", length_from=lambda p: p.dcp_block_length % 2
            ),
            1,
            padwith=b"\x00",
        ),
...
class DCPMACBlock(Packet):
    fields_desc = [
        ByteEnumField("option", 1, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 1, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
...
class DCPNameOfStationBlock(Packet):
    fields_desc = [
        ...
        ShortEnumField("block_info", 0, BLOCK_INFOS),
        StrLenField(
            "name_of_station", "et200sp", length_from=lambda x: x.dcp_block_length - 2
        ),
        PadField(
            StrLenField(
                "padding", b"\x00", length_from=lambda p: p.dcp_block_length % 2
...
class DCPDeviceIDBlock(Packet):
    fields_desc = [
        ByteEnumField("option", 2, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 3, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
        LenField("dcp_block_length", None),
        ShortEnumField("block_info", 0, BLOCK_INFOS),
...
class DCPDeviceRoleBlock(Packet):
    fields_desc = [
        ByteEnumField("option", 2, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 4, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
...
class DeviceOption(Packet):
    fields_desc = [
        ByteEnumField("option", 2, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 5, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
    ]
...
class DCPAliasNameBlock(Packet):
    fields_desc = [
        ByteEnumField("option", 2, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 6, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
        FieldLenField(
            "dcp_block_length", None, length_of="alias_name", adjust=lambda p, x: x + 2
        ),
        ShortEnumField("block_info", 0, BLOCK_INFOS),
        StrLenField(
            "alias_name", "et200sp", length_from=lambda x: x.dcp_block_length - 2
        ),
        PadField(
            StrLenField(
                "padding", b"\x00", length_from=lambda p: p.dcp_block_length % 2
...
class DCPDeviceInstanceBlock(Packet):
    fields_desc = [
        ByteEnumField("option", 2, DCP_OPTIONS),
        MultiEnumField(
            "sub_option", 7, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ),
        LenField("dcp_block_length", 4),
        ShortEnumField("block_info", 0, BLOCK_INFOS),
...
def guess_dcp_block_class(packet, **kargs):
    ...
    class_switch_case = {
        ...
        # Device Inactive
        0x06: {0x00: "Reserved (0x00)", 0x01: "Device Initiative (0x01)"},
        # ALL Selector
...
class ProfinetDCP(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ByteEnumField("option", 2, DCP_OPTIONS), lambda pkt: pkt.service_type == 0
        ),
        ConditionalField(
            MultiEnumField(
                "sub_option", 3, DCP_SUBOPTIONS, fmt="B", depends_on=lambda p: p.option
        ...
        ConditionalField(
            LenField("dcp_block_length", 0), lambda pkt: pkt.service_type == 0
        ),
        # DCP SET REQUEST #
        ConditionalField(
        ...
        ConditionalField(
            ...
            and pkt.service_type == 0
            and pkt.option == 2
            and pkt.sub_option == 2,
        ),
        # MAC
        ConditionalField(
            MACField("mac", "00:00:00:00:00:00"),
        ...
        ConditionalField(
            StrLenField(
                "name_of_station", "et200sp", length_from=lambda x: x.dcp_block_length
            ),
            lambda pkt: pkt.service_id == 5
            and pkt.service_type == 0
            and pkt.option == 2
            and pkt.sub_option == 2,
        ),
        ...
        ConditionalField(
            StrLenField(
                "alias_name", "et200sp", length_from=lambda x: x.dcp_block_length
            ),
            lambda pkt: pkt.service_id == 5
            and pkt.service_type == 0
            and pkt.option == 2
            and pkt.sub_option == 6,
        ),
