...
class DAGMCObjUnknown(Packet):
    ...
    fields_desc = [
        ByteEnumField("otype", 3, DAGMC_OBJTYPE),
...
class RPLOptDAGMC(_DAGMetricContainer):
    ...
    fields_desc = [
        ByteEnumField("otype", 2, RPLOPTSSTR),
        ByteField("len", None),
        _PhantomAutoPadField("autopad", 0),
        _OptionsField(
            "options", [], DAGMCObjUnknown, 8, length_from=lambda pkt: 8 * pkt.len
        ),
    ]
