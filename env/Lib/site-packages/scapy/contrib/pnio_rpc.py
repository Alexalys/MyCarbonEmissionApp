...
F_PARAMETERS_BLOCK_ID = [
    "No_F_WD_Time2_No_F_iPar_CRC",
    "No_F_WD_Time2_F_iPar_CRC",
    "F_WD_Time2_No_F_iPar_CRC",
    "F_WD_Time2_F_iPar_CRC",
    "reserved_4",
    "reserved_5",
    "reserved_6",
    "reserved_7",
...
class FParametersBlock(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField(
            "F_CRC_Length", 0, 2, ["CRC-24", "depreciated", "CRC-32", "reserved"]
        ),
...
class ARBlockReq(Block):
    ...
    fields_desc = [
        ...
        FieldLenField(
            "StationNameLength", None, fmt="H", length_of="CMInitiatorStationName"
        ),
        StrLenField(
            "CMInitiatorStationName", "", length_from=lambda pkt: pkt.StationNameLength
        ),
    ]
    # default block_type value
    block_type = 0x0101
...
class IOCRAPI(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "IOCSs", [], IOCRAPIObject, count_from=lambda p: p.NumberOfIOCS
        ),
    ]
...
class ExpectedSubmoduleBlockReq(Block):
    ...
    fields_desc = [
        ...
        FieldLenField("NumberOfAPIs", None, fmt="H", count_of="APIs"),
        PacketListField(
            "APIs", [], ExpectedSubmoduleAPI, count_from=lambda p: p.NumberOfAPIs
        ),
    ]
    # default block_type value
    block_type = 0x0104
...
class NDRData(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "blocks", [], _guess_block_class, length_from=lambda p: p.args_length
        ),
    ]
...
class PNIOServiceReqPDU(Packet):
    ...
    def can_handle(cls, pkt, rpc):
        ...
        # type = 0 => request
        if rpc.getfieldval("type") == 0 and str(rpc.object_uuid).startswith(
            "dea00000-6c97-11d1-8271-"
        ):
            return True
        return False
...
class PNIOServiceResPDU(Packet):
    ...
    fields_desc = [
        EndiannessField(
            IntEnumField("status", 0, ["OK"]), endianess_from=dce_rpc_endianess
        ),
        NDRData,
    ]
    overload_fields = {
    ...
    def can_handle(cls, pkt, rpc):
        """heuristic guess_payload_class"""
        # type = 2 => response
        if rpc.getfieldval("type") == 2 and str(rpc.object_uuid).startswith(
            "dea00000-6c97-11d1-8271-"
        ):
            return True
        return False
