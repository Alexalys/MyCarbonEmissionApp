...
class MACsecSA(object):
    ...
    def make_iv(self, pkt):
        ...
        if self.xpn_en:
            tmp_pn = (self.pn & 0xFFFFFFFF00000000) | (
                pkt[MACsec].pn & 0xFFFFFFFF
            )  # noqa: E501
            tmp_iv = self.ssci + struct.pack("!Q", tmp_pn)
            return bytes(
                bytearray(
                    [a ^ b for a, b in zip(bytearray(tmp_iv), bytearray(self.salt))]
                )
            )  # noqa: E501
        else:
    ...
    def decrypt(self, orig_pkt, assoclen=None):
        ...
        decryptor = Cipher(
            algorithms.AES(self.key), modes.GCM(iv, icv), backend=default_backend()
        ).decryptor()
...
class MACsec(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            XShortEnumField("type", None, ETHER_TYPES), lambda pkt: "type" in pkt.fields
        ),
    ]
