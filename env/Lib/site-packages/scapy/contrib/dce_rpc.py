    ...
    def set_endianess(self, pkt):
        ...
        if isinstance(end, str) and end:
            if isinstance(self.fld, UUIDField):
                self.fld.uuid_fmt = (
                    UUIDField.FORMAT_LE if end == "<" else UUIDField.FORMAT_BE
                )
            else:
...
class DceRpc(Packet):
    ...
    fields_desc = [
        ...
        EndiannessField(
            UUIDField("object_uuid", None), endianess_from=dce_rpc_endianess
        ),
        EndiannessField(
            UUIDField("interface_uuid", None), endianess_from=dce_rpc_endianess
        ),
        ...
        EndiannessField(
            IntField("interface_version", 1), endianess_from=dce_rpc_endianess
        ),
        ...
        EndiannessField(
            XShortField("interface_hint", 0xFFFF), endianess_from=dce_rpc_endianess
        ),
        EndiannessField(
            XShortField("activity_hint", 0xFFFF), endianess_from=dce_rpc_endianess
        ),
        EndiannessField(
            LenField("frag_len", None, fmt="H"), endianess_from=dce_rpc_endianess
        ),
