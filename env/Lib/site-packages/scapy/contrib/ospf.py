...
class OSPF_Hdr(Packet):
    ...
    def answers(self, other):
        if (
            isinstance(other, OSPF_Hdr) and self.area == other.area and self.type == 5
        ):  # Only acknowledgements answer other packets
...
class LLS_Crypto_Auth(LLS_Generic_TLV):
    ...
    fields_desc = [
        ShortField("type", 2),
        FieldLenField(
            "len", 20, fmt="B", length_of=lambda x: x.authdata + 4
        ),  # noqa: E501
...
class FieldLenField32Bits(FieldLenField):
    def i2repr(self, pkt, x):
        return (
            repr(x) if not x else str(FieldLenField.i2h(self, pkt, x) << 2) + " bytes"
        )  # noqa: E501
...
class OSPF_LLS_Hdr(Packet):
    ...
    fields_desc = [
        XShortField("chksum", None),
        FieldLenField32Bits(
            "len", None, length_of="llstlv", adjust=lambda pkt, x: (x + 4) >> 2
        ),  # noqa: E501
        PacketListField(
            "llstlv", [], _LLSGuessPayloadClass, length_from=lambda x: (x.len << 2) - 4
        ),
    ]  # noqa: E501
...
class OSPF_LSReq_Item(Packet):
    ...
    fields_desc = [
        IntEnumField("type", 1, _OSPF_LStypes),
        IPField("id", "1.1.1.1"),
        IPField("adrouter", "1.1.1.1"),
    ]
...
class OSPFv3_Router_LSA(OSPF_BaseLSA):
    ...
    fields_desc = [
        ...
        OSPFv3OptionsField(),
        PacketListField(
            "linklist", [], OSPFv3_Link, length_from=lambda pkt: pkt.len - 24
        ),
    ]
