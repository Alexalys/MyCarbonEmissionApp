    ...
    def __repr__(self):
        ...
        if self.sources or self.sinks:
            ...
            if self.sources:
                s += "%s%s" % (
                    ct.punct(",").join(
                        ct.field_name(s.name) for s in self.sources
                    ),  # noqa: E501
                    ct.field_value(">"),
                )
            s += ct.layer_name("#")
        ...
        if self.high_sources or self.high_sinks:
            ...
            if self.high_sources:
                s += "%s%s" % (
                    ct.punct(",").join(
                        ct.field_name(s.name) for s in self.high_sources
                    ),  # noqa: E501
                    ct.field_value(">>"),
                )
            s += ct.layer_name("#")
        ...
        if self.trigger_sources or self.trigger_sinks:
            ...
            if self.trigger_sources:
                s += "%s%s" % (
                    ct.punct(",").join(
                        ct.field_name(s.name) for s in self.trigger_sources
                    ),  # noqa: E501
                    ct.field_value("^"),
                )
            s += ct.layer_name("#")
            if self.trigger_sinks:
                s += "%s%s" % (
                    ct.field_value("^"),
                    ct.punct(",").join(
                        ct.field_name(s.name) for s in self.trigger_sinks
                    ),
                )  # noqa: E501
            s += ct.punct("]")
...
class TermSink(Sink):
    ...
    def _start_windows(self):
        if not self.opened:
            ...
            cmd = (
                '$app = Start-Process PowerShell -ArgumentList \'-command &{$host.ui.RawUI.WindowTitle=\\"%s\\";Get-Content \\"%s\\" -wait}\' -passthru; echo $app.Id'
                % (self.name, self.__f.replace("\\", "\\\\"))
            )  # noqa: E501
            proc = subprocess.Popen(
                [conf.prog.powershell, cmd], stdout=subprocess.PIPE
            )  # noqa: E501
