...
class LayersList(List[Type["scapy.packet.Packet"]]):
    def __init__(self):
        ...
        self._backup_dict = (
            {}
        )  # type: Dict[Type[Packet], List[Tuple[Dict[str, Any], Type[Packet]]]]  # noqa: E501
...
class CacheInstance(Dict[str, Any], object):
    ...
    def summary(self):
        # type: () -> str
        return "%s: %i valid items. Timeout=%rs" % (
            self.name,
            len(self),
            self.timeout,
        )  # noqa: E501
...
def _prompt_changer(attr, val, old):
    ...
    try:
        ...
        pass
    try:
        apply_ipython_style(get_ipython())  # type: ignore
    except NameError:
        pass
...
def _set_conf_sockets():
    ...
    if conf.use_bpf:
        ...
        conf.L3socket6 = functools.partial(L3bpfSocket, filter="ip6")  # type: ignore
    ...
    if LINUX:
        ...
        conf.L3socket6 = functools.partial(L3PacketSocket, filter="ip6")  # type: ignore
...
class Conf(ConfClass):
    ...
    iface = Interceptor(
        "iface", None, _iface_changer
    )  # type: 'scapy.interfaces.NetworkInterface'  # type: ignore  # noqa: E501
    layers = LayersList()
    ...
    #: history file
    histfile = os.getenv(
        "SCAPY_HISTFILE", os.path.join(os.path.expanduser("~"), ".scapy_history")
    )
