    ...
    if WINDOWS:
        ...
        def __init__(self):
            self._fd = ctypes.windll.kernel32.CreateEventA(
                None, 0, 0, "SelectableObject %s" % random.random()
            )

        def call_release(self):
            if ctypes.windll.kernel32.PulseEvent(ctypes.c_void_p(self._fd)) == 0:
                warning(ctypes.FormatError())

        def _close_fd(self):
            if (
                self._fd
                and ctypes.windll.kernel32.CloseHandle(ctypes.c_void_p(self._fd)) == 0
            ):
                warning(ctypes.FormatError())
                self._fd = None
...
def select_objects(inputs, remain):
    ...
    for i in list(inputs):
        if getattr(i, "__selectable_force_select__", False):
            ...
            raise TypeError("Invalid type: %s (must extend SelectableObject)")
    if natives:
    ...
    if events:
        ...
        if len(events) == 1:
            res = ctypes.windll.kernel32.WaitForSingleObject(
                ctypes.c_void_p(events[0].fileno()), remainms
            )
        else:
            res = ctypes.windll.kernel32.WaitForMultipleObjects(
                len(events),
                (ctypes.c_void_p * len(events))(*[x.fileno() for x in events]),
                False,
                remainms,
            )
...
class Automaton_metaclass(type):
    def __new__(cls, name, bases, dct):
        ...
        for ioev in cls.iosupersockets:
            setattr(
                ...
                _ATMT_to_supersocket(ioev.atmt_as_supersocket, ioev.atmt_ioname, cls),
            )  # noqa: E501
    ...
    def build_graph(self):
        ...
        for c, k, v in (
            [("purple", k, v) for k, v in self.conditions.items()]
            + [("red", k, v) for k, v in self.recv_conditions.items()]  # noqa: E501
            + [("orange", k, v) for k, v in self.ioevents.items()]  # noqa: E501
        ):
            for f in v:
...
class Automaton(six.with_metaclass(Automaton_metaclass)):
    ...
    def send(self, pkt):
        if self.state.state in self.interception_points:
            ...
            cmd = Message(
                type=_ATMT_Command.INTERCEPT, state=self.state, pkt=pkt
            )  # noqa: E501
            ...
            if cmd.type == _ATMT_Command.REJECT:
                ...
                pkt = cmd.pkt
                self.debug(
                    3, "INTERCEPT: packet replaced by: %s" % pkt.summary()
                )  # noqa: E501
    ...
    def _run_condition(self, cond, *args, **kargs):
        try:
            self.debug(
                5, "Trying %s [%s]" % (cond.atmt_type, cond.atmt_condname)
            )  # noqa: E501
