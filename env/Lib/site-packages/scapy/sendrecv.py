    ...
    def __init__(
        ...
        if _flood:
            ...
            if isinstance(pkt, types.GeneratorType) or prebuild:
                ...
                self.tobesent = SetGen(pkt) if not isinstance(pkt, Gen) else pkt
        ...
        while retry >= 0:
            ...
            if multi:
                remain = [
                    p
                    for p in itertools.chain(*six.itervalues(self.hsent))
...
def srp(
    ...
    **kargs  # type: Any
    ...
    iface = resolve_iface(iface or conf.iface)
    s = iface.l2socket()(
        promisc=promisc, iface=iface, filter=filter, nofilter=nofilter, type=type
    )
    result = sndrcv(s, x, *args, **kargs)
    s.close()
    return result
...
def __sr_loop(
    ...
    **kargs  # type: Any
    ...
    try:
        while True:
            ...
            if verbose > 1:
                print("\rsend...\r", end=" ")
            res = srfunc(
                pkts, timeout=timeout, verbose=0, chainCC=True, *args, **kargs
            )  # noqa: E501
    ...
    if verbose and n > 0:
        print(
            ct.normal(
                "\nSent %i packets, received %i packets. %3.1f%% hits."
                % (n, r, 100.0 * r / n)
            )
        )  # noqa: E501
...
def srflood(
    ...
    **kargs  # type: Any
    ...
    iface = resolve_iface(iface or conf.iface)
    s = iface.l3socket()(
        promisc=promisc, filter=filter, iface=iface, nofilter=nofilter
    )  # noqa: E501
...
def sr1flood(
    ...
    **kargs  # type: Any
    ...
    iface = resolve_iface(iface or conf.iface)
    s = iface.l3socket()(
        promisc=promisc, filter=filter, nofilter=nofilter, iface=iface
    )  # noqa: E501
...
class AsyncSniffer(object):
    ...
    def _setup_thread(self):
        ...
        self.thread = Thread(
            target=self._run, args=self.args, kwargs=self.kwargs, name="AsyncSniffer"
        )
        ...
        **karg  # type: Any
        ...
        if offline is not None:
            ...
            if isinstance(offline, str):
                # Single file
                offline = [offline]
            if isinstance(offline, list) and all(
                isinstance(elt, str) for elt in offline
            ):
                # List of files
                sniff_sockets.update(
                    (
                ...
                sniff_sockets[
                    ...
                    if flt is None
                        ...
                        tcpdump(
                            offline, args=["-w", "-"], flt=flt, getfd=True, quiet=quiet
                        )
                    )
                ] = offline
            else:
        ...
        if not sniff_sockets or iface is not None:
            ...
            if isinstance(iface, list):
                sniff_sockets.update(
                    (L2socket(type=ETH_P_ALL, iface=ifname, **karg), ifname)
                    for ifname in iface
                )
            elif isinstance(iface, dict):
                sniff_sockets.update(
                    (L2socket(type=ETH_P_ALL, iface=ifname, **karg), iflabel)
                    for ifname, iflabel in six.iteritems(iface)
                )
            else:
                sniff_sockets[L2socket(type=ETH_P_ALL, iface=iface, **karg)] = iface
        ...
        if not all(select_func == sock.select for sock in sniff_sockets):
            warning(
                "Warning: inconsistent socket types ! "
...
def bridge_and_sniff(
    ...
    **kargs  # type: Any
    ...
    for arg in ["opened_socket", "offline", "iface"]:
        if arg in kargs:
            log_runtime.warning(
                "Argument %s cannot be used in " "bridge_and_sniff() -- ignoring it.",
                arg,
            )
            del kargs[arg]
    ...
    def prn_send(pkt):
        ...
        try:
            ...
            log_runtime.warning(
                "Cannot forward packet [%s] received on %s",
                pkt.summary(),
                pkt.sniffed_on,
                exc_info=True,
            )
