...
/i7kinChIXSAmRgA==\n""",
    ...
    def get_local_dict(cls):
        return {
            x: y.name for (x, y) in six.iteritems(cls.__dict__) if isinstance(y, File)
        }
    ...
    def get_URL_dict(cls):
        return {
            x: y.URL for (x, y) in six.iteritems(cls.__dict__) if isinstance(y, File)
        }
...
def run_test(
    test, get_interactive_session, theme, verb=3, ignore_globals=None, my_globals=None
):
    """An internal UTScapy function to run a single test"""
    start_time = time.time()
    test.output, res = get_interactive_session(
        test.test.strip(),
...
def usage():
    print(
        """Usage: UTscapy [-m module] [-f {text|ansi|HTML|LaTeX|live}] [-o output_file]
