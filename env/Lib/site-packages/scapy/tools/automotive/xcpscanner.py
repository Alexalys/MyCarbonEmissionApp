...
def parse_inputs():
    ...
    parser.add_argument(
        "--sniff_time",
        "-t",
        help="Duration in milliseconds a sniff is waiting " "for a response.",
        type=int,
        default=100,
    )
    parser.add_argument("channel", help="Linux SocketCAN interface name, e.g.: vcan0")
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Display information during scan"
    )
    parser.add_argument(
        "--broadcast",
        "-b",
        action="store_true",
        help="Use Broadcast-message GetSlaveId instead of " 'default "Connect"',
    )
...
def main():
    ...
    try:
        ...
        results = (
            scanner.scan_with_get_slave_id()
            if scanner_params.broadcast
            else scanner.scan_with_connect()
        )  # Blocking
