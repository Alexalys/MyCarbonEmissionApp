...
def main():
    ...
    if (
        ...
        or end is None
        or channel is None
        or (PYTHON_CAN and interface is None)
    ):
        usage(True)
    ...
    if not extended_can_id and (end >= 0x800 or start >= 0x800):
        print(
            "Standard can identifiers must be < 0x800.\n"
    ...
    try:
        if PYTHON_CAN:
            if python_can_args:
                interface_string = "CANSocket(bustype=" "'%s', channel='%s', %s)" % (
                    interface,
                    channel,
                    python_can_args,
                )
                ...
                interface_string = "CANSocket(bustype=" "'%s', channel='%s')" % (
                    interface,
                    channel,
                )
