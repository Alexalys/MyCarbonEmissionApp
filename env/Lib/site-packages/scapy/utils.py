...
def do_graph(
        ...
        if WINDOWS:
            ...
            with ContextManagerSubprocess(conf.prog.display):
                target = subprocess.Popen(
                    [conf.prog.display], stdin=subprocess.PIPE
                ).stdin
    ...
    if isinstance(target, str):
        if target.startswith("|"):
            target = subprocess.Popen(
                target[1:].lstrip(), shell=True, stdin=subprocess.PIPE
            ).stdin
            ...
            for j in range(len(lstcol)):
                for k in range(len(lstcol)):
                    if i != j or j != k or k != i:
                        yield trans(
                            lstcol[(i + j) % len(lstcol)],
                            lstcol[(j + k) % len(lstcol)],
                            lstcol[(k + i) % len(lstcol)],
                        )  # noqa: E501
...
class EnumElement:
    ...
    def __repr__(self):
        # type: () -> str
        return "<%s %s[%r]>" % (
            self.__dict__.get("_name", self.__class__.__name__),
            self._key,
            self._value,
        )  # noqa: E501
...
def import_object(obj=None):
    ...
    if obj is None:
        obj = sys.stdin.read()
    return six.moves.cPickle.loads(
        zlib.decompress(base64_bytes(obj.strip()))
    )  # noqa: E501
...
class RawPcapReader:
    ...
    def __init__(self, filename, fdesc=None, magic=None):  # type: ignore
        ...
        ):  # little endian, nanosecond-precision  # noqa: E501
            self.endian = "<"
            self.nano = True
        else:
    ...
    def _read_packet(self, size=MTU):
        ...
        return (
            self.f.read(caplen)[:size],
            RawPcapReader.PacketMetadata(
    ...
    def read_packet(self, size=MTU):
        # type: (int) -> Packet
        return cast(Packet, self._read_packet()[0])
...
class RawPcapNgReader(RawPcapReader):
    ...
    PacketMetadata = collections.namedtuple(
        "PacketMetadata", ["linktype", "tsresol", "tshigh", "tslow", "wirelen"]
    )
    ...
    def _read_block_tail(self, blocklen):
        ...
        if blocklen % 4:
            ...
            warning(
                "PcapNg: bad blocklen %d (MUST be a multiple of 4. "
...
class RawPcapWriter:
    ...
    def __init__(
        ...
        if isinstance(filename, str):
            ...
            if gz:
                self.f = cast(
                    _ByteStream, gzip.open(filename, append and "ab" or "wb", 9)
                )
            else:
    ...
    def write(self, pkt):
        ...
        if isinstance(pkt, bytes):
            ...
            for p in IterSocket(pkt).iter:
                ...
                if not isinstance(p, bytes) and self.linktype != conf.l2types.get(
                    type(p), None
                ):
                    warning(
                        "Inconsistent linktypes detected!"
    ...
    def write_packet(
        ...
        # type: (...) -> None
        self._write_packet(
            bytes(packet), sec=sec, usec=usec, caplen=caplen, wirelen=wirelen
        )
...
class PcapWriter(RawPcapWriter):
    ...
    def write_packet(
        ...
        if hasattr(packet, "time"):
            if sec is None:
                ...
                usec = int(
                    round(
                        (packet.time - sec)
                        * (1000000000 if self.nano else 1000000)  # type: ignore
                    )
                )
        if usec is None:
            usec = 0
        ...
        self._write_packet(rawpkt, sec=sec, usec=usec, caplen=caplen, wirelen=wirelen)
...
def tcpdump(
    ...
    if pktlist is None:
        ...
        try:
            tmpfile.writelines(iter(lambda: pktlist.read(1048576), b""))  # type: ignore
    ...
    if proc is None:
        ...
        return
    if dump:
        data = b"".join(iter(lambda: proc.stdout.read(1048576), b""))  # type: ignore
        proc.terminate()
        return data
    if getproc:
        return proc
...
def make_table(*args, **kargs):
    ...
    return __make_table(
        lambda l: "%%-%is" % l, lambda l: "%%-%is" % l, "", *args, **kargs
    )
