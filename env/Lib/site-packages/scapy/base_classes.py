...
class OID(Gen[str]):
    ...
    def __iterlen__(self):
        # type: () -> int
        return reduce(
            operator.mul, (max(y - x, 0) + 1 for (x, y) in self.cmpt), 1
        )  # noqa: E501
...
class Packet_metaclass(_Generic_metaclass):
    def __new__(
        ...
        if (
            ...
            current_fld = dct[
                "fields_desc"
            ]  # type: List[Union['scapy.fields.Field'[Any, Any], Packet_metaclass]]  # noqa: E501
        ...
        newcls.aliastypes = [newcls] + getattr(newcls, "aliastypes", [])  # type: ignore
