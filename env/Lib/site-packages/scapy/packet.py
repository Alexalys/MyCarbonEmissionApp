...
class Packet(
    six.with_metaclass(
        Packet_metaclass, BasePacket, _CanvasDumpExtended  # type: ignore
    )
):
    __slots__ = [
        "time",
    ...
    def upper_bonds(self):
        ...
        for fval, upper in self.payload_guess:
            print(
                ...
                % (
                    upper.__name__,
                    ", ".join("%-12s" % ("%s=%r" % i) for i in six.iteritems(fval)),
                )
            )  # noqa: E501
    ...
    def __init__(
        ...
        **fields  # type: Any
        ...
        self.raw_packet_cache_fields = (
            None
        )  # type: Optional[Dict[str, Any]]  # noqa: E501
    ...
    def copy_fields_dict(self, fields):
        ...
        if fields is None:
            return None
        return {
            fname: self.copy_field_value(fname, fval)
        ...
        def loop(todo, done, self=self):
            ...
            if todo:
                ...
                for payl in payloads:
                    ...
                    pkt = self.clone_with(payload=payl, share_time=share_time, **done2)
                    yield pkt
    ...
    def convert_to(self, other_cls, **kwargs):
        ...
        raise TypeError(
            "Cannot convert {} to {}".format(type(self).__name__, other_cls.__name__)
        )
    ...
    def convert_packet(cls, pkt, **kwargs):
        ...
        raise TypeError(
            "Cannot convert {} to {}".format(type(pkt).__name__, cls.__name__)
        )
...
def split_top_down(
    ...
    **fval  # type: Any
    ...
    if lower in upper._overload_fields:
        ...
        if any(
            k not in ofval or ofval[k] != v for k, v in six.iteritems(fval)
        ):  # noqa: E501
            return
        upper._overload_fields = upper._overload_fields.copy()
...
def explore(layer=None):
    ...
    if layer is None:  # GUI MODE
        ...
        if action == "contribs":
            ...
            tree = defaultdict(
                list
            )  # type: Dict[str, Union[List[Any], Dict[str, Any]]]  # noqa: E501
        ...
        previous = (
            []
        )  # type: List[Dict[str, Union[List[Any], Dict[str, Any]]]]  # noqa: E501
        while True:
            # Generate tests & form
    ...
    for f in obj.fields_desc:
        ...
        if verbose and isinstance(cur_fld, EnumField) and hasattr(cur_fld, "i2s"):
            if len(cur_fld.i2s or []) < 50:
                long_attrs.extend(
                    "%s: %d" % (strval, numval)
                    for numval, strval in sorted(six.iteritems(cur_fld.i2s))
                )
            ...
            if verbose:
                cur_i2s = cur_fld.i2s_multi.get(
                    cur_fld.depends_on(cast(Packet, obj if is_pkt else obj())), {}
                )
                if len(cur_i2s) < 50:
                    long_attrs.extend(
                        "%s: %d" % (strval, numval)
                        for numval, strval in sorted(six.iteritems(cur_i2s))
                    )
...
def ls(
    ...
    if obj is None or is_string:
        ...
        try:
            fields = _pkt_ls(obj, verbose=verbose)  # type: ignore
            is_pkt = isinstance(obj, Packet)
            # Print
